...
| --- | --- | --- | --- | --- | --- |
| Q-affine-fixed-loci-20260906 | Q — specialize `GObjects(G, Schemes(R))` to represented affine schemes and compute common fixed subschemes from the fixed ideal generated by `g^*(a)-a` on chosen group/algebra generators | ChatGPT continuation `2026-09-06-affine-fixed-loci`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/group/g_objects.py`; `src/dzack_research/preamble/categories/schemes/schemes.py`; `tests/schemes/test_group_actions_fixed_loci.py`; `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `92fdd1a4`; expanded only to regenerate the AGENTS-required live preamble survey before Q source edits. A released the self-algebra/augmentation substrate at `4150e30b`, and S released represented affine scheme maps at `dbef76cc`. Implement the first literal PORT §13.3 fixed-subscheme release before invariant-ring quotients. | 2026-09-05T18:22:00Z |
