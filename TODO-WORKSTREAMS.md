...
| --- | --- | --- | --- | --- | --- |
| Q-affine-invariant-quotient-20260906 | Q — for a supported finite linear action on affine polynomial space, compute a finite presentation of the invariant algebra, represent `Spec(A) -> Spec(A^G)`, and realize the affine categorical-quotient factorization; first specimen the `C_2` coordinate swap | ChatGPT continuation `2026-09-06-affine-invariant-quotient-handoff-0406`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/schemes/schemes.py`; `tests/schemes/test_group_action_quotients.py`; `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `a1d0fc79`; consumes Q fixed-locus release `28b23405`, A action substrate `4150e30b`, and S represented affine maps `dbef76cc`. The claimed working tree now has the finite invariant presentation, quotient morphism, affine factorization, and zero-variable boundary; quotient/fixed-locus regressions pass 11/11. A normal source-commit attempt reached the repository-wide Sage-aware mypy gate and was blocked by 8,341 existing/imported errors. The new regression now passes standalone `mypy-sage.ini`, and the failed gate reported no direct error on the added `schemes.py` quotient/helper ranges. Keep the source uncommitted and the claim active; do not bypass the gate. | 2026-09-05T20:25:13Z |
...
| Q | `28b23405`: `GObjects(G, Schemes(R))` equips a represented affine scheme with a fresh acted copy carrying represented coordinate pullbacks; the generic chosen-presentation relator verifier rejects invalid generator images. For a chosen finite group generating set and a finite chosen algebra framing, `fixed_ideal()` is generated by `g^*(a)-a` on those generators and `fixed_subscheme()` is the represented common equalizer. The `C_2` coordinate swap on `QQ[x,y]` gives the diagonal ideal `(x-y)`, and the new affine-action regressions together with the generic G-object regressions pass 8/8. | The invariant-quotient implementation is complete in the claimed working tree and its quotient/fixed-locus regressions pass 11/11, but it is not a release until a source commit passes the normal gate. The current gate stops on the repository-wide Sage-aware mypy baseline (8,341 errors); the new regression itself is standalone-mypy clean and the added quotient/helper source ranges have no direct gate diagnostic. | Claimed / source gate blocked: `Q-affine-invariant-quotient-20260906` |
