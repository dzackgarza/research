...
| A | `1be70fcb`: owned groups multiply by composition (permutation and GAP word engines reversed, words and cosets read in the owned order, mixed-engine morphisms represented through inverses); permutation-group elements act on points; `G.action_on`, `G.orbit`, `G.orbits`, `G.stabilizer`, `G.is_transitive`; G-objects verify a supplied action on the chosen relators. | Torsor membership and transporters; the scheme specialization `GObjects(G, Schemes)` for Q. | Unclaimed |
