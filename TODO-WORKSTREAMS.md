...
| --- | --- | --- | --- | --- | --- |
| P-presented-pushout-20260905 | P — make commutative-algebra pushouts accept represented maps out of chosen presented algebras through their endpoint Hom owners | ChatGPT continuation of repo TODO priorities; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/algebras/free_algebras.py`; `tests/algebras/test_relative_presentations.py` | `2845d8fe51eface4a19406c95271dbe8cc006670` / clean P paths; concrete `PresentedAlgebraMorphism` rejection reproduced on released substrate | 2026-09-05T10:28:34Z |
| A-H-group-law-and-substrate-20260905 | A/H — owned groups multiply by composition (permutation law reversed, relators reversed), permutation elements act on points, `G.action_on`; tensor label sets iterate, internal Homs have generating sets, so Tor/Ext go green | Claude Code session 014pGhSMwfktxgNks1sDaZeo; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/group/groups.py`; `src/dzack_research/preamble/categories/group/g_sets.py`; `src/dzack_research/preamble/categories/group/g_objects.py`; `src/dzack_research/preamble/categories/sets/set_categories.py`; `src/dzack_research/preamble/categories/modules/module_morphisms/module_morphisms.py`; `src/dzack_research/preamble/categories/modules/internal_hom.py`; `src/dzack_research/preamble/categories/modules/derived_functors.py`; `tests/groups/`; `tests/modules/test_derived_functors.py`; `tests/modules/test_tensor_hom_adjunction.py`; `src/dzack_research/preamble/categories/modules/pure/modules.py`; `src/dzack_research/preamble/categories/modules/framed/finitely_generated/finitely_presented_modules.py` | `4b1786dbbec6f8bd8937d8ff34dc916052ad308a` / claimed | 2026-09-05T10:15:36Z |
...
| R | Read-only P integration found that a principal ideal in the relative presented quotient `QQ[t][x,y]/(xy-t)` fails in `CommutativeIdeal`: Sage `syz` does not accept the quotient engine. Consequently the public ring-quotient path cannot yet consolidate the relative presentation. | Repair represented ideals/quotients on presented quotient engines while preserving the original algebra base; then quotient prime-local unit/ideal/residue operations. | Unclaimed |
...
| P | `66466ec7`: relative `xy=t` presentation and special fiber released. Its algebra-side successive-quotient backend also preserves `T`, appends old and new relations, and returns quotient maps commuting with the `T`-structure through two successive quotients. The public ring-side quotient is now an explicit R prerequisite because its ideal constructor fails on this quotient engine. | Remove the concrete `AlgebraMorphism` restriction from the pushout backend: maps out of a chosen presented source come from its represented algebra Hom and are currently rejected solely by Python element class. | `P-presented-pushout-20260905` |
...
| A | `4174e261`: `GObjects(G, C)` owns actions as group morphisms `G -> Aut_C(X)` with `action_of`, `act`, `is_invariant`, and the equivariant Hom `Mor_G(X, Y)` inside `Mor_C(X, Y)`; `GSets(G)` is `GObjects(G, Sets())`, `GroupModules(R, G)` refines `GObjects(G, Modules(R))`; the finite G-set constructor accepts owned finite ordered sets. | Scheme specialization for Q: `GObjects(G, Schemes)` needs a scheme automorphism Hom that accepts a morphism datum; also `g(point)` for owned permutation-group elements and `G.action_on(points)` (both in `group/groups.py`), and, ruled 2026-09-05 and verified at `007784d3`: actions are left actions with rho(s1 s2) = rho(s1) rho(s2), checked on the chosen relators; the owned permutation-group law still follows GAP's left-to-right product, so `group/groups.py` must reverse it (and the relator words it reads) to match. | Unclaimed |
