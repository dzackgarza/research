...
| --- | --- | --- | --- | --- | --- |
| G-principal-open-restrictions-20260906 | G — connect scheme-level distinguished opens to structure-sheaf sections, represent restriction maps on functions and localized modules, and verify composition on a principal-open overlap | ChatGPT continuation `2026-09-06-g-principal-open-restrictions-0437`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/schemes/ringed_spaces.py`; `src/dzack_research/preamble/categories/schemes/schemes.py`; `tests/schemes/test_structure_sheaf_restrictions.py`; `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `1b24ca8a`; consumes S represented distinguished-open/affine-map substrate through `dbef76cc`, R localization maps through `cf3a11fe`, and M localized-module maps through `65fe44e6`. No R/M contract edit is reserved: this release must build from those public maps or return a missing operation to its owner. | 2026-09-05T20:52:23Z |
...
| G | Structure-sheaf local values in PORT §§8.4, 9.4. | Active first release: identify scheme-level principal opens with their localized section rings and represent compatible function/module restriction maps, including composition on a principal-open overlap. | Claimed: `G-principal-open-restrictions-20260906` |
