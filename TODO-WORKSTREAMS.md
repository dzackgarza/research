...
| D | `8298c13d`: represented localizations satisfy `Omega^1_{S^{-1}A/R} = S^{-1}Omega^1_{A/R}` with the localization source/unit retained. `f06a8c7d`: for a selected finite commutative presentation `A=P/I`, `KahlerDifferentials(A)` now retains the genuine conormal source `A tensor_P I ~= I/I^2`, `A tensor_P Omega^1_{P/R}`, the map `f |-> df`, and the quotient projection to `Omega^1_{A/R}`; residue-field base change gives cotangent/tangent data. For `QQ[t][x,y]/(xy-t)`, the origin and a smooth point have tangent dimensions 2 and 1 and fibered conormal ranks 0 and 1. Affine `singular_subscheme()` is represented under explicit field-base, finite-presentation, and equidimensionality hypotheses as `V(Fitt_d Omega)`; for `QQ[x,y]/(xy)` it is exactly `V(x,y)`. Two exact prerequisites were repaired at their owners: nested polynomial presentation membership flattens `R[t][x_1,...,x_n]` to the canonical multivariate Singular regime before module-membership testing, and prime-spectrum point construction preserves an owned quotient ideal so its exact primality backend is used. The focused D/R/S gate passes 3/3; the full schemes suite is 44 passed with only the pre-D `xy=t` scalar-base pushout failure, and an isolated `a107a334` archive reproduces the broader module/de Rham/ring failures. The staged commit gate reaches the known global Sage/mypy baseline (8,723 errors in 152 files) without checker modification or suppression. | Represent the supported flatness/local-freeness criterion next, beginning with the `xy=t` family over `QQ[t]`, then use that hypothesis to extend the relative smooth/nonsmooth locus beyond the field-base case. | Released: `f06a8c7d` |
