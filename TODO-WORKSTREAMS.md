...
| --- | --- | --- | --- | --- | --- |
| C-refine-audit-20260905 | C — adopt and finish interrupted constructor/refinement audit; restore pre-edit megadoc gate | ChatGPT continuation of repo TODO priorities; `/home/dzack/research` | write: `TODO-PRIORITIES.md`; `archives/preamble/src-TODO.md`; `justfile`; `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html`; `src/dzack_research/preamble/categories/_lattice.py`; `src/dzack_research/preamble/categories/algebras/algebras.py`; `src/dzack_research/preamble/categories/algebras/derivations.py`; `src/dzack_research/preamble/categories/algebras/free_algebras.py`; `src/dzack_research/preamble/categories/algebras/graded_algebras.py`; `src/dzack_research/preamble/categories/algebras/kahler_differentials.py`; `src/dzack_research/preamble/categories/algebras/restricted_scalars.py`; `src/dzack_research/preamble/categories/definite_lattices.py`; `src/dzack_research/preamble/categories/divisors/cartier_divisor_groups.py`; `src/dzack_research/preamble/categories/divisors/class_groups.py`; `src/dzack_research/preamble/categories/divisors/divisor_groups.py`; `src/dzack_research/preamble/categories/divisors/picard_groups.py`; `src/dzack_research/preamble/categories/divisors/weil_divisor_groups.py`; `src/dzack_research/preamble/categories/forms/forms.py`; `src/dzack_research/preamble/categories/functions/lebesgue_graded.py`; `src/dzack_research/preamble/categories/functors/abelianization.py`; `src/dzack_research/preamble/categories/functors/group_scalar_change.py`; `src/dzack_research/preamble/categories/group/groups.py`; `src/dzack_research/preamble/categories/group/profinite/absolute_galois_group.py`; `src/dzack_research/preamble/categories/lattice_morphisms.py`; `src/dzack_research/preamble/categories/lattices.py`; `src/dzack_research/preamble/categories/modules/connections.py`; `src/dzack_research/preamble/categories/modules/fractional_ideals.py`; `src/dzack_research/preamble/categories/modules/framed/finitely_generated/finitely_presented_modules.py`; `src/dzack_research/preamble/categories/modules/framed/formed/discriminant_modules.py`; `src/dzack_research/preamble/categories/modules/framed/formed/form_modules.py`; `src/dzack_research/preamble/categories/modules/framed/formed/torsion_form_modules.py`; `src/dzack_research/preamble/categories/modules/framed/fraction_field_quotients.py`; `src/dzack_research/preamble/categories/modules/framed/framed_free_modules.py`; `src/dzack_research/preamble/categories/modules/group_modules/group_lattices.py`; `src/dzack_research/preamble/categories/modules/internal_hom.py`; `src/dzack_research/preamble/categories/modules/localizations.py`; `src/dzack_research/preamble/categories/modules/module_morphisms/module_morphisms.py`; `src/dzack_research/preamble/categories/modules/pure/modules.py`; `src/dzack_research/preamble/categories/modules/pure/torsion_modules.py`; `src/dzack_research/preamble/categories/rings/commutative_algebra.py`; `src/dzack_research/preamble/categories/rings/commutative_ideals.py`; `src/dzack_research/preamble/categories/rings/number_fields.py`; `src/dzack_research/preamble/categories/rings/ring_foundation.py`; `src/dzack_research/preamble/categories/schemes/schemes.py`; `src/dzack_research/preamble/categories/sets/set_categories.py`; `src/dzack_research/utilities/megadoc.py` | `74f7884b35f7e89233b1f001535f7de98ff8d6e3` / adopted dirty checkpoint | 2026-09-05T08:12:45Z |
| A-left-actions-20260905 | A — actions are left actions: verify the homomorphism law of a supplied action on the group's chosen relators in left-composition order; tests import the session module only | Claude Code session 014pGhSMwfktxgNks1sDaZeo; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/group/g_objects.py`; `tests/groups/test_g_objects.py`; `tests/modules/test_derived_functors.py` | `f388dd14ad793fad9efeb78679bc763feb57fa7b` / claimed | 2026-09-05T09:54:11Z |
...
| --- | --- | --- | --- |
| C | Interrupted constructor/refinement audit adopted; lattice import now reaches the original graph-rendering gate. Generated DOT uses unquoted category identifiers containing `.`, which Graphviz rejects. | Repair DOT identifier quoting in the megadoc generator, regenerate/read the artifact, then verify the remaining `refine()` census and constructor semantics before checkpointing. | `C-refine-audit-20260905` |
...
| A | `4174e261`: `GObjects(G, C)` owns actions as group morphisms `G -> Aut_C(X)` with `action_of`, `act`, `is_invariant`, and the equivariant Hom `Mor_G(X, Y)` inside `Mor_C(X, Y)`; `GSets(G)` is `GObjects(G, Sets())`, `GroupModules(R, G)` refines `GObjects(G, Modules(R))`; the finite G-set constructor accepts owned finite ordered sets. | Scheme specialization for Q: `GObjects(G, Schemes)` needs a scheme automorphism Hom that accepts a morphism datum; also `g(point)` for owned permutation-group elements and `G.action_on(points)` (both in `group/groups.py`), and the action side (left/right) convention for the composition law, which is not verified at construction. | Unclaimed |
...
| H | `d4c9af0a`: `Tor(n, M, N)` and `Ext(n, M, N)` in `modules/derived_functors.py` from the free resolution, tensor functor, internal Hom and cochain cohomology; every specimen is red because the shared substrate is: `CartesianProductOfFamily` label sets do not iterate (so `TensorProduct` of presented modules fails), a non-matrix-space internal Hom has no module generating set, and `FinitelyPresentedTorsionModules(ZZ)` uses a second copy of `ZZ`. | Green Tor/Ext once C releases `sets/set_categories.py`, `module_morphisms.py`, `internal_hom.py`, `torsion_modules.py`; then functoriality in the resolved argument and `ext`/`tor`/`projective_dimension` methods on presented modules (`pure/modules.py`). | Unclaimed |
