...
| --- | --- | --- | --- | --- | --- |
| M-localized-framing-20260905 | M — exact localized equality/vanishing, fibers, and exact-map kernel transport | ChatGPT continuation `2026-09-05-9b7b0ec7`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/modules/localizations.py`; `src/dzack_research/preamble/categories/modules/framed/finitely_generated/finitely_presented_modules.py`; `src/dzack_research/preamble/categories/modules/pure/modules.py`; `src/dzack_research/preamble/categories/modules/module_morphisms/module_morphisms.py`; `src/dzack_research/preamble/categories/functors/module_localization.py`; `tests/rings/test_commutative_algebra.py`; generated-set write: `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `6353897caac4f7092861b99f7fa6487243c56fed`; fiber bookkeeping plus equality/vanishing/localized ideals are green. With the R dispatch repaired locally, 5/6 of the combined public-localization/Nakayama/exactness checks pass. The sole failure is kernel transport: `LocalizationKernelComparison` localizes the known source-kernel inclusion and then calls generic `.image()`, which recomputes a presented kernel over `ZZ[1/2]` and falls into the field-polynomial Singular backend. Since localization is exact and the source kernel already carries a chosen inclusion, transport that subobject structure rather than recomputing its image | 2026-09-05T15:22:51Z |
| R-module-localize-dispatch-20260905 | R — restore the public ring-side `S^{-1}M` dispatch needed by M exact-map tests | ChatGPT continuation `2026-09-05-9b7b0ec7`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/rings/ring_foundation.py` | `6353897caac4f7092861b99f7fa6487243c56fed`; a local import of `module_localization_functor` in `LocalizationRings.ParentMethods.localize_module()` removes the runtime `NameError` without creating an import cycle. The general finite-carrier public localization regression now passes; the exactness test proceeds into M and exposes kernel-subobject transport instead | 2026-09-05T15:22:51Z |
...
| R | `8e4b7ae7`: represented quotient ideals compute exact module relations by lifting generators and defining equations to the polynomial cover; quotient Krull dimension and nested quotient computations use the same cover data. The `xy=0` origin is prime/maximal, its prime localization has the expected local units/maximal ideal/residue map, and finite/prime localization preserves algebra bases and validates external fraction denominators. The pinned R integration set passes 8/8. The current tiny dispatch repair makes the public `localize_module()` path execute and its finite-carrier regression pass. | Land the dispatch repair once the mandatory repository gate is green; M can already consume the working-tree repair and now owns the remaining exactness failure. | `R-module-localize-dispatch-20260905` |
| M | Against R's released `8e4b7ae7`, the recovered framing repair plus cyclic-annihilator criterion gives exact localized fraction equality and finite-generator vanishing for the represented presentation backend. The fiber retains the exact prime-localized module used before residue-field base change. With the local R dispatch repair, public localization, finite-carrier torsion, Nakayama, fiber, equality/zero, and localized-ideal checks pass; 5/6 of the combined exactness probe is green. Normal source commits remain frozen by the unchanged whole-repository Sage mypy baseline. | Preserve `ModuleSubobjects` through `ModuleLocalizationFunctor` and have localized-kernel provenance return that transported subobject directly. The current `.image()` recomputation is mathematically redundant and fails only because it asks the wrong presentation backend over `ZZ[1/2]`. | `M-localized-framing-20260905` |
