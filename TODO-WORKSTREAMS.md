...
| --- | --- | --- | --- | --- | --- |
| M-localized-framing-20260905 | M — exact localized equality/vanishing, then fiber bookkeeping through the same prime localization | ChatGPT continuation `2026-09-05-9b7b0ec7`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/modules/localizations.py`; `src/dzack_research/preamble/categories/modules/framed/finitely_generated/finitely_presented_modules.py`; `src/dzack_research/preamble/categories/modules/pure/modules.py`; `tests/rings/test_commutative_algebra.py`; generated-set write: `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `609558433c628b1f8a98a0f07715ebb75a86b9fc`; the persistent localization regression and the formerly blocked localized-ideal regression pass 2/2. Exact equality/zero is green for finite generated and prime-complement localizations, including `(ZZ/6)_(2)` versus `(ZZ/6)_(5)`. Two normal commit attempts, the second with full `.envrc`, both freeze at the repository Sage mypy baseline; full environment still reports 8069 errors in 146 files and no diagnostic in either changed M implementation file. The next existing M regression fails only at `fiber._preamble_fiber_localization is local_quotient`: `FinitelyGeneratedModules.fiber()` constructs `localized` and immediately base-changes it without recording that represented localization on the fiber | 2026-09-05T15:15:59Z |
| R-module-localize-dispatch-20260905 | R — restore the public ring-side `S^{-1}M` dispatch needed by M exact-map tests | ChatGPT continuation `2026-09-05-9b7b0ec7`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/rings/ring_foundation.py` | `7577a4fedf70166980bb49b777003e6f5a9af0ca`; `LocalizationRings.ParentMethods.localize_module()` calls `module_localization_functor(self)` but the name is not imported, so both the general finite-carrier localization regression and module-localization exactness regression stop at `NameError` before reaching M. `ring_foundation.py` is clean and otherwise unclaimed; use a local import to avoid the ring/module import cycle | 2026-09-05T15:19:20Z |
...
| R | `8e4b7ae7`: represented quotient ideals compute exact module relations by lifting generators and defining equations to the polynomial cover; quotient Krull dimension and nested quotient computations use the same cover data. The `xy=0` origin is prime/maximal, its prime localization has the expected local units/maximal ideal/residue map, and finite/prime localization preserves algebra bases and validates external fraction denominators. The pinned R integration set passes 8/8. | Repair the clean ring-side `localize_module()` dispatch: it currently calls an undefined `module_localization_functor`, blocking M's public localization and exactness tests before any module code runs. | `R-module-localize-dispatch-20260905` |
| M | Against R's released `8e4b7ae7`, the recovered framing repair plus cyclic-annihilator criterion gives exact localized fraction equality and finite-generator vanishing for the represented presentation backend. The fiber now retains the exact prime-localized module used before residue-field base change; the first-class localization/fiber test, exact equality/zero regression, and localized-ideal regression pass 3/3. Normal source commits remain frozen by the unchanged whole-repository Sage mypy baseline: 8069 errors in 146 files even with the full repo `.envrc`; neither changed M implementation file is diagnosed. | Continue exact maps through the public localization API after the small R dispatch repair. Nakayama is already green; the exactness regression is presently blocked only by R's undefined `module_localization_functor` name. | `M-localized-framing-20260905` |
