...
| --- | --- | --- | --- | --- | --- |
| S-affine-morphism-endpoints-20260905 | S — preserve owned endpoints when wrapping native affine identity/structure/Spec morphisms, so affine Spec and fiber-product maps live in the intended owned Hom objects | ChatGPT continuation `2026-09-05-92d036b8`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/schemes/schemes.py`; `tests/schemes/test_affine_schemes.py`; generated-set write in pinned clone: `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `4761c4e03d22586ad37542b88b1b42c2f1137b01`; direct owner-level FiberProduct probe reaches `Spec(R[x])` then fails because `refine_scheme_morphism` rebuilds the Hom from Sage-native endpoints instead of the known owned schemes | 2026-09-05T11:30:59Z |
...
| R | P integration isolates two presented-quotient gaps. A principal ideal in `QQ[t][x,y]/(xy-t)` fails in `CommutativeIdeal` because Sage `syz` rejects the quotient engine. Separately, `QQ(i) coproduct_QQ QQ(i) = QQ[x0,x1]/(x0^2+1,x1^2+1)` is constructed and detected non-domain, but the generic quotient engine has no `krull_dimension()` implementation. | Repair represented ideals/syzygies and Krull dimension on presented quotient engines while preserving the original algebra base; then quotient prime-local unit/ideal/residue operations. | Unclaimed |
...
| P | `66466ec7`: relative `xy=t`, special fiber, and scalar-preserving successive algebra quotients. `86ed6a99`: free/presented pushout spans. `289d4756`: `K.as_algebra()` uses canonical public `QQ`, keeps the original number-field engine, and carries a chosen primitive-element polynomial presentation plus the finite power-basis module model; relative fields use an absolute primitive element and exact structural isomorphism. The four P regressions pass 4/4. The immutable Gaussian coproduct now constructs and is non-domain before reaching R's separate Krull-dimension gap. | Continue §8.4 at the remaining owned-map boundary: affine-Spec and fiber-product factorization through the represented ring/algebra Hom owners, without changing R-owned quotient algorithms. | Released: `289d4756` |
...
| S | Affine Spec, spaces, opens, and affine products in PORT §§8.4–10. Read-only integration of P's released maps finds that `Spec(R[x])` loses its owned endpoints while wrapping the native structure morphism: `refine_scheme_morphism` asks for an owned Hom on Sage-native scheme endpoints and raises `TypeError`. The public fiber-product expectation also has a separate missing `FiberProduct` session export, left to the export owner. | Thread the already-known owned domain/codomain through native affine morphism wrappers, then recheck the fiber-product universal map directly through its owner API. | `S-affine-morphism-endpoints-20260905` |
...
| A | `a0078ac8`: `1be70fcb`: owned groups multiply by composition (permutation and GAP word engines reversed, words and cosets read in the owned order, mixed-engine morphisms represented through inverses); permutation-group elements act on points; `G.action_on`, `G.orbit`, `G.orbits`, `G.stabilizer`, `G.is_transitive`; G-objects verify a supplied action on the chosen relators. Induction/coinduction decompose along owned left cosets (cosets are single objects); the G-set functors' set maps compose in Sets. | Spec at `8573cc83`: expectations spell functors/adjunctions/constructions on categories and group modules as `Modules(R[G])`. Next: the owned group algebra `R[G]` with inclusion and augmentation maps; coextension of scalars; `Modules(ZZ[H]).induction/coinduction/restriction` and the trivial-action/invariants functors as scalar change; retire `GroupModules`, `functors/group_actions.py`, `functors/group_induction.py`; category-sited G-set functors (`FiniteGSets(G).orbits_functor()`, `FiniteSets().trivial_action(G)`, ...). The expectation tests spell `InductionFunctor(subgroup, group)` with no base ring: the functor over the whole fibration of group modules over rings, not one fibre. Torsor membership and transporters; the scheme specialization `GObjects(G, Schemes)` for Q. | Unclaimed |
