...
| --- | --- | --- | --- | --- | --- |
| P-relative-xy-t-20260905 | P — relative polynomial presentation over a free parameter algebra; first specimen `QQ[t][x,y]/(xy-t)` and its `t=0` fiber | ChatGPT continuation of repo TODO priorities; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/algebras/free_algebras.py`; `tests/algebras/test_relative_presentations.py` | `b2859d6b1e7627e0b8c7fd14f7e50247c21b4591` / source edit in progress; writable regression path added after confirming the construction expectation subtree is immutable | 2026-09-05T10:22:00Z |
...
| P | Chosen presentations and quotient/pushout/scalar-change implementations in PORT §8.4. A direct `QQ[t][x,y]/(xy-t)` specimen retains its relative base and chosen presentation, but its nested quotient engine cannot reduce even zero: Singular rejects the coefficient ring and toy Buchberger only reduces over fields. | Flatten relative polynomial presentations over a free polynomial parameter algebra while retaining the parameter structure map and relative lift; verify the `t=0` special fiber. | `P-relative-xy-t-20260905` |
