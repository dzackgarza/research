...
| S | `7b40b82a`: native affine scheme morphisms are wrapped in the Hom of their stated owned endpoints; identities, structure maps, Spec maps, affine points and product projections retain those endpoints and affine points carry their evaluation pullback. `Spec` uses owned-input identity rather than equality for its cache and a bare ring remains over its stated owned self. Adopted affine point domains construct their structure map from the scalar injection. A fresh 18,613-line survey was read before the edit; eight focused affine/projective/product/subscheme/FiberProduct specimens pass 8/8 on the clean surveyed snapshot. Missing `Coproduct`/`FiberProduct` session exports remain export-owner defects, and the generic slice identity still has a separate abstract-category `Endsets` failure. | Construct the released `xy=t` family and its `t=0` special fiber as actual morphisms over the parameter scheme, with the commuting base-change square; R is needed only for the later local ring/stalk. | Released: `7b40b82a` |
