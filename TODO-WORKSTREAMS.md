...
| --- | --- | --- | --- | --- | --- |
| P-number-field-presentation-20260905 | P — give `K.as_algebra()` the canonical `QQ` scalar parent and the primitive-element chosen finite presentation, enabling coproducts such as `QQ(i) coproduct_QQ QQ(i)` | ChatGPT continuation `2026-09-05-92d036b8`; pinned source checkout from `/home/dzack/research` | write: `src/dzack_research/preamble/categories/rings/number_fields.py`; `src/dzack_research/preamble/categories/algebras/free_algebras.py`; `tests/algebras/test_relative_presentations.py`; generated-set write in pinned clone: `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | source dependency `1be70fcb2f7e20f87ffaf3beab712a699cbbb40d`; main board at `ca5683c0a7837da19368131b73ad04cc9fd55217`; pinned clone provides the stable survey snapshot while A writes disjoint main-checkout paths | 2026-09-05T10:45:37Z |
...
| P | `66466ec7`: relative `xy=t`, special fiber, and scalar-preserving successive algebra quotients released. `86ed6a99`: represented free/presented pushout spans released. Read-only diagnosis of the next immutable specimen found that `K.as_algebra()` manufactures an unrefined second `QQ`, so its finite-presentation/framing categories are parameterized by the wrong parent; the field engine already supplies the canonical polynomial lift needed by `_PresentedAlgebraParent`. | Construct nontrivial number-field algebras over canonical public `QQ` from their primitive-element polynomial while retaining the field computation engine and power-basis module model; verify the Gaussian coproduct. The remaining `krull_dimension()` failure of that coproduct is a separate ring-invariant prerequisite. | `P-number-field-presentation-20260905` |
...
| A | `a0078ac8`: `1be70fcb`: owned groups multiply by composition (permutation and GAP word engines reversed, words and cosets read in the owned order, mixed-engine morphisms represented through inverses); permutation-group elements act on points; `G.action_on`, `G.orbit`, `G.orbits`, `G.stabilizer`, `G.is_transitive`; G-objects verify a supplied action on the chosen relators. Induction/coinduction decompose along owned left cosets (cosets are single objects); the G-set functors' set maps compose in Sets. | The expectation tests spell `InductionFunctor(subgroup, group)` with no base ring: the functor over the whole fibration of group modules over rings, not one fibre. Torsor membership and transporters; the scheme specialization `GObjects(G, Schemes)` for Q. | Unclaimed |
