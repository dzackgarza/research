...
| P | `66466ec7`: relative `xy=t`, special fiber, and scalar-preserving successive algebra quotients released. `86ed6a99`: commutative-algebra pushouts now validate maps by their represented endpoint Homs rather than one concrete element class, so both free-source and chosen-presented-source spans work; the two relative-presentation regressions pass live and the original free-source span passes directly. The public ring-side quotient remains an explicit R prerequisite. | The immutable composition specimen next fails earlier at `QQ(i) coproduct_QQ QQ(i)`: an unframed finite field algebra carries no represented coproduct backend. Recover its exact chosen coefficient presentation for coproduct/pushout construction, then continue to affine-Spec map factorization. | Released: `86ed6a99` |
