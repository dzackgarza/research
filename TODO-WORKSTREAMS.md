...
| --- | --- | --- | --- | --- | --- |
| M-localized-framing-20260905 | M — preserve transported localization framing, then make presented-module fraction equality/vanishing exact through cyclic annihilators; first release: an inverted element kills its presented module | ChatGPT continuation `2026-09-05-9b7b0ec7`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/modules/localizations.py`; `src/dzack_research/preamble/categories/modules/framed/finitely_generated/finitely_presented_modules.py`; `tests/rings/test_commutative_algebra.py`; generated-set write: `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `41b437a778496a0932b8d1f63965a3e504656afc`; framing repair passes `test_ideal_localization_extension_contraction_colon_and_saturation`. Exact probes now cover finite generated and prime-complement localization in both directions: `QQ[x]/(x^2)` dies after inverting `x`, `QQ[x]/(x+1)` survives; at `(x) <= QQ[x,y]`, `R/(x)` survives while `R/(y)` dies; `(ZZ/6)_(2)` survives while `(ZZ/6)_(5)` dies. Selected finite-presentation `is_zero()` now defers outside its PID invariant-factor regime so localized finite generators reach the exact criterion. Add persistent regressions, regenerate the claimed docs, then retry the normal source gate | 2026-09-05T15:08:28Z |
...
| M | Against R's released `8e4b7ae7`, the recovered initialization-order repair preserves source framing and the blocked localized-ideal extension/contraction regression passes. Presented fraction equality now uses `Ann_R(d) ∩ S`: prime complements test whether a cyclic annihilator generator escapes the prime, while finitely generated `S=<s_i>` tests `prod(s_i) in radical(Ann_R(d))`, with a direct annihilator witness fallback when radical computation is unavailable. The finite-generator zero test checks the source generating family, and selected finite presentations defer to it outside the PID invariant-factor regime. Probes verify both truth values for `QQ[x]` localization, prime-complement localization in `QQ[x,y]`, and `(ZZ/6)_(p)`. | Add the exact equality/zero regressions to the existing commutative-algebra localization tests, regenerate the coherent megadoc snapshot, and retry the normal source gate on current HEAD. The separate fiber-cache identity failure remains after all localization/rank assertions and belongs to the next M fiber bookkeeping release. | `M-localized-framing-20260905` |
