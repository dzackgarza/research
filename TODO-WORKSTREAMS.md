...
| M | `55c16f43`: represented finite-presentation localization has exact fraction equality and finite-generator vanishing from cyclic annihilators, retains the exact prime-localized module used by fibers, preserves `ModuleSubobjects` and their chosen inclusion through `ModuleLocalizationFunctor`, and transports kernels directly instead of recomputing an image over the localized ring. `65fe44e6`: selected Fitting ideals commute with localization. `da64cc2e`: `Modules.is_flat()` now decides the represented field/PID regimes; over a PID it uses the exact theorem flat iff torsion-free. Chosen integral algebra presentations over a PID provide that torsion-free decision by eliminating the relative variables from the defining ideal and testing the scalar-map kernel. | Construct actual finite-free/local-free trivialization isomorphisms for supported finitely presented PID modules and finite algebras; projectivity/local freeness must be witnessed by represented comparison maps, not only predicates. | Released: `da64cc2e` |
