...
| --- | --- | --- | --- | --- | --- |
| S-xy-family-20260905 | S — represent `xy=t` over the parameter scheme and its `t=0` special fiber with the commuting base-change square; make affine Spec carry an explicit scheme base | ChatGPT continuation `2026-09-05-S-xy-t-handoff-2124`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/schemes/schemes.py`; `src/dzack_research/preamble/categories/schemes/affine_spec.py`; `tests/schemes/test_affine_schemes.py`; generated-set write in pinned clone: `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `3807077322e8408d8eb154aed763d4cf02d1b6af`; direct family probe shows `Spec(T)` must be viewed as the terminal T-scheme when T is the scalar ring, while Sage already supplies distinct `Spec(A,S)` native schemes for explicit bases | 2026-09-05T13:25:34Z |
...
| S | `7b40b82a`: owned affine morphism endpoints, point pullbacks and affine FiberProduct maps released. The next family probe constructs the relative algebra but fails at `Spec(A)`: for parameter ring `T=QQ[t]`, `Spec(T)` defaults to its QQ-scheme view, while the family needs the same spectrum as the terminal T-scheme. Sage natively supports the required distinction as `Spec(A,S)`. Missing `Coproduct`/`FiberProduct` session exports and generic-slice `Endsets` remain separate owners. | Add an explicit scheme-base parameter to owned affine Spec, key its cache by coordinate-algebra/base identities, thread `AffineSpecFunctor(R)` through that view, then verify `xy=t` and its `t=0` fiber square. | `S-xy-family-20260905` |
