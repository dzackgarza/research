...
| M | Against R's released `8e4b7ae7`, the recovered framing repair plus cyclic-annihilator criterion now gives exact localized fraction equality and finite-generator vanishing for the represented presentation backend. The committed-test candidate passes 2/2 with the localized-ideal regression and covers finite generated, prime-complement, and integer-prime examples in both directions. Normal source commits remain frozen by the unchanged whole-repository Sage mypy baseline: 8069 errors in 146 files even with the full repo `.envrc`; neither changed M implementation file is diagnosed. | The next M defect is exact fiber bookkeeping, not localization mathematics: `FinitelyGeneratedModules.fiber()` already computes `localized = self.localize_at_prime(point)` before residue-field base change, but fails to preserve that object on the returned fiber as required by the existing regression. Record that represented intermediate and rerun the first-class localization/fiber test; keep source changes uncommitted until the mandatory global gate is green rather than bypassing it. | `M-localized-framing-20260905` |
