...
| --- | --- | --- | --- | --- | --- |
| M-localized-framing-20260905 | M — preserve transported localization framing, then make presented-module fraction equality/vanishing exact through cyclic annihilators; first release: an inverted element kills its presented module | ChatGPT continuation `2026-09-05-9b7b0ec7`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/modules/localizations.py`; `src/dzack_research/preamble/categories/modules/framed/finitely_generated/finitely_presented_modules.py`; generated-set write: `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `02989533188625af32091f31adb5d1671de511f5`; framing repair passes the blocked localized-ideal regression; cyclic-annihilator fraction equality now returns `True` for the generator of `QQ[x]/(x^2)` after inverting `x`. The combined category MRO dispatches `is_zero()` first to `_SelectedFinitePresentationModules.ParentMethods`, whose PID-only invariant-factor implementation masks `LocalizedModules.ParentMethods.is_zero`; claim expanded to make that generic method defer outside its represented PID regime. Source commit remains blocked by the whole-repo mypy baseline. Concurrent dirty `all.py`, `group_actions.py`, `group_modules.py`, and `pure/modules.py` remain outside this reservation and untouched | 2026-09-05T15:02:34Z |
...
| R | `8e4b7ae7`: represented quotient ideals compute exact module relations by lifting generators and defining equations to the polynomial cover; quotient Krull dimension and nested quotient computations use the same cover data. The `xy=0` origin is prime/maximal, its prime localization has the expected local units/maximal ideal/residue map, and finite/prime localization preserves algebra bases and validates external fraction denominators. The pinned R integration set passes 8/8. | R's denominator/unit/residue and presented-quotient substrate is released. Generic localized-ideal inclusion is blocked by M: `LocalizedModule` loses its transported framing during initialization. Resume R ideal extension/contraction after `M-localized-framing-20260905`. | Released checkpoint: `8e4b7ae7` |
| M | Against R's released `8e4b7ae7`, the recovered initialization-order repair preserves source framing and the blocked localized-ideal extension/contraction regression passes. Presented fraction equality now uses `Ann_R(d) ∩ S`: prime complements test whether a cyclic annihilator generator escapes the prime, while finitely generated `S=<s_i>` tests `prod(s_i) in radical(Ann_R(d))`, with a direct annihilator witness fallback when radical computation is unavailable. The generator of `QQ[x]/(x^2)` therefore vanishes after inverting `x`. The attempted source commit passed Sage preflight/syntax but remains frozen by the repository-wide mypy baseline. | Repair zero-test dispatch for selected finite presentations outside the PID invariant-factor regime so localized finite generators reach the exact localization criterion. Then rerun the localized-ideal regression and finite/prime localization probes. Do not regenerate the shared docs while the unclaimed module/action edits are changing. | `M-localized-framing-20260905` |
...
| A | `0e25eedb`: the owned group algebra `R[G]` with group inclusion, augmentation, centre and Maschke; `Groups().group_algebra(R)` on subgroup inclusions. `2ddbfcb4`: group modules are `Modules(R[G])`, refining `GObjects(G, Modules(R))`; `GroupModules` retired. `d73b5709`: G-set vocabulary on `FiniteSets()`/`FiniteGSets(G)`; lattices with an action are `Lattices(ZZ[G])`. `5ad9314c`: `Modules(S)(M, rho)` from a ring morphism `S -> End(M)`; `Hom_R(S, -)` as `Modules(R).coextension_of_scalars(f)` for `S` finitely framed over `R`, with `Res_f -| Hom_R(S, -)`; the scalar-change triple spelled on `Modules(R)`. `bd7a44e8`: induction/restriction/coinduction are scalar extension/restriction/coextension along `R[H] -> R[G]`, recognized on the map and realized on a transversal; `Modules(R[G])(M, rho)` restricts a ring morphism along `G -> R[G]`. | Trivial action, invariants and coinvariants as scalar change along the augmentation `R[G] -> R` (restriction along `R -> R[G]` of a group module is the forgetful functor, not yet a case of `RestrictionOfScalarsFunctor`); retire `functors/group_actions.py`. Coextension along `ZZ -> ZZ[x]` needs countable products of modules. Torsor membership and transporters; stabilizers as predicate subgroups; submonoids of `NN`/`ZZ`; the scheme specialization `GObjects(G, Schemes)` for Q. Known red: the two G-set adjunction tests in `tests/groups/test_group_adjunctions.py` (chosen preimage of the underlying-set functor) fail on `bd7a44e8` and before it. | Unclaimed |
