...
| --- | --- | --- | --- | --- | --- |
| G-affine-module-gluing-20260906 | G — on a finite distinguished affine cover, restrict finitely framed local modules to overlaps, accept represented transition isomorphisms, verify inverse and triple-cocycle conditions, and construct the glued module sheaf with its compatible local-section module | ChatGPT continuation `2026-09-06-g-affine-module-gluing-0437`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/schemes/ringed_spaces.py`; `src/dzack_research/preamble/categories/schemes/gluing.py`; `tests/schemes/test_module_sheaf_gluing.py`; `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `44dfd973`; consumes G principal-open restrictions `f02a69f8` and released M scalar-change/localization maps through `65fe44e6`. No M path is reserved: if exact base change of transition maps is missing, return that repair to M rather than duplicating it here. | 2026-09-05T21:08:04Z |
...
| D | `8298c13d`: for a represented localization `S^{-1}A`, `KahlerDifferentials(S^{-1}A)` is the represented module localization `S^{-1}Omega^1_{A/R}` with its source/unit metadata retained, and derivations extend to fractions by `d(a/s)=s^{-1}d(a)-a s^{-2}d(s)`. For `A=QQ[x,y]/(xy)` localized at `x`, the universal derivation has `d(y)=0`, `d(x^{-1})=-x^{-2}dx`, and the transported `Fitt_1` is the unit ideal. The new localization regression passes both in an isolated three-file patch over released R and in the live checkout. The full de Rham file still stops earlier at the pre-existing unsupported general internal-Hom kernel specimen. | Construct the conormal/tangent comparison and feed the differential/Fitting calculation into S's represented singular subscheme for the `xy=t` family. | Released: `8298c13d` |
...
| G | `f02a69f8`: finite distinguished affine covers `X = union D(f_i)` are represented when the `f_i` generate the unit ideal; repeated `D(f)` construction reuses one stable represented open and coordinate localization. The structure sheaf identifies `O_X(D(f))` with that localization and represents restrictions `A_f -> A_h` exactly when the target inverts the source denominator family, with overlaps `D(f_i f_j)` and both chart-to-overlap composites checked. For an `A`-module `M`, the associated affine module sheaf has sections `M_f` and restriction maps represented as `A_f`-linear maps into scalar restriction of `M_h` along `A_f -> A_h`; both cover charts agree with global restriction on the overlap. The G-specific/legacy structure-sheaf gate passes 4/4. The unrelated `xy=t` affine-pullback regression fails at current HEAD and identically in a read-only pre-G archive of `1f9173ad`; it is not introduced by this release. | Active next release: finite affine module descent — represented overlap restrictions, transition isomorphisms with inverse/triple-cocycle checks, and the actual module of compatible local sections. Scheme and algebra gluing follow once this descent contract is released. | Claimed: `G-affine-module-gluing-20260906` |
...
| Q | `28b23405`: `GObjects(G, Schemes(R))` equips a represented affine scheme with a fresh acted copy carrying represented coordinate pullbacks and computes common fixed subschemes. `dfdb460d`: for supported finite linear actions on polynomial affine space over a field, the scheme now computes Sage/Singular invariant generators, derives the defining kernel relations by elimination to obtain a chosen finite presentation of `A^G`, represents the inclusion `A^G -> A` and quotient `Spec(A) -> Spec(A^G)`, and factors invariant affine maps through that quotient with the defining triangle checked. The regressions cover the `C_2` coordinate swap, the central-sign involution with its nontrivial quadratic relation, a `C_3` linear action, rejection of a nonlinear action, and the zero-variable quotient including its scalar factorization; together with fixed-locus regressions they pass 11/11. | Global/glued quotient constructions now wait on G's restriction maps and compatible affine/module gluing; the supported affine categorical quotient itself has no remaining Q prerequisite. | Released: `dfdb460d` |
