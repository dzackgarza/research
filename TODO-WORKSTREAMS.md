...
| --- | --- | --- | --- | --- | --- |
| Q-affine-fixed-loci-20260906 | Q — specialize `GObjects(G, Schemes(R))` to represented affine schemes and compute common fixed subschemes from the fixed ideal generated by `g^*(a)-a` on chosen group/algebra generators | ChatGPT continuation `2026-09-06-affine-fixed-loci-takeover`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/group/g_objects.py`; `src/dzack_research/preamble/categories/schemes/schemes.py`; `tests/schemes/test_group_actions_fixed_loci.py`; `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `559b554e`; adopted the prior owner's in-flight source and regenerated-doc changes without alteration or reset. A released the self-algebra/augmentation substrate at `4150e30b`, and S released represented affine scheme maps at `dbef76cc`. Finish the first literal PORT §13.3 fixed-subscheme release before invariant-ring quotients. | 2026-09-05T19:09:59Z |
...
| Q | Requirements in PORT §13.3. | Active first release: represented affine `G`-scheme actions and common fixed ideals/subschemes. Invariant-algebra quotient maps remain the next Q release. | Claimed: `Q-affine-fixed-loci-20260906` |
