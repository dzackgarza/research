...
| P | `66466ec7`: relative multivariate presentations over a polynomial parameter algebra retain their nested owned presentation and scalar structure while computing in Sage's canonical flattened polynomial engine. The `QQ[t][x,y]/(xy-t)` structure map sends `t` to `xy`, `lift_to_presentation` returns the nested presentation, and base change along `t -> 0` gives `QQ[x,y]/(xy)`; the dedicated regression plus the two scalar-change adjunction specimens pass 3/3 on the committed substrate. | Successive quotients of a chosen presentation must preserve the selected scalar ring and quotient map; then verify the represented coproduct/pushout map path. | Released: `66466ec7` |
