...
| --- | --- | --- | --- | --- | --- |
| M-localized-framing-20260905 | M — preserve transported localization framing, then make presented-module fraction equality/vanishing exact through cyclic annihilators; first release: an inverted element kills its presented module | ChatGPT continuation `2026-09-05-9b7b0ec7`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/modules/localizations.py`; generated-set write: `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `d85c4d75f226f6482d1f31a4d856f259938a6b5d`; inherited framing repair now passes `test_ideal_localization_extension_contraction_colon_and_saturation`; exact probes give `Ann_{ZZ}(1 mod 6)=(6)` and `Ann_{QQ[x]}(1 mod x)=(x)`. Source commit remains blocked by the whole-repo mypy baseline (8046 errors in unchanged/imported files; none reported in `modules/localizations.py`). Concurrent dirty `all.py`, `group_actions.py`, `group_modules.py`, and `pure/modules.py` are outside this reservation and remain untouched | 2026-09-05T14:54:33Z |
...
| M | Against R's released `8e4b7ae7`, the recovered initialization-order repair preserves the source framing and the previously blocked localized-ideal extension/contraction regression passes. For a presented source element `d`, the represented cyclic submodule `Rd` computes `Ann_R(d)` exactly in the active one-generator/PID regimes; probes recover `(6)` for `1 in ZZ/6` and `(x)` for `1 in QQ[x]/(x)`, giving the standard localization witness `Ann_R(d) ∩ S != empty`. The attempted source commit passed Sage preflight/syntax but is frozen by the repository-wide mypy baseline (8046 errors in unchanged/imported files, none in `modules/localizations.py`). | Use the cyclic-annihilator witness in localized fraction equality, including prime-complement and finitely generated multiplicative-set cases, then make `is_zero()` test a finite presented generating family rather than enumerate only finite modules. Do not regenerate the shared docs while the newly appeared unclaimed module/action edits are changing. | `M-localized-framing-20260905` |
