...
| --- | --- | --- | --- | --- | --- |
| M-localized-framing-20260905 | M — preserve the transported framing on `LocalizedModule`, unblocking localized ideal inclusions after R's denominator/unit release | ChatGPT continuation `2026-09-05-ffafca2a`; `/home/dzack/research` | write: `src/dzack_research/preamble/categories/modules/localizations.py`; generated-set write: `docs/preamble-megadoc.md`; `docs/preamble-graph.json`; `docs/preamble-graph.dot`; `docs/preamble-graph.html` | `8e4b7ae7fd2ff47924cd709b6eab90be6271908b`; reproduced by `test_ideal_localization_extension_contraction_colon_and_saturation`: `LocalizedModules.ParentMethods.__init__` records the source framing, then `FramedModules.ParentMethods.__init__` overwrites it with `None`, so the localized ideal inclusion receives no generator-label set | 2026-09-05T14:08:02Z |
...
| M | R's released `8e4b7ae7` denominator/unit contract exposes a concrete framing defect: `LocalizedModules.ParentMethods.__init__` derives the source generator set/function, but the later `FramedModules.ParentMethods.__init__` call receives no framing and overwrites both with `None`; localized ideal inclusion therefore has no generator labels. | Preserve the derived framing through initialization, then rerun localized ideal extension/contraction and the existing finite/prime module-localization regressions before proceeding to presentation-based localized equality/vanishing. | `M-localized-framing-20260905` |
