...
Let $\mathcal C$ be a pointed $\infty$-category, with zero object $*$, and suppose
that it has the finite limits and colimits required below.

## Fibers and cofibers {#sec-cobase-changes}

For $f\colon X\to Y$, its fiber over the canonical basepoint $0\colon *\to Y$ is
...
$$
For $\mathcal C=\mathcal S_*$, the $\infty$-category of pointed spaces, this
recovers the classical adjunction
$$
