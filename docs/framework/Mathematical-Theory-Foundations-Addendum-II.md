...
The missing theory is not a new collection of named categories.
It is a small collection of standard higher-categorical constructions:
...
In particular, this addendum does **not** identify the source of a construction, its essential image, and the fibers of its presentation map.
It also does not promote every essential image to a new primitive in the categorical presentation.
Each is obtained functorially from the original functor and is named only when later mathematics uses it.

The ambient setting is a universe-stratified \((\infty,2)\)-category
\[
\mathfrak{Cat}
\]
of infinity-categories, infinity-functors, natural transformations, and higher modifications.
A model may be supplied by an infinity-cosmos and its homotopy \(2\)-category, scaled simplicial sets, complete Segal \(2\)-spaces, or another equivalent presentation.
The constructions below use only mapping infinity-categories, arrow and comma infinity-categories, cores, homotopy fibers, adjunctions, and localizations.
...
This addendum does not choose a model of spectra or derived affine schemes.
Those remain parameters of the larger theory.
The general operadic statements apply to spectral or derived ambients after such an ambient symmetric monoidal infinity-category has been fixed.
...
\mathbb{Adj}
\]
denote a chosen **walking adjunction** in the selected model of \((\infty,2)\)-categories.
It is generated by two objects \(0,1\), two \(1\)-morphisms
\[
f:0\longrightarrow1,
\qquad
u:1\longrightarrow0,
...
Thus the unit and counit are not separate categorical primitives.
They are evaluations of the universal cells of the walking adjunction.
...
\]
be endpoint evaluation, and let \(\tau\) exchange the two factors.
Define the infinity-groupoid of oppositely directed pairs of arrows by
\[
...
A point of this fiber supplies the unit, counit, and all triangle coherences.
The assertion that \(F\) is left adjoint to \(U\) is the propositional truncation of the assertion that this fiber is inhabited.
...
Its points consist of a right adjoint \(U\), unit, counit, and coherent triangle data.
Define \(\operatorname{LAdj}(U)\) dually.
...
Consequently, the existence of a right or left adjoint is property-like.
A chosen adjunction remains data: its unit and counit are available for subsequent constructions and recognition theorems.

### Remark 1.6 (Fixed pair versus varying adjoint)

The theorem concerns the full space of right-adjoint data for a fixed left adjoint, allowing the right adjoint and coherent identifications to vary.
It should not be replaced by an assertion that a particular strict record of unit and counit fields is literally unique.
The invariant statement is contractibility in the appropriate infinity-groupoid.

### Convention 1.7 (Adjunction assertions) {#conv-adjunction-assertions}

A bare relation saying that two functors are “adjoint” is insufficient when the unit or counit is used later.
The mathematical datum is a lift to
\[
\operatorname{Adj}(\mathfrak{Cat}).
\]

Existence may be recorded by propositional truncation.
Any construction involving a unit, counit, monad, comonad, localization map, or recognition morphism uses a named chosen lift.
...
\]
be a functor.
The **essential image** \(\operatorname{EssIm}_{\mathcal D}(F)\) is the full replete infinity-subcategory of \(\mathcal D\) spanned by objects equivalent to \(F(X)\) for some \(X\in\mathcal C\).
...
The essential image is a standard construction applied to \(F\). It is not an additional primitive attached independently to the source and codomain.
It may be given a short name when it is repeatedly used, but its defining data remain the displayed factorization.
...
This is the standard comma construction on the cores.
The notation \(\operatorname{Pres}(F)\) is only an abbreviation for the displayed homotopy pullback.
...
The equivalence of Proposition 3.3 does not identify these roles.
In particular, it does not identify \(\mathcal C\) with the full essential image in \(\mathcal D\).
...
\]
be fully faithful.
Then \(J\) induces an equivalence from
\[
\operatorname{EssIm}_{\mathcal D}(F)
\]
...
This is the general reason that categories of free modules, based free modules, and basis-preserving free modules must be distinguished.
The same issue occurs for monoid algebras in bialgebras with arbitrary morphisms versus bialgebras equipped with distinguished bases and basis-preserving morphisms.
...
\]
be fully faithful and replete.
Let \(F:\mathcal C\to\mathcal D\). To identify \(\mathcal E\) with \(\operatorname{EssIm}_{\mathcal D}(F)\) over \(\mathcal D\), it is necessary and sufficient to supply:
...
\]
is not well typed as an identification of categories over \(\mathcal D\). One may instead state that \(F\) factors through \(\mathcal E\), that the corestricted functor is essentially surjective, or that \(\mathcal E\) is a category of chosen presentations.
...
For an arbitrary free--forgetful adjunction, the left adjoint need not be fully faithful.
The counit therefore does not generally recognize free objects.
Recognition by the counit is a theorem obtained only after proving full faithfulness of the left adjoint.
...
An object \(X\in\mathcal C\) is **local** if \(\eta_X\) is an equivalence.
The local objects form precisely the essential image of \(i\).
...
c:=jR
\]
is the associated **colocalization** or **coreflection** endofunctor.
The counit
\[
...
An object \(X\in\mathcal C\) is **colocal** if \(\varepsilon_X\) is an equivalence.
The colocal objects form precisely the essential image of \(j\).
...
\]
have finite support.
Then \(x\) is group-like if and only if
\[
a_m^2=a_m,
\qquad
a_ma_n=0\quad(m\ne n),
...
\]
is coreflective.
A bialgebra \(B\) belongs to this essential image if and only if the counit
\[
...
If \(R\) has nontrivial idempotents, the unit need not be an isomorphism and the functor need not be fully faithful.
The essential image remains well defined, and a chosen presentation of \(B\) remains a pair
\[
(M,\phi),
\qquad
...
Corollary 7.4 concerns the essential image in the category of bialgebras and arbitrary bialgebra morphisms.
If one instead works with bialgebras equipped with distinguished bases and restricts morphisms to preserve those bases, one has changed the codomain category.
Full faithfulness and equivalence with the source must then be proved for that new functor.
They do not follow from an objectwise description of the standard basis.
...
Let \((\mathcal C,J)\) be a site and let \(\mathcal S\) denote the infinity-category of spaces.
The full replete inclusion
\[
i_J:
...
Thus sheaves form a reflective localization of presheaves.
...
\]
is an equivalence.
Sheafification exhibits the infinity-category of sheaves as the localization of presheaves at the class of \(J\)-local equivalences.
...
Let prestacks be space-valued presheaves on a site and stacks be the sheaves for the chosen topology.
The inclusion of stacks into prestacks is fully faithful and admits a left adjoint
\[
a_J^{\mathrm{st}}:
...
Stackification is the same reflective-localization construction as sheafification, applied to space-valued presheaves.
Stacks in groupoids are obtained by the appropriate truncation.
A prestack is already a stack precisely when the unit to its stackification is an equivalence.

### Remark 9.2 (No separate foundational mechanism)

Sheafification and stackification are named instances of reflective localization.
Their units, counits, local objects, and local equivalences are obtained by evaluating the corresponding chosen adjunctions.
They should not be represented by unrelated axioms or by a bare claim that one category is contained in another.
...
Thus the free abelian sheaf on a presheaf of sets is obtained by taking the objectwise free abelian presheaf and then sheafifying.
This is Lemma 1.2 on composition of adjunctions.

### Remark 10.2 (Change of site and mates)

For a functor of underlying sites, precomposition is defined at the presheaf level.
Whether it preserves sheaves is a theorem.
If it does not, the sheaf-valued functor is obtained by composing with sheafification.
Comparisons between this composite and any corestricted inverse-image functor are mates of the presheaf-level square; equivalence is a Beck--Chevalley assertion under the appropriate continuity and cocontinuity hypotheses.
...
Under standard hypotheses, \(U_{\mathcal O}\) is monadic.
This identifies
\[
\operatorname{Alg}_{\mathcal O}(\mathcal V)
\]
...
In familiar one-colored settings, the underlying object of a free \(\mathcal O\)-algebra may be computed from the operations of \(\mathcal O\), tensor powers of the input, and homotopy coinvariants for symmetric groups.
Such formulae require the precise operad, enrichment, and colimit hypotheses.
...
\]
be a strong symmetric monoidal functor between admissible operadic ambients.
It induces
\[
\Phi_{\mathcal O}:
\operatorname{Alg}_{\mathcal O}(\mathcal V)
...
This comparison is an equivalence when \(\Phi\) preserves the colimits and operadic tensor-power constructions entering the free-algebra functor.
It is not an equivalence merely because \(\Phi\) is written with the same symbol on underlying objects.
...
\]
be a reflective localization of presentable symmetric monoidal infinity-categories.
It is **compatible with the tensor product** if, for every morphism \(f\) inverted by \(L\) and every \(X\in\mathcal V\), the morphism
\[
...
Let \(L\dashv i\) be a tensor-compatible reflective localization as in Definition 12.3, and let \(\mathcal O\) be an infinity-operad for which the relevant algebra categories and free-algebra functors exist.
...
Its essential image is the full replete subcategory of \(\mathcal O\)-algebras whose underlying objects are \(L\)-local, provided the tensor-compatibility hypotheses ensure that the operadic operations remain local.
The unit recognizes this essential image.
These hypotheses are part of the theorem and cannot be replaced by a bare assertion that localization preserves algebra structures.
...
The source and essential image are compared by \(F_{\mathrm{ess}}\). The fibers of \(p_F\) are chosen-presentation spaces.
No equality or equivalence among these is asserted without the relevant full-faithfulness or comparison theorem.

### Convention 13.2 (Morphism conventions are part of image statements) {#conv-morphism-convention-image}

Every essential-image statement names its ambient codomain and its morphisms.
A theorem for arbitrary module, algebra, bialgebra, sheaf, or operadic-algebra morphisms does not automatically transfer to a category with basis-preserving, frame-preserving, strict, strong, lax, or other restricted morphisms.
...
The statements about free operadic algebras and monoidal localizations apply in spectra or derived categories after the ambient presentable symmetric monoidal infinity-category has been fixed.
This addendum does not select a model of spectra, derived commutative rings, derived affine schemes, or their topology.

The choice between simplicial commutative rings, connective \(E_\infty\)-rings, or other models remains a parameter of the derived-geometric theory.
It is not a missing component of the adjunction and essential-image calculus.

### Remark 15.2 (SageCat)

The mathematical normalization of Sage categories is not altered here.
The present addendum supplies standard adjunction, image, presentation, and localization constructions that a Sage correspondence may reference.
The definition and limit/colimit closure properties of \(\mathbf{SageCat}\) remain separate questions.
...
Sheafification and stackification instantiate the second recognition pattern.
The monoid-algebra adjunction over a connected base instantiates the first.
Free operadic algebras fit the same adjunction object, essential-image factorization, and presentation-fiber calculus.
