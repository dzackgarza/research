...
- *H5 — the discriminant lives in a comparison of two sequences.* $A_L$ is presented as $L^{\#}/L \cong \operatorname{coker}\tilde b$ inside the comparison (@def-metric-dual and @thm-double-complex), never as a bare cokernel formula; defining it by formula before the sequence that gives it meaning is a convention violation, not a nit.
