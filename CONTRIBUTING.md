...
- **Rule**: Put an operation on the appropriate owned `ObjectType`, `ElementType`, Hom-category/arrow type, or functor type of the most general owned category on which the operation is actually defined.
  ...
  **Placement test.** The placement of an operation introduced on `C` is correct exactly when the operation is not well-defined on any supercategory of `C`.  The correct home is the supremum of the categories on which it is well-defined.  When that supremum is not a category in the tree, the finding is a missing category, axiom or property with its own mathematical meaning, or a defect in the declared graph; it is never a reason to leave the operation where it is.  The test applies equally to object, element and arrow operations; an arrow operation is judged on the Mor categories of the supercategories.
