...
- **Rule**: An operation that yields several values yields **one element of the product of a family over an index set**.  Before writing a tuple, ask which product it is a point of *and over which index set*, then construct it through `CartesianProductOfFamily(index_set, family)`: a pair of naturals is the constant family \(\mathbb N\) over a chosen two-element set; a tensor's shape is the constant family \(\mathbb N\) over that tensor's own index set; the components of a commutative square are the family of its two homsets.  A bare Python tuple never appears in a public signature, a return, or an annotation.

  `Product(A, B)` and `CartesianProductOfSets(...)` are sugar over that constructor -- they pick `Sets.Δ[n-1]` for you -- so `CON-14` governs them exactly as it governs `FreeModule(R, n)`: the arity is a chosen set, and a construction that says only the arity has not named its index set.  Reach for the sugar only where the chosen set is genuinely arbitrary, and never where the index set is the mathematics, as it is for a shape.
