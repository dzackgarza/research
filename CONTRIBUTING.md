...
- **Rationale**: A variable belongs to the formula that presents one map, not to the set of maps. Storing one on the space turns a representation choice for formulas into defining data, lets two spaces on one \(X\to Y\) differ by the name of a variable, and makes `space.indeterminate()` an element with no referent, from which incoherent statements (`2^x` as a statement about the space) then follow. Element construction outside \(\operatorname{Mor}(X, Y)\) also duplicates the one boundary where maps enter.
  ...
  Three further patterns fail for the same reason: each places a datum on an object that cannot own it.
  (1) Calling the stored variable the identity map of \(X\) does not repair the design. It turns the bound variable of a formula into an element of the space, so `2^x` reads as an operation on that element. The map \(t\mapsto 2^t\) is given by its own definition, not obtained by operating on an element of the space.
  (2) A space cannot supply the coordinate of its domain. A coordinate is a function on \(X\), and which coordinates exist depends on \(X\) alone: one real coordinate, a vector \((x_0, x_1)\), local coordinates on a chart. A space that answers `coordinate()` either fixes a one-dimensional real domain in advance or answers for an object it does not own. Two coordinates on one \(X\) present the same map in two ways, and the map does not change.
  (3) A one-argument constructor such as `C(Infinity, RR)` fixes the codomain silently. \(C^\infty(\mathbb R, \mathbb R)\), \(C^\infty(\mathbb R, \mathbb C)\) and \(C^\infty(\mathbb R, \mathbb R^n)\) are different objects, and the functoriality in \(B\), post-composition with \(B\to B'\), cannot be stated when \(B\) is not an argument.

- **Violation Example**: `Lp(p)` built with `indeterminate=SR.var("x")` and `ell(p)` with `SR.var("n")`, storing the variable and handing it out as `indeterminate()`, with `space(expr)` reading an expression in it as the map; a test writing `x = space.indeterminate()` and then `space(exp(-x^2))`; `x = C(Infinity, RR).coordinate()` followed by `x**2 + 1` as an element of the space.
