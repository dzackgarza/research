...
#### `CON-17`: The Hom Set Owns the Construction of Its Elements; Every Object Owns Its Own Coordinate

- **Rule**: A space of maps \(A\to B\) -- continuous, smooth, integrable, square-summable, bounded -- is a subobject of \(X = \operatorname{Mor}_{\mathbf C}(A, B)\), cut out by its condition, with the structure its category adds. Its defining data are that inclusion and that structure. Each such space is a family of functors in both arguments: \(C^n(A, B)\), \(L^p(A, B)\), \(W^{k,p}(A, B)\), graded by \(n\), \(p\) or \((k, p)\), and possibly regarded as sheaves on \(A\). So every constructor takes \(A\) and \(B\): `C(Infinity, RR, RR)`, `Lp(2, RR, RR)`, `ell(2, NN, RR)`.

  How an element is given belongs to the element constructor of \(X\), which is the one boundary. It accepts a callable, a lambda, a formula (`f(t) = integral(x^2, x, 0, t)`), and in finitary cases a table, a matrix or a tensor. A subobject constructs its element there and admits it by its condition. A map is given by how it acts on an arbitrary element: \(t\mapsto 2^t\) says what the map does to an arbitrary \(t\in A\).

  An object's coordinate is a symbolic general element of that object, or a system of local coordinates on it, and each object owns its own. A coordinate of \(A\) stands for an arbitrary point of \(A\), and a map \(A\to B\) is built from it. \(\mathbb R\) supplies one coordinate, \(\mathbb R^2\) supplies \(x = (x_0, x_1)\), and a manifold supplies local coordinates on a chart. In one dimension a formula may equally bind a variable of its own (`f(t) = t^2`). A coordinate of \(X = \operatorname{Mor}(A, B)\) is something else: a general element of \(X\), an arbitrary map \(f\). It is used to build maps out of \(X\). An operator is an element of \(\operatorname{Mor}_{\mathbf D}(X, X)\) and is given by its action on the general \(f\): `f = X.coordinate()`, then `H(lambda f: exp(f))` for \(H = \operatorname{Mor}_{\mathbf D}(X, X)\). A function space may also have coordinates in the usual sense, as an infinite-dimensional vector or a finite-dimensional manifold. The two coordinates are never interchangeable. A coordinate of \(A\) is never obtained from \(X\), and a coordinate of \(X\) never stands for a point of \(A\).

- **Rationale**: A point of \(A\) and an element of \(\operatorname{Mor}(A, B)\) are different mathematical objects. A coordinate stands for an arbitrary element of the object that supplies it. Asking \(X\) for its coordinate and then using it as a point of \(A\) substitutes an arbitrary function for an arbitrary number. After that, expressions that mean nothing are accepted. `2^x` with \(x = \operatorname{id}_{\mathbb R}\) has no mathematical meaning and is not what anyone intends. `x**2 + 1` evaluated at \(2\) has no meaning once \(x\) is a general function. Storing a variable on the space as its "indeterminate" is the same conflation built into the object's data: the variable is meant as a point of \(A\), and it is attached to \(X\). Only \(A\) knows its own points and coordinates, so only \(A\) can supply them: one real number, a vector \((x_0, x_1)\), a chart. A one-argument constructor such as `C(Infinity, RR)` fixes the codomain without stating it. \(C^\infty(\mathbb R, \mathbb R)\), \(C^\infty(\mathbb R, \mathbb C)\) and \(C^\infty(\mathbb R, \mathbb R^n)\) are different objects, and the functoriality in \(B\), post-composition with \(B\to B'\), cannot be stated when \(B\) is not an argument. Element construction outside \(\operatorname{Mor}(A, B)\) also duplicates the one boundary where maps enter.

- **Violation Example**: `Lp(p)` built with `indeterminate=SR.var("x")` and `ell(p)` with `SR.var("n")`, storing a variable meant as a point of the domain and handing it out as `indeterminate()`, with `space(expr)` reading an expression in it as the map; a test writing `x = space.indeterminate()` and then `space(exp(-x^2))`; `x = C(Infinity, RR).coordinate()` followed by `x**2 + 1` and `f.evaluate_at(RR(2)) == RR(5)`, which uses the space's general element as a point of \(\mathbb R\).

- **Correct Example**: \(L^2(\mathbb R, \mathbb R)\) as the subobject of \(\operatorname{Mor}_{\mathbf{Set}}(\mathbb R,\mathbb R)\) of square-integrable maps, with its quotient by null functions where the classes are meant; `x = RR.coordinate()` and `C(Infinity, RR, RR)(x^2 + 1)`; `f(t) = exp(-t^2)` binding `t` in its own definition, constructed in `Sets().Mor(RR, RR)` and admitted by the space; a local parameter at a point of a scheme obtained from the scheme and used to build the map; `f = X.coordinate()` for \(X = C^\infty(\mathbb R, \mathbb R)\) and the operator `X.Mor(X)(lambda f: exp(f))`.
...
- **Rule**: Before writing a class for "the `X`s that are `P`", decide whether `P` is a property of the objects or a chosen datum.  A property (finitely generated, torsion, free, Noetherian, separated, symmetric, countable) is a nested `class P(CategoryWithAxiom)` on the base that first defines it, with a `SubcategoryMethods` accessor and, for a new name, one `all_axioms` registration; there is no standalone class.  A chosen datum (a chosen multiplication, a differential, an augmentation, an enumeration, an order, a Koszul parity) is a class, and it declares the axiom category of the property it truncates to.  A chosen generating epimorphism, presentation or syzygy tower is a truncated resolution and lives in the category of resolutions over the base (`CAT-29`), whose image in the base is the relative finiteness axiom.  The crossing from property to datum is one named method that computes the datum once (AGENTS.md, *Property subcategories vs data subcategories*).  A property with more than one established name in the literature takes the reference text's name; choosing between Bourbaki's "alternating" and Sage's "strictly commutative" is not coining, and a property class kept "because no axiom was coined" is this rule violated, not respected.
...
- **Correct Example**: `OwnedRings.Noetherian`, `Schemes.Separated`, `Modules.FinitelyPresented` as nested axiom classes; a chosen finite presentation of `M` as a point of the truncation-1 resolutions over `Modules(R)`, whose image is `Modules(R).FinitelyPresented()` (`CAT-29`); the parity as part of the grading datum on `GradedModules`, read by the `Supercommutative` axiom.
...
#### `CAT-29`: Framings, Presentations and Resolutions Are Objects of a Category of Resolutions; Frameability Is a Relative Property

- **Rule**: A chosen generating epimorphism, a chosen presentation and a chosen syzygy tower are truncations of one datum, a *resolution*, and they live in a category of resolutions over `C`, never in an axiom subcategory of `C`.  Fix a projective class \(\mathcal P\) in `C` -- usually the retracts of free objects \(G(X)\) for the free/forgetful comonad \(G = FU\).  The category \(\operatorname{Res}_{\mathcal P,n}(C)\to C\) has as fibre over \(X\) the augmented simplicial objects \(P_\bullet\to X\), truncated at degree \(n\), levelwise in \(\mathcal P\), acyclic through degree \(n\) (for modules, Dold--Kan returns the augmented chain complexes of `lean-categories` `FOUNDATIONS.md` §76--77).  \(n = \infty\) is a cofibrant replacement.  Levelwise conditions (free, finitely generated, finite rank) are the pullbacks of property classifiers along degree evaluation; global conditions (acyclicity, length) are conditions on the whole diagram.  Two indices are kept apart: the *truncation* \(n\) says how far the resolution is chosen, and the *length* says above which degree it vanishes.  A generating epimorphism \(F(S)\twoheadrightarrow X\) is truncation 0; a presentation is truncation 1; `FP_n` is the existence of a resolution finite through truncation \(n\).  A length-0 resolution by free objects, \(0\to F(S)\xrightarrow{\cong}X\to0\), is a basis: that, and only that, is a *framing*.  Frameability conditions -- free, stably free, finitely generated, finitely presented, `FP_n`, finite length -- are relative properties of `C` and are axioms with qualified names (`CAT-19`).  Every object is resolvable: the counit \(F(UX)\twoheadrightarrow X\) begins the canonical comonadic (bar) resolution \(G^{\bullet+1}X\to X\), so "has a resolution" is never an axiom.  *Framed* in the geometric sense is the manifold notion: a framed manifold is a manifold with a length-0 resolution of its tangent bundle by trivial bundles, i.e. a trivialization of \(TX\) (the \(G=\{e\}\) G-structure); a stable framing trivializes \(TX\oplus\mathbb R^k\), a framing in the stabilized category.

- **Rationale**: the property "some framing exists" is true of every object once the counit is admitted, so an axiom named for it separates nothing; and a chosen framing is data whose forgetful functor to `C` is not injective on objects (many resolutions sit over one \(X\)), so it is not a subcategory (`CON-16`, chosen enrichment).  Sage's joins close under axioms, so a global `Framed` axiom was propagated across branches as though it were a property of every branch: the meet of the categories of `ZZ` and `QQ` came out with `Framed` on `Modules(ZZ)` and `QQ` was refused, while `Framed` membership was decided by a stored datum.  The resolution picture also recovers the geometric notion: through Serre--Swan a trivialization of \(TM\) is a basis of \(\operatorname{Der}(C^\infty(M))\), the length-0 case above, and for an algebra the resolution computes the cotangent complex whose trivialization is the algebraic framing.  Sources: nLab *framed manifold*, *G-structure*; Quillen, *Homotopical Algebra*, II.4 (simplicial objects over a category with enough projectives); Barr--Beck, *Homology and standard constructions* (comonadic resolutions); Christensen--Hovey, *Quillen model structures for relative homological algebra* (projective classes); Lurie, *Higher Topos Theory* §5.5.8 (the nonabelian derived category \(\mathcal P_\Sigma\)).  Section numbers are to be checked against the texts before a docstring cites them.

- **Violation Example**: a global `Framed` axiom registered in `all_axioms` and declared on objects, groups, modules and algebras (`objects.py`, `FramedModules(R)`, `FramedAlgebras(R)`); membership in `C.Framed()` decided by a framing stored on the object; `is_framed_algebra()` reading an attribute; a chosen-presentation class declaring `Framed` as its supercategory.

- **Correct Example**: \(\operatorname{Res}_{\mathcal P,1}(\mathrm{Mod}_R)\) holding the chosen presentation of \(M\), its image in `Modules(R).FinitelyPresented()` the property; `FramedFreeModules(R)`, a free module with a chosen basis, as the length-0 resolutions; `QQ` a `ZZ`-module whose canonical resolution begins with \(F(U\mathbb Q)\twoheadrightarrow\mathbb Q\) and which is not finitely generated; a framed smooth manifold as a manifold with a trivialization of its tangent bundle.
...
- **Rule**: Distinguish a property from a chosen witness of that property.
  A property category such as finitely generated or finitely presented states existence; a chosen generating morphism or presentation is a truncated resolution in the category of resolutions over `C` (`CAT-29`), which owns the operations that consume that choice.
...
- **Correct Example**: `FinitelyPresentedGroups()` records the property; `GroupsWithChosenFinitePresentation()` carries and exposes the selected presentation.  A truncation-0 resolution of `M` carries the chosen epimorphism from a free module, while `Modules(R).FinitelyGenerated()` only states that some finite one exists.
...
- **Rule**: Put an operation on the appropriate owned `ObjectType`, `ElementType`, Hom-category/arrow type, or functor type of the most general owned category on which the operation is actually defined.
  ...
  **Placement test.** The placement of an operation introduced on `C` is correct exactly when the operation is not well-defined on any supercategory of `C`.  State it as a poset problem.  Let `P` be the declared category poset and `U_C = {D in P | D > C}` the up-set of `C`.  For each operation `f` introduced on `C`, the set `W_f = {D in U_C | f is well-defined on D}` is closed downward within `U_C`, and the home of `f` is `max W_f`, or `C` itself when `W_f` is empty.  When `W_f` has several maximal elements, or its maximum is not a category in `P`, the finding is a missing category, axiom or property with its own mathematical meaning, or a defect in the declared graph; it is never a reason to leave the operation where it is.  The test applies equally to object, element and arrow operations; an arrow operation is judged on the Mor categories of the supercategories.
