...
def classical_lattice(name: str) -> FreeQuadraticModule_integer_symmetric:
    ...
    if name == "U":
        ...
        gram_matrix = Matrix(
            ZZ,
            [[4, 3, 2, 3, 1, 1, 1], [3, 4, 3, 3, 1, 1, 1], [2, 3, 4, 3, 2, 2, 1], [3, 3, 3, 4, 2, 1, 2], [1, 1, 2, 2, 2, 1, 1], [1, 1, 2, 1, 1, 2, 0], [1, 1, 1, 2, 1, 0, 2]],
        )
        ...
        gram_matrix = Matrix(ZZ, [[2, 1, 1, 0, 1, 1], [1, 4, 1, 1, 1, 3], [1, 1, 2, 1, 1, 1], [0, 1, 1, 2, 1, 2], [1, 1, 1, 1, 2, 2], [1, 3, 1, 2, 2, 4]])
...
def scaled_lattice(lattice: FreeQuadraticModule_integer_symmetric, scale_factor: int) -> FreeQuadraticModule_integer_symmetric:
    """
...
def build_all_test_case_lattices() -> dict[tuple[str, ...], _TestCaseResult]:
    ...
    for spec_list, k_dim in TEST_CASES:
        ...
        try:
            ...
            # Store results
            results[tuple(spec_list)] = {"lattice": lattice, "verification": verification, "k_dimension": k_dim}

        except Exception as e:
            print(f"  ERROR: {e}")
            results[tuple(spec_list)] = {"error": str(e), "k_dimension": k_dim}
