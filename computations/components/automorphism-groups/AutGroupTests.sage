...
test_cases = [
    ...
    },
    {
        ...
        "citation": "Aut(C_8) is isomorphic to C_2 x C_2. [Wikipedia]",
    },
    {
        "name": "Quaternion Group Q8",
        "group": QuaternionGroup(),
        "expected_aut": SymmetricGroup(4),
        "citation": "Aut(Q_8) is isomorphic to S_4. [Wikipedia]",
    },
    {
        ...
        "expected_aut": S(3),
        "citation": "Aut(C_2 x C_2) is isomorphic to S_3. [Groupprops]",
    },
    {
        "name": "Finitely Presented Group (S3)",
        "group": SymmetricGroup(3).as_finitely_presented_group(),
        "expected_aut": S(3),
        "citation": "Aut(S_3) is isomorphic to S_3. [Standard result]",
    },
    {
        "name": "Matrix Group GL(2,2)",
        "group": GL(2, 2),
        "expected_aut": S(3),
        "citation": "GL(2,2) is isomorphic to S_3, so Aut(GL(2,2)) is S_3. [Wikipedia]",
    },
    {
        "name": "Abelian Group C2 x C3",
        "group": AbelianGroup([2, 3]),
        "expected_aut": C(2),
        "citation": "Aut(C_2 x C_3) is isomorphic to C_2. [Standard result]",
    },
    {
...
cyclic_test_cases = [
    {
        ...
        "group": C(n),
        "expected_aut": Gm(n),
        "citation": f"Aut(C_{n}) is isomorphic to (Z/{n}Z)^* [Standard result]",
    }
...
symmetric_test_cases = [
    {
        ...
        "group": S(n),
        "expected_aut": S(n),
        "citation": f"Aut(S_{n}) is isomorphic to S_{n} for n >= 3, n != 6. [Wikipedia]",
    }
...
dihedral_test_cases = [
    {
        ...
        "citation": f"Aut(D_{n}) is isomorphic to Hol(C_{n}). [Standard result]",
    }
...
def run_all_automorphism_tests():
    ...
    for test_case in test_cases + cyclic_test_cases + symmetric_test_cases + dihedral_test_cases:
        ...
        if expected_aut is not None:
            assert computed_aut.is_isomorphic(expected_aut), (
                print_fail(f"FAIL: Aut({name}) not isomorphic to expected group") or False
            )
            print_pass(f"PASS: Aut({name}) is isomorphic to {expected_aut}")
        elif expected_order is not None:
            assert computed_aut.order() == expected_order, (
                print_fail(
                    f"FAIL: |Aut({name})|: Expected {expected_order}, got {computed_aut.order()}"
                ) or False
            )
...
def run_random_automorphism_tests(num_tests=20):
    ...
    for _ in range(num_tests):
        ...
        assert computed_aut.is_isomorphic(Gm(n)), (
            print_fail(f"FAIL: Aut(C_{n}) not isomorphic to (Z/{n}Z)*") or False
        )
        assert computed_aut.order() == euler_phi(n), (
            print_fail(f"FAIL: |Aut(C_{n})|: Expected {euler_phi(n)}, got {computed_aut.order()}")
            or False
        )
        print_pass(f"PASS: Aut(C_{n}) is isomorphic to (Z/{n}Z)*")
    print()
...
def run_direct_product_tests(num_tests=5):
    ...
    for _ in range(num_tests):
        ...
        group = AbelianGroup([n, m])
        computed_aut = aut(group)
        if gcd(n, m) == 1:
            expected_order = euler_phi(n * m)
            assert computed_aut.order() == expected_order, (
                print_fail(
                    f"FAIL: |Aut(C_{n} x C_{m})|: Expected {expected_order}, got {computed_aut.order()}"
                ) or False
            )
