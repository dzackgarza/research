...
### Nine atoms, Lo10, Milgram's modulus, and AF10's local model, in `lean/Atoms.lean`
...
Normality is written too, and it did not want a sheaf of rings after all: the
germs at a point are a `Subring` quotiented by an `Ideal`, both elementary, and
the sheaf was only ever one way of organising them.  What remains of AF10 is the
**dimension stratification** $V_{(d)}$ with $V_0$ dense of full dimension, which
needs a dimension function on an analytic germ — the one place in this graph
where the next step is neither a definition over existing primitives nor a
proof, but a theory.
