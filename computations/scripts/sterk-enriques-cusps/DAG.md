...
| HS17 | the **almost-complex structure** as a smooth section: $J$ a section of $\mathrm{End}(TM)$ with $J\circ J = -\mathrm{id}$ | decomposed out of HS1, whose $(1,1)$-tensor field had no definition under it | HS1; Mathlib `ContMDiffSection` (`Geometry/Manifold/VectorBundle/ContMDiffSection.lean`), the hom-bundle of `.../VectorBundle/Hom.lean`, `TangentSpace`, `IsManifold` over `𝓘(ℂ, E)` |
...
| AF10 | **Theorem 9.2, the analyticity criterion**: for $V$ a locally compact second-countable space, a locally finite disjoint union of irreducible normal analytic spaces $V_i$, the sheaf of $\mathcal{Q}$-functions makes $V$ an analytic space provided (i) each $V_{(d)}$ is closed with $V_0$ dense of full dimension, (ii) each point has a fundamental system of neighbourhoods meeting $V_0$ connectedly, (iii) the $\mathcal{Q}$-functions restrict to the structure sheaf of each $V_i$, and (iv) they separate points locally | Baily–Borel 9.1, 9.2 | **AF16** for the space, **AF18** for normality and **AF19** for the stratification and condition (i) — the three clauses split out of this row — with Mathlib `Topology/Sheaves/`, `SecondCountableTopology` and `LocallyCompactSpace` for the criterion's own hypotheses.  What is left in AF10 itself is Baily–Borel's $\mathcal{Q}$-functions and the criterion's proof.  `BochaoKong/nullstellensatz`'s `LocalComplexGeometry` (local biholomorphisms, Weierstrass preparation) is the nearest read development and is the local theory |
...
| Node | Statement | Source | Next dependency |
| --- | --- | --- | --- |
| Pa1 | $\mathrm{qu}(\mathbb{Z}_p)$, $\mathrm{qu}(\mathbb{Z})$, $\mathrm{bil}(\mathbb{Z})$: the semigroups of quadratic and bilinear forms in which the relations below are stated | Nikulin §1, 8° | **Pa12**, the semigroup on a fixed family of representatives, split out of this row; over Mathlib `PadicInt` and `Padics`, with the operation `QuadraticMap.prod` with `Isometry.inl`/`inr` (`QuadraticForm/Prod.lean`) modulo `QuadraticMap.IsometryEquiv` — **not** the `AddCommMonoid (QuadraticMap R M N)` instance, which adds two forms on one module pointwise |
...
| Node | Statement | Source | Next dependency |
| --- | --- | --- | --- |
| V1 | $C^+$-matrices: nondecomposable positive matrices of order $n$, each the Gram matrix of a unique simplicial angle in $E^n$, with all entries of the inverse positive | 1975 §2 | **V14** for indecomposability, split out of this row; over Mathlib `Matrix`, `Matrix.PosDef` and `PosSemidef` (`LinearAlgebra/Matrix/PosDef.lean`), `Matrix.det`, `Matrix.nonsing_inv` (`Matrix/NonsingularInverse.lean`) for the condition on the inverse's entries, `Matrix.rank` |
...
| AF14–AF19 | `AnalyticOnNhd` and its `add`/`neg`/`mul`/`mono`, `IsOpen`, `Homeomorph`, `Subring`, `Ideal.Quotient`, `IsIntegrallyClosed`, `ringKrullDim`, `IsClosed`, `closure`, `IsIrreducible` | the analytic subset, the structure sheaf by extension, the chart, the germ ring, normality, and the stratification — **all six are written** in `lean/Atoms.lean`.  The chain needs no ideal sheaf and no sheafification, which is what the reduced formulation buys |
| V14 | `Matrix.blockDiagonal`, `Equiv`, `Sum` | indecomposability, written |
| Pa12 | `PadicInt`, `QuadraticMap.prod`, `IsometryEquiv`, `Setoid`, `finSumFinEquiv`, `LinearEquiv.sumArrowLequivProdArrow` | the semigroup on a fixed family of representatives, written |
| HS17 | `ContMDiffSection`, the hom-bundle of `.../VectorBundle/Hom.lean`, `TangentSpace` | $J$ as a smooth section squaring to $-\mathrm{id}$ |
...
**There are four leaves: AF14, Lo1, E14, E15**, and each names Mathlib
declarations and nothing else — `AnalyticOnNhd`; `QuadraticForm` with
`QuadraticForm/Signature.lean`; `IsCoveringMap` with `IsEvenlyCovered`; `Finsupp`
with `Order.height` and `Scheme.functionField`.  Every one was read in the pinned
source.  The audit below is what the leaf set looked like when it had seven
members and their cells were coarser; the clause-by-clause reading is what turned
eleven of its clauses into the nodes AF14–AF19, V14, Pa12, HS17, Lo10, E16 and
the F-stratum additions, which is why the four that remain rest on Mathlib
directly.
...
question: a local `.lake` build can be partial, and whether a given module has an
`olean` on some machine is a fact about that machine and **not about this graph**
— it never decides what a definition should say.  Where the two came apart the
mathematics won: normality is `IsIntegrallyClosed`, which the pinned tree has,
rather than an unfolding chosen to route around a module that happened to be
uncompiled locally.
...
| AF10, **complete**: local model, germ ring, normality, stratification | `Sterk.IsAnalyticSubset` — the common zero locus in an open $U\subseteq\mathbb{C}^n$ of finitely many functions `AnalyticOnNhd`; with `IsAnalyticSubset.subset` and `isAnalyticSubset_self`.  `HolomorphicAtOnSubset` gives the structure sheaf **concretely**, as the functions that agree near a point with one analytic on an ambient neighbourhood, and `holomorphicAtOnSubset_of_analyticOnNhd` is its first lemma.  `AnalyticChart` is a homeomorphism of an open set of $X$ onto such a subset, and `IsLocallyAnalyticSpace` says the charts cover.  **This is the step that looked like a programme and was not**: describing the structure sheaf by extension rather than as a quotient of the ambient sheaf by an ideal sheaf keeps the whole definition inside what the pinned Mathlib supplies, and no sheafification is needed.  Irreducibility is Mathlib's `IsIrreducible`.  The local ring follows without sheafification: `holomorphicSubring` is a **`Subring`** of all functions — sums and products of functions that extend do extend, on the intersection of the two neighbourhoods, by `AnalyticOnNhd.add`, `.neg`, `.mul` and `.mono` — `vanishingIdeal` is the `Ideal` of functions vanishing near the point, `germRing` is the quotient, and and `IsNormalAtPoint` is `IsIntegrallyClosed` of that ring.  The stratification closes it: `germDim` is `ringKrullDim` of the germ ring, `dimStratum` and `dimStratumExact` are Baily–Borel's $V_{(d)}$, with `dimStratum_subset` and `dimStratum_mono`, and `StratificationAdmissible` states condition (i).  **That condition is a hypothesis of Theorem 9.2 and not a conclusion**, so what AF10 needed from this graph was the ability to *state* it, which needed the dimension function and the strata and now has both |
| F1.16's Gauss sum, and **the modulus half of Milgram** | `Sterk.ratCircleToRealCircle : AddCircle (2 : ℚ) →+ AddCircle ((2 : ℚ) : ℝ)`, because a discriminant form is valued in `ℚ/2ℤ` while the circle character is stated for a real period; **`Sterk.discriminantGaussSum`**, the sum over `AddCircle.toCircle_addChar`; `discriminantGaussSum_zero`, that the zero form's sum is the group's order; and `MilgramStatement`, the theorem written out as a proposition about the data.  Mathlib's `gaussSum` is for a `MulChar`/`AddChar` pair on a finite ring, which a discriminant form is not, so this object existed nowhere.  Four steps of the proof are now proved on top of it: `norm_gaussTerm` (each term has modulus one), `discriminantGaussSum_mul_conj` (the sum times its conjugate expands over pairs — the identity Milgram's proof starts from), `gaussTerm_add` with `gaussTerm_zero` (the terms are multiplicative in the value, so a difference of values contributes one term), and `conj_gaussTerm` (conjugation inverts a term, proved through `Complex.mul_conj` and the modulus).  Four more carry it further: `gaussTerm_sub`, `sum_gaussTerm_reindex` (the substitution $b = a+c$, by `Fintype.sum_equiv (Equiv.addLeft a)`), `discriminantGaussSum_mul_conj_eq_sum_diff` (so the squared modulus is a double sum over a point and a **difference**), and `sum_gaussTerm_diff_factor` — under the polar identity, which is what F1.15's doubling makes sayable and which the theorem takes as a **hypothesis** rather than assuming about $q$, the $a$-sum factors into a term in $c$ alone times a character sum in $a$.  `polChar` packages `fun a => -(\mathrm{pol}\ a\ c)` as an `AddChar A ℂ` from two hypotheses a discriminant form satisfies, `sum_polChar` evaluates it by `AddChar.sum_eq_ite`, and **`discriminantGaussSum_mul_conj_of_nondegenerate` proves $|G(q)|^2 = |A|$** — the modulus half of Milgram's theorem, complete, with nondegeneracy as the hypothesis that the polar character is nontrivial away from zero |
...
**AF10's local model is written, and the sheaf quotient turned out to be
avoidable.**  A *reduced* analytic space is locally a zero locus whose structure
sheaf is the functions that extend holomorphically, and that description needs
neither an ideal sheaf nor sheafification — only `AnalyticOnNhd`, which exists.
AF10 is complete.  Normality did not want a sheaf of rings — the germs at a
point are a `Subring` quotiented by an `Ideal` — and the stratification did not
want a theory of analytic dimension, because the dimension of a germ is the Krull
dimension of that ring and `ringKrullDim` is built.  Condition (i) is a
hypothesis of Baily–Borel's theorem, so stating it was the whole requirement.

**Nothing in the graph now rests on an object that does not exist.**  Of the
eleven clauses the leaf audit found to be content, all eleven are written and
compile.  One statement was *relocated* rather than proved: Milgram's claim about
the **argument** of $G(q)$ is about a lattice's signature and not about $(A,q)$,
so it belongs with F3.2's bridge and is recorded there, not as a gap in the
Gauss-sum layer.  What is left in this graph is what it was always for: proving
the nodes.
...
**Milgram's modulus half is proved**: $|G(q)|^2 = |A|$ for a nondegenerate form.
The other half says the *argument* of $G(q)$ is $\exp(2\pi i\sigma/8)$, and that
one cannot be proved from $A$ and $q$ alone — $\sigma$ is the signature of a
**lattice**, so the statement needs F3.2's bridge between the lattice and its
discriminant form.  That is a fact about the statement rather than a gap in the
proof: the Gauss-sum layer is as complete as the data it is stated over allows,
and the remaining step belongs with F3.2.
