...
Four steps of the proof are proved on it.  What is unproved is the rest: the
substitution that turns the double sum into a sum over the difference, the polar
identity that separates it, the evaluation by `AddChar.sum_eq_ite`, and then the
reduction to the $p$-adic pieces.  The route is now written down in the file
step by step, which is what makes the remainder a finite list rather than a
name.
