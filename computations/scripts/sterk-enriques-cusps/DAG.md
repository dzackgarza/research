...
forms* (`73KFL5DK`), §3.  Two corrections to how C1 and C2 were recorded fall
out immediately: the diagram classification is **Coxeter's**, not Vinberg's, and
**the algorithm is in the 1972 paper, not the 1975 one** — the 1975 paper
contains no occurrence of *algorithm*.
...
| V10 | Coxeter's classification: the nondecomposable $C^+$ and $C^0$ matrices with $a_{ij} = -\cos(\pi/m_{ij})$, by diagram, with rank subscripts | 1975 §4, **attributed to Coxeter [1]** | V1, V2 |
...
as previously recorded bundled V8, V10 and V11 into one row across two papers
and an attribution to the wrong author, which is the `FSC-06` defect twice over.
