...
What remains of AF10 is **normality**, which is a condition on the local ring at
a point and so does want the structure sheaf as a sheaf of *rings* rather than
the pointwise predicate; and the **dimension stratification** $V_{(d)}$ above it.
Those two are the residue, and they are smaller than the programme this row
described before the model was written.
