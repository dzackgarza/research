...
| Wave 1 overall | **Incomplete** — #225-blocked: Hom-category UP, Actions category, Cat/Sets substrate. Pre-#225: owned affine presentations for general DM moduli / remaining `Mbar_*` (beyond proving-set open `M_{0,3}` / `M_{0,4}` / `M_{0,5}` / `M_{1,1}` / `M_{1,2}` and proper `Mbar_{0,3}` / `Mbar_{0,4}` / `Mbar_{0,5}` / `Mbar_{1,1}` / `Mbar_{1,2}` when owned) |
