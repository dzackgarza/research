...
| Wave 1 overall | **Incomplete** — #225-blocked: Hom-category UP, Actions category, Cat/Sets substrate. Pre-#225: owned affine presentations for general DM moduli / remaining `Mbar_*` (beyond proving-set open `M_{0,3}` / `M_{0,4}` / `M_{0,5}` / `M_{1,1}` and proper `Mbar_{0,3}` / `Mbar_{0,4}` / `Mbar_{1,1}` when owned) |
...
7. Atlas/pullback/Aut/fiber-product: structural deepenings landed (quotient presentation, product étale atlas, `FiberProductStack`, mediating UP, Sage `Action`, proving-set étale certificates + finite étale groupoid flags, owned `M_{0,3}`/`M_{0,4}`/`M_{0,5}`/`M_{1,1}` open charts + `Mbar_{0,3}`/`Mbar_{0,4}`/`Mbar_{1,1}` proving-set covers including Hesse `Γ(3)` when `2` not invertible and `3` invertible, `etale_atlas_gap()` for named fail-closed alts); equation-level étaleness on arbitrary/general moduli atlases / `Mbar_{0,5}` / `M_{1,2}`, full 2-cat Hom UP, and #225 Actions wiring remain (named gaps, not wins).
