...
| `atlas()` / `etale_atlas()` | **Deepened, still incomplete** — single owned-presentation registry (`moduli/atlas_ownership.py`: `owned_etale_atlas_presentations()` / `dispatch_etale_atlas` / `etale_atlas_gap_from_registry`). Every `ModuliStack.etale_atlas()` goes through that dispatch: owned row → equation-level domain, else fail-closed `AtlasChart` + structured `etale_atlas_gap()`. Default cardinality **10** (parametric open Knudsen `M_{0,n}` for all `n≥3` via `_knudsen_open_M0n_affine_scheme`; parametric open Legendre/Hesse `M_{1,n}` for all `n≥1` via `_legendre_open_M1n_affine_scheme` / `_hesse_open_M1n_affine_scheme`; plus proper `Mbar_{0,3}`–`Mbar_{0,5}` and compact `(1,1)`/`(1,2)`). Expand `expand_open_m0n_through=8` + `expand_open_m1n_through=4` → **21** concrete rows / **15** type keys. Fail-closed: `(1,*)` when neither `2` nor `3` is a unit (Weierstrass `𝔾_m` evidence for `n≤2`); unowned types (e.g. `M_{2,0}`, `Mbar_{0,n}` for `n>5`, `Mbar_{1,n}` for `n>2`) get `reason=no_owned_affine_etale_presentation` — **no invented Kapranov / fake compact charts**. Remaining pre-#225 atlas debt is **general `(g,n)` beyond open Knudsen / open level genus-1**. Wave 2 still waits on [#225](https://github.com/dzackgarza/research/pull/225) merge. |
