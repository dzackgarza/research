...
| `atlas()` / `etale_atlas()` | **Deepened, still incomplete** — single owned-presentation registry (`moduli/atlas_ownership.py`: `owned_etale_atlas_presentations()` / `dispatch_etale_atlas` / `etale_atlas_gap_from_registry`). Every `ModuliStack.etale_atlas()` goes through that dispatch: owned row → equation-level domain, else fail-closed `AtlasChart` + structured `etale_atlas_gap()`. Default cardinality **6** (parametric open Knudsen `M_{0,n}` for all `n≥3`; parametric open Legendre/Hesse `M_{1,n}` for all `n≥1`; parametric compact Legendre/Hesse `Mbar_{1,n}` for all `n≥1` via `_legendre_compact_M1n_covering_space` / `_hesse_compact_M1n_covering_space` (`ℙ¹` for `n=1`); plus parametric proper Kapranov `Mbar_{0,n}` for `3≤n≤8` via `kapranov_iterated_blowup_P_{n-3}`, specializing to point / `ℙ¹` / `Bl₄(ℙ²)` / `Bl(ℙ³)` / lazy `Bl(ℙ⁴)` / lazy `Bl(ℙ⁵)`). Expand `expand_open_m0n_through=8` + `expand_open_m1n_through=4` + `expand_compact_m1n_through=4` + `expand_proper_m0n_through=8` → **28** concrete rows / **20** type keys. Fail-closed: `(1,*)` when neither `2` nor `3` is a unit (Weierstrass `𝔾_m` evidence for `n≤2`); genus-2 with named gap `igusa_binary_sextic_PGL2` (`reason=genus_2_igusa_binary_sextic_unavailable`); `Mbar_{0,n}` for `n>8` with named gap `kapranov_iterated_blowup_P_{n-3}` — **no invented charts**. Remaining pre-#225 atlas debt is **general `(g,n)` beyond open Knudsen / level genus-1 / proper genus-0 through `n=8` / named genus-2**. Wave 2 still waits on [#225](https://github.com/dzackgarza/research/pull/225) merge. |
