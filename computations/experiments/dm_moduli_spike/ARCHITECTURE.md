...
| `atlas()` / `etale_atlas()` | **Deepened, still incomplete** — single owned-presentation registry (`moduli/atlas_ownership.py`: `owned_etale_atlas_presentations()` / `dispatch_etale_atlas` / `etale_atlas_gap_from_registry`). Every `ModuliStack.etale_atlas()` goes through that dispatch: owned row → equation-level domain, else fail-closed `AtlasChart` + structured `etale_atlas_gap()`. Default cardinality **9** (parametric open Knudsen `M_{0,n}` for all `n≥3`; parametric open Legendre/Hesse `M_{1,n}` for all `n≥1`; parametric compact Legendre/Hesse `Mbar_{1,n}` for all `n≥1` via `_legendre_compact_M1n_covering_space` / `_hesse_compact_M1n_covering_space` (`ℙ¹` for `n=1`); plus proper `Mbar_{0,3}`–`Mbar_{0,6}` via Kapranov `Bl₄(ℙ²)` / `Bl(ℙ³)`). Expand `expand_open_m0n_through=8` + `expand_open_m1n_through=4` + `expand_compact_m1n_through=4` → **26** concrete rows / **18** type keys. Fail-closed: `(1,*)` when neither `2` nor `3` is a unit (Weierstrass `𝔾_m` evidence for `n≤2`); unowned types (e.g. `M_{2,0}`, `Mbar_{0,n}` for `n>6`) get `reason=no_owned_affine_etale_presentation` with named gap `kapranov_iterated_blowup_P_{n-3}` — **no invented Kapranov charts**. Remaining pre-#225 atlas debt is **general `(g,n)` beyond open Knudsen / level genus-1 / proper genus-0 through `n=6`**. Wave 2 still waits on [#225](https://github.com/dzackgarza/research/pull/225) merge. |
