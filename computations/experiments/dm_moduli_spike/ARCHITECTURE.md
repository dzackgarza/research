...
| `atlas()` / `etale_atlas()` | **Deepened, still incomplete** — single owned-presentation registry (`moduli/atlas_ownership.py`: `owned_etale_atlas_presentations()` / `dispatch_etale_atlas` / `etale_atlas_gap_from_registry`). Every `ModuliStack.etale_atlas()` goes through that dispatch: owned row → equation-level domain, else fail-closed `AtlasChart` + structured `etale_atlas_gap()`. Default cardinality **12** (one **parametric** open Knudsen `M_{0,n}` for all `n≥3` via `_knudsen_open_M0n_affine_scheme`, plus proper `Mbar_{0,3}`–`Mbar_{0,5}` and `(1,1)`/`(1,2)` Legendre/Hesse); `expand_open_m0n_through=8` lists **17** concrete rows / **13** type keys. Fail-closed: `(1,*)` when neither `2` nor `3` is a unit owns Weierstrass `𝔾_m` as **owned_not_finite_etale** gap evidence; unowned types (e.g. `M_{2,0}`, `Mbar_{0,n}` for `n>5`) get `reason=no_owned_affine_etale_presentation` — **no invented Kapranov charts**. Remaining pre-#225 atlas debt is **general `(g,n)` beyond open genus-0 Knudsen**. Wave 2 still waits on [#225](https://github.com/dzackgarza/research/pull/225) merge. |
