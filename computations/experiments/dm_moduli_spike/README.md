...
| `atlas()` / `etale_atlas()` as `U → X` | **Deepened, incomplete** — owned-presentation registry: **9** rows by default (parametric open Knudsen `M_{0,n}` for all `n≥3` + parametric open Legendre/Hesse `M_{1,n}` for all `n≥1` + parametric compact Legendre/Hesse `Mbar_{1,n}` for all `n≥1` + proper genus-0 `Mbar_{0,3}`–`Mbar_{0,6}`); `expand_open_m0n_through=8` + `expand_open_m1n_through=4` + `expand_compact_m1n_through=4` yields **26** inspectable rows / **18** type keys. Open Knudsen / open+compact `M_{1,n}` / `Mbar_{1,n}` / proper `Mbar_{0,n}` (`n≤6`) equation-level True under hypotheses; `M_{2,0}` / `Mbar_{0,n}` (`n>6`, named gap `kapranov_iterated_blowup_P_{n-3}`) fail-closed. `(1,*)` without units `2`/`3`: Weierstrass `𝔾_m` fail-closed evidence (`n≤2`). Remaining pre-#225 atlas debt: general `(g,n)` beyond open Knudsen / level genus-1 / proper genus-0 through `n=6`. Wave 1 incomplete; Wave 2 waits on #225 merge. |
