...
| `atlas()` / `etale_atlas()` as `U → X` | **Deepened, incomplete** — owned-presentation registry: **10** rows by default (parametric open Knudsen `M_{0,n}` for all `n≥3` + parametric open Legendre/Hesse `M_{1,n}` for all `n≥1` + proper genus-0 + compact `(1,1)`/`(1,2)`); `expand_open_m0n_through=8` + `expand_open_m1n_through=4` yields **21** inspectable rows / **15** type keys. Open Knudsen / open `M_{1,n}` equation-level True under hypotheses; `M_{2,0}` / `Mbar_{0,n}` (`n>5`) / `Mbar_{1,n}` (`n>2`) fail-closed. `(1,*)` without units `2`/`3`: Weierstrass `𝔾_m` fail-closed evidence (`n≤2`). Remaining pre-#225 atlas debt: general `(g,n)` beyond open Knudsen / open level genus-1. Wave 1 incomplete; Wave 2 waits on #225 merge. |
