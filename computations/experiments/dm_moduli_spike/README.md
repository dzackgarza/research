...
| `atlas()` / `etale_atlas()` as `U → X` | **Deepened, incomplete** — fail-closed `has_equation_level_etale_certificate()`; positive on affine covering + finite `G`, proving-set open `M_{0,3}` / `M_{0,4}` / `M_{0,5}` (Knudsen config) / `M_{1,1}` (Legendre when `2` invertible; Hesse `Γ(3)` / `SL₂(𝔽₃)` when `2` not invertible and `3` invertible, including char 2), and proper `Mbar_{0,3}` / `Mbar_{0,4}` (`ℙ¹` affine cover) / `Mbar_{1,1}` (same level-structure hypotheses). Fail-closed with `etale_atlas_gap()`: `(1,1)` when neither Legendre nor Hesse applies, `Mbar_{0,5}`, `M_{1,2}`, general `(g,n)`, products with a formal factor. Still open: affine presentations for general DM moduli / remaining compactifications |
...
| Wave 1 complete (incl. full coherence) | **FAIL** — structural deepenings landed; #225-blocked: Hom-category UP, Actions category, Cat/Sets substrate; pre-#225: equation-level étaleness for general `(g,n)` / char-2 `(1,1)` / `Mbar_{0,5}` / `M_{1,2}` (owned for proving-set open `M_{0,3}` / `M_{0,4}` / `M_{0,5}` / `M_{1,1}` and proper `Mbar_{0,3}` / `Mbar_{0,4}` / `Mbar_{1,1}` + quotient proving set) |
