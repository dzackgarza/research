...
| `atlas()` / `etale_atlas()` as `U → X` | **Deepened, incomplete** — owned-presentation registry: **11** rows by default (parametric open Knudsen `M_{0,n}` for all `n≥3` + parametric open Legendre/Hesse `M_{1,n}` for all `n≥1` + parametric compact Legendre/Hesse `Mbar_{1,n}` for all `n≥1` + parametric proper Kapranov `Mbar_{0,n}` for `3≤n≤8` + open Igusa `M_{2,0}` + compact Igusa `Mbar_2` via Kapranov `Mbar_{0,6}/S₆` + parametric open marked Rosenhain `M_{2,n}` for `n≥1` + parametric compact marked Kapranov-fiber-product Rosenhain `Mbar_{2,n}` for `n≥1`, all Igusa under `2∈Rˣ` + open del Pezzo `M_{3,0}` via 7-points-in-`ℙ²` / `W(E₇)` under `2∈Rˣ`, non-hyperelliptic dense open); `expand_open_m0n_through=8` + `expand_open_m1n_through=4` + `expand_compact_m1n_through=4` + `expand_proper_m0n_through=8` + `expand_open_m2n_through=4` + `expand_compact_m2n_through=4` yields **39** inspectable rows / **31** type keys. Open Knudsen / open+compact `M_{1,n}` / `Mbar_{1,n}` / proper `Mbar_{0,n}` (`n≤8`, lazy sample certs for `n≥7`) / open `M_{2,n}` (Igusa Rosenhain / `S₆`) / proper unmarked `Mbar_2` (Kapranov `Mbar_{0,6}` / `S₆`) / proper marked `Mbar_{2,n}` (Kapranov fiber-product Rosenhain / `S₆`, lazy sample) equation-level True under hypotheses; Igusa types without `2` unit: `igusa_requires_two_invertible`; `Mbar_{0,n}` (`n>8`, named gap `kapranov_iterated_blowup_P_{n-3}`) fail-closed. `(1,*)` without units `2`/`3`: Weierstrass `𝔾_m` fail-closed evidence (`n≤2`). Remaining pre-#225 atlas debt: proper/marked genus 3, `g≥4`, hyperelliptic binary-octic locus of `M_3`, and char-2 Igusa/del Pezzo. Wave 1 incomplete; Wave 2 waits on #225 merge. |
