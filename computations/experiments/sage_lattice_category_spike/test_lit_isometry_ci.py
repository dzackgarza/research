...
and Groups*, 3rd ed. (1999) [Zotero T2WVLTDB | CS10 | attachment TCJKXU3D], and is
reconstructed here in CS10's own g = g_0 g_1 factorization (g_0 = |Weyl group|,
...
def test_root_lattice_weyl_group_orders_rank_ge_7_match_conway_sloane_chapter_4():
    ...
    The expected orders are the CS10 g_0 g_1 products; the spike computes O(L)
    through Sage's orthogonal_group (PARI/GAP), so a green line is the *published*
    order reproduced by an independent engine.
    """
    # (name, CS10 g = g_0 * g_1, determinant, is_unimodular)
    cases = [
    ...
    # pin the raw CS10 numbers too, so the factored forms above cannot drift silently
