...
class SyntheticIsometryGroup(IsometryGroup, Parent):
    ...
    def _delegated_engine(self) -> "SageGenus":
    ...
    def subgroup(self, gens: "RawVectors") -> SyntheticIsometrySubgroup:
...
class SyntheticIsometrySubgroup(IsometrySubgroup):
    ...
    def __init__(self, ambient: SyntheticIsometryGroup, gens: "RawVectors") -> None:
    ...
    def preserves(self, subobject: "Subobject") -> bool:
        ...
        from .homsets import Subobject

        assert isinstance(subobject, Subobject), f"preserves takes a subobject (M.subobject(...)/an image/kernel), not a bare lattice; found={type(subobject)}"
