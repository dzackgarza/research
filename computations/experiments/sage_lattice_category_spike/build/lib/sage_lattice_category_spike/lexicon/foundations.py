...
type OrderedSet = TotallyOrderedFiniteSet | Sequence
"""A set carrying a distinguished linear order.

``TotallyOrderedFiniteSet`` is what the repo *produces* -- ``finite_ordered_set``
returns one, and every framing set, generating family and name family is one.
``Sequence`` is admitted only as the input boundary, where a caller writes a
literal tuple that the constructor transports into the order.

The retired alias was ``Sequence`` alone, whose own docstring conceded that
uniqueness of members "is the producing constructor\'s contract, not the
type\'s" -- an admission that it named a list, not a set."""
