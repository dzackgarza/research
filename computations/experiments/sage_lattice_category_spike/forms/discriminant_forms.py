...
if TYPE_CHECKING:
    ...
    def typed_cached_method[**P, R](_function: Callable[P, R]) -> Callable[P, R]: ...
else:
...
class SyntheticDiscriminantForm(FiniteAbelianGroup, DiscriminantElementParent):
    ...
    def invariant_factor_gens(self) -> tuple[Any, ...]:
    ...
    def base_extend(self, base_ring: Any) -> Any:
        ...
        When all n_i become units in S (e.g. ``S = Frac(R)``), every
        summand collapses and the result is the zero form.
        """
        R = self.base_ring()
        ...
        if all(base_ring(ZZ(n)).is_unit() for n in self.invariants()) or not self.invariants():
            # All torsion killed by localization → zero module (each subclass
            # supplies its own zero form -- polymorphism, not isinstance).
    ...
    def gens_vector(self, element: Any) -> tuple[Any, ...]:
        return self.coordinates(element)
    ...
    def gens_to_invariant_factor_gens(self) -> Any:
        transform = self.invariant_factor_gens_to_gens().inverse()
        transform.set_immutable()
        return transform
    ...
    def contains_subgroup(self, subgroup: Any) -> bool:
        return isinstance(subgroup, SyntheticDiscriminantSubgroup) and subgroup.ambient() is self
    ...
    def p_torsion(self, p: Any, k: Any = 1) -> Any:
        return finite_p_torsion(self, p, k)

    def relations_among(self, gens: Any) -> Any:
        return finite_relations_among(self, gens)

    def basis_from_generators(self, generators: Any) -> tuple[Any, ...]:
        return finite_basis_from_generators(self, generators)
...
class SyntheticLatticeQuotient(SyntheticDiscriminantForm):
    ...
    def __hash__(self) -> int:
        from .discriminant import lattice_key

        return hash(
            (
                lattice_key(self.cover_lattice()),
                lattice_key(self.relation_lattice()),
                self.invariants(),
            )
        )
...
class SyntheticBilinearDiscriminantForm(BilinearDiscriminantForm, SyntheticDiscriminantForm):
    ...
    def __init__(self, gram_matrix: Any, category: Any = None, invariants: Any = None) -> None:
        ...
        if invariants is not None:
            ...
            assert all(invariants[i] * raw_gram[i, j] in ZZ for i in range(raw_gram.nrows()) for j in range(raw_gram.ncols())), (
    ...
    def b(self, left: Any, right: Any) -> Any:
        ...
        return rational_mod(
            vector(QQ, left.coefficient_vector()) * self.gram_matrix_bilinear() * vector(QQ, right.coefficient_vector()),
            1,
        )
    ...
    def orthogonal_submodule_to(self, subgroup_or_gens: Any) -> Any:
        ...
        if len(cover_basis) == self.ngens() and all(sage_form(cover_basis[i]).order() == invariant for i, invariant in enumerate(self.invariants())):
            ...
            assert len(sage_generators) == self.ngens() and all(sage_generators[i].order() == invariant for i, invariant in enumerate(self.invariants())), (
    ...
    def is_maximal_isotropic(self, subgroup_or_gens: Any) -> bool:
        ...
        if not self.is_isotropic_subgroup(subgroup):
            return False
        return all(not (subgroup._key() < candidate._key()) for candidate in self.isotropic_subgroups())
    ...
    def _bilinear_normal_presentation(self) -> Any:
        ...
        presented = TorsionQuadraticModule(
            sage_form.V(),
            sage_form.W(),
            modulus=sage_form._modulus,
            modulus_qf=sage_form._modulus,
        )
    ...
    def _delegated_orthogonal_group(self, kind: str) -> Any:
        ...
        if kind == "bilinear":
            ...
            sage_form = TorsionQuadraticModule(
                sage_form.V(),
                sage_form.W(),
                modulus=sage_form._modulus,
                modulus_qf=sage_form._modulus,
...
class SyntheticQuadraticDiscriminantForm(QuadraticDiscriminantForm, SyntheticBilinearDiscriminantForm):
    ...
    def brown_invariant_per_block(self) -> tuple[Any, ...]:
        ...
        from ..sage_patches import brown_indecomposable

        assert self._quadratic_modulus() == 2, (
            f"the Brown invariant is defined for quadratic values in QQ/2ZZ only; quadratic modulus={self._quadratic_modulus()}, invariants={self.invariants()}"
        )
...
class SyntheticSourcedDiscriminantForm(SourcedDiscriminantForm, SyntheticQuadraticDiscriminantForm):
    ...
    def __eq__(self, other: object) -> bool:
        return (
            ...
            and lattice_key(self.source_lattice()) == lattice_key(other.source_lattice())
    ...
    def gram_matrix_quadratic(self) -> Any:
        ...
        form = bilinear_form + matrix.diagonal(
            QQ,
            [rational_mod(raw_form[i, i], self._quadratic_modulus()) - bilinear_form[i, i] for i in range(raw_form.nrows())],
        )
        form.set_immutable()
        return form
    ...
    def preimage_lattice(self, subgroup_or_gens: Any, label: str = "preimage_lattice") -> Any:
        r"""The lattice ``pi^-1(H)`` in the source rational hull."""
        subgroup = self._subgroup(subgroup_or_gens)
