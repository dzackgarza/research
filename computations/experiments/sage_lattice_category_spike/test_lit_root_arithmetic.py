...
``http://127.0.0.1:23119`` on 2026-07-04):
  ...
  - Conway & Sloane, *Sphere Packings, Lattices and Groups* (3rd ed.).
    Zotero item key ``T2WVLTDB`` (the stable anchor cited below and by the rest
    of the suite as ``CS99``). The library item is the 2010 softcover reprint, so
    its Better-BibTeX key is ``CS10``; the extraction ``TCJKXU3D`` /
    ``CS99_extracted.md`` is the 1999 3rd edition the group refers to as CS99.
    Line numbers below index that extraction.
...
# (name, rank, det, is_unimodular, [N(0), N(2), N(4), N(6)], CS99 theta-series location)
...
_THETA = [
    # A_2 (hexagonal): Theta_{A_2} = 1 + 6 q^2 + 6 q^6      [CS99 sec 6.2, line 4744]
    ("A2", 2, 3, False, [1, 6, 0, 6]),
    # E_6: 1 + 72 q^2 + 270 q^4 + 720 q^6                   [CS99 sec 8.3, line 5196]
    ("E6", 6, 3, False, [1, 72, 270, 720]),
    # E_7: 1 + 126 q^2 + 756 q^4 + 2072 q^6                 [CS99 sec 8.2, line 5131]
    ("E7", 7, 2, False, [1, 126, 756, 2072]),
    # E_8: 1 + 240 q^2 + 2160 q^4 + ...,  N_m = 240 sigma_3(m/2)
    #   => N(6) = 240 * sigma_3(3) = 240 * 28 = 6720        [CS99 sec 8.1, line 5049]
...
def test_theta_series_low_shells_match_conway_sloane(name, rank, det, unimodular, shells):
    r"""[CS99 / Zotero T2WVLTDB] SPLAG Ch. 4 prints the theta series of A_2
    ...
    # significant content: the CS99 theta-series shell counts
    N0, N2, N4, N6 = shells
...
# (name, rank, det, dual_det, CS99 dual-section location)
_DUAL = [
    # A_n^* : det = 1/(n+1)                                 [CS99 sec 6.6, line 4845]
    ("A2", 2, 3, QQ(1) / 3),
    ("A4", 4, 5, QQ(1) / 5),
    # D_n^* : det = 1/4                                     [CS99 sec 7.4, line 4998]
    ("D5", 5, 4, QQ(1) / 4),
    ("D6", 6, 4, QQ(1) / 4),
    # E_6^* : det = 1/3                                     [CS99 sec 8.3, line 5219]
    ("E6", 6, 3, QQ(1) / 3),
    # E_7^* : det = 1/2                                     [CS99 sec 8.2, line 5148]
    ("E7", 7, 2, QQ(1) / 2),
    # E_8^* = E_8 : det = 1                                 [CS99 sec 8.1, line 5034]
    ("E8", 8, 1, QQ(1)),
]
...
def test_dual_lattice_determinants_match_conway_sloane(name, rank, det, dual_det):
    r"""[CS99 / Zotero T2WVLTDB] SPLAG Ch. 4 tabulates each dual lattice: A_n^*
...
# (name, rank, det, glue invariant factors, root count, CS99 location)
_LLL = [
    ("E6", 6, 3, (3,), 72),    # [CS99 sec 8.3, lines 5185/5191/5196]
    ("E7", 7, 2, (2,), 126),   # [CS99 sec 8.2, lines 5122/5128/5131]
    ("E8", 8, 1, (), 240),     # [CS99 sec 8.1, lines 5034/5049]
    ("D6", 6, 4, (2, 2), 60),  # [CS99 sec 7.1, lines 4910/4919]
]
...
def test_lll_reduction_preserves_conway_sloane_invariants(name, rank, det, glue, tau):
    r"""[CS99 / Zotero T2WVLTDB] LLL / reduced_basis change the generating basis
    ...
    roots. This pins L.LLL() invariants back to the CS99 values and asserts they
    ...
    # significant content: LLL preserves the CS99 invariants
