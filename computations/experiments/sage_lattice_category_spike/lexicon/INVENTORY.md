...
Category vocabulary: `CategoryObject`, `CategoryMorphism`, `Functor`. `Functor` means a source category, target category, object action on `CategoryObject`s, and morphism action on `CategoryMorphism`s; object-only or morphism-only data is not a functor.
Morphisms, homs, and lattice functors: `LatticeMorphism`, `LatticeHomset`, `LatticeSimilarity`, `LatticeBaseChangeFunctor`, `Subobject` (the pair `(L, ι: L ↪ M)` — runtime authority `morphisms/homsets.py`, referenced by the contracts here), `IsometryHomset` (`Isom(L, M)` as a first-class parent: existence/classification questions are homset questions; `is_isometric` is its emptiness router), `EmbeddingHomset` (`Emb(L, M)` likewise — the home of the Nikulin-class machinery).
`LatticeBaseChangeFunctor` is the scalar-extension functor `Lattices(R) → Lattices(S)` induced by a canonical map `R → S`; it rebuilds each lattice object through the target category root and acts on lattice morphisms through target homsets by the same coefficient formula on the base-changed generators.
