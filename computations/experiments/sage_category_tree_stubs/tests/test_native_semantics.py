...
def test_every_sage_owned_dot_vertex_is_native_mapped() -> None:
    ...
    assert not undocumented, f"sage-owned DOT vertices with no native map (not in research-only catalog): {undocumented}"
    mapped = mapped_native_instances()
    owned = sage_owned_vertices()
    assert set(mapped.keys()) == set(owned), (
        f"mapped {len(mapped)} != sage-owned {len(owned)}: missing={set(owned) - set(mapped.keys())}, extra={set(mapped.keys()) - set(owned)}"
    )
...
def test_native_solid_is_compositional_subgraph_of_stub() -> None:
    ...
    assert not missing, f"native solid edges with no stub path (after VectorSpaces/axiom remaps): {missing}"
...
def test_native_axioms_are_subset_of_stub_axioms() -> None:
    ...
    assert result["only_in_native"] == set(), f"native-only axiom attachments vs stub: {result['only_in_native']}"
...
def test_native_is_subcategory_implies_stub() -> None:
    ...
    assert not failures, "native is_subcategory without stub agreement (child, parent, stub_ok, native_ok):\n" + "\n".join(f"  {row}" for row in failures)


def test_named_join_axioms_are_subset_of_native() -> None:
    gaps = named_join_axiom_gaps()
    assert not gaps, "named joins whose stub axioms are not contained in native axioms:\n" + "\n".join(f"  {name}: stub={stub} native={native}" for name, stub, native in gaps)
