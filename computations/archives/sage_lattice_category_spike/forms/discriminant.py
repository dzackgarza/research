...
def finite_all_subgroups(parent: "SageParent") -> tuple[Any, ...]:
    ...
    while frontier:
        ...
        for element in parent.elements():
            generated = parent.subgroup_generated_by(tuple(subgroup.group_generators()) + (element,))
            key = generated._key()
            if key not in seen:
                seen[key] = generated
...
class SyntheticDiscriminantSubgroup:
    ...
    def _closure(self) -> set[Any]:
        ...
        ranges = [range(self.ambient().order(generator)) for generator in self._group_generators]
        elements = set()
        for coefficients in product(*ranges):
            element = self.ambient().zero()
            for coefficient, generator in zip(coefficients, self._group_generators):
...
class SyntheticOrthogonalGroup(DiscriminantOrthogonalGroup):
    ...
    def as_permutation_group(self) -> "PermutationGroup":
        ...
        permutations = [[positions[action(element)] for element in elements] for action in self.group_generators()]
        if not permutations:
