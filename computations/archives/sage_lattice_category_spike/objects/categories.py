...
def own_methods(entity: type) -> type:
    ...
    delta = {name: value for name, value in vars(entity).items() if not name.startswith("__")}
    delta["__doc__"] = entity.__doc__
    return type("ParentMethods", (), delta)


_own_methods = own_methods
...
class Lattices(CatObject, Category_over_base_ring):
    ...
    def from_gram_matrix(
        ...
        return construct_synthetic_lattice(gram, base_ring, label, cartan_type=cartan_type, names=names)
    ...
    def _default_hom_resolution(self, domain: Lattice, codomain: Lattice) -> tuple[Lattice, Lattice, SageCategory]:
        ...
        if codomain.base_ring().coerce_map_from(domain.base_ring()) is not None:
            transported_domain = Lattices(domain.base_ring()).base_change(codomain.base_ring())(domain)
        ...
        if domain.base_ring().coerce_map_from(codomain.base_ring()) is not None:
            transported_codomain = Lattices(codomain.base_ring()).base_change(domain.base_ring())(codomain)
    ...
    def Hom(
        ...
        return SageHom(resolved_domain, resolved_codomain, category=resolved_category)
    ...
    def morphism(
        ...
        return self.Hom(domain, codomain)(matrix_data)
