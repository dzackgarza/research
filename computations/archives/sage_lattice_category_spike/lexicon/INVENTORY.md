...
`morphism.kernel()` computes the kernel matrix, builds an **abstract** module $S$, and returns the inclusion $S\hookrightarrow M$ — the kernel matrix never supplies $S$'s generators, it only records how those abstract generators are written in $M$'s. This is what Sage's matrices conflate: Sage says the submodule *is* spanned by a basis sitting inside an ambient module, which forces the ambient-module bookkeeping this repo eschews.
