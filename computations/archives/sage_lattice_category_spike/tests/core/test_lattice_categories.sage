...
def assert_matrix_equal(left: lexicon.Matrix, right: lexicon.Matrix) -> None:
    assert matrix(QQ, left) == matrix(QQ, right)


def test_category_axioms_bind_to_synthetic_lattice_contract() -> None:
    C = Lattices(ZZ)
...
def test_indefinite_hyperbolic_plane_is_first_class_synthetic_lattice() -> None:
    U = Lattice("U", label="U")
...
def test_category_dispatch_uses_form_axioms_not_constructor_factories() -> None:
...
def test_positive_definite_algorithms_are_axiom_gated() -> None:
    A2 = Lattice("A2(-1)", label="A2")
    U = Lattice("U", label="U")
...
def test_raw_module_methods_are_available_only_through_accessors() -> None:
    A2 = Lattice("A2(-1)", label="A2")
...
def test_rationalization_is_scalar_extension_not_external_ambient_embedding() -> None:
    U = Lattice("U", label="U")
...
def test_elements_have_owned_module_arithmetic_and_bilinear_form() -> None:
    U = Lattice("U", label="U")
    e, f = U.gens()
...
def test_rank_one_subobject_is_a_lattice_plus_inclusion() -> None:
    U = Lattice("U", label="U")
...
def test_dual_is_owned_zz_lattice_in_the_rationalization() -> None:
    A2 = Lattice("A2(-1)", label="A2")
    A2_dual = A2.dual()
...
def test_overlattice_is_the_inclusion_morphism_into_a_plain_lattice() -> None:
    U = Lattice("U", label="U")
...
def test_orthogonal_direct_sum_and_tensor_product_constructions_are_synthetic() -> None:
    U = Lattice("U", label="U")
...
def test_discriminant_group_is_owned_invariant_factor_quotient_with_forms() -> None:
    A2 = Lattice("A2(-1)", label="A2")
...
def test_odd_integral_discriminant_group_keeps_qq_mod_zz_form() -> None:
...
def test_discriminant_group_is_typed_as_a_finite_abelian_group_not_a_zz_module() -> None:
...
def test_discriminant_subgroups_actions_and_overlattice_construction_are_owned() -> None:
...
def test_homs_are_form_preserving_by_construction() -> None:
    U = Lattice("U", label="U")
...
def test_hom_image_and_kernel_are_synthetic_lattices() -> None:
    A2 = Lattice("A2(-1)", label="A2")
    A2_dual = A2.dual()
...
def test_integral_lattice_inclusion_into_dual_is_a_synthetic_morphism() -> None:
    A2 = Lattice("A2(-1)", label="A2")
    A2_dual = A2.dual()
    inclusion = A2.hom(Matrix(ZZ, 2, 2, [2, -1, -1, 2]), codomain=A2_dual)
...
def test_definiteness_and_nondegeneracy_predicates_derive_from_signature() -> None:
    A2 = Lattice("A2(-1)", label="A2")
    minus_A2 = Lattice(-A2.gram_matrix(), label="-A2")
    U = Lattice("U", label="U")
...
def test_radical_quotient_kills_the_radical_and_realizes_eperp_mod_e() -> None:
    U = Lattice("U", label="U")
...
def test_lattice_element_pairs_through_the_star_operator() -> None:
    r"""
...
def test_discriminant_group_element_pairs_through_the_star_operator() -> None:
    r"""
...
def test_explicit_isometries_act_on_discriminant_groups() -> None:
    U = Lattice("U", label="U")
...
def test_functor_layer_realizes_the_categorical_identities() -> None:
    U = Lattice("U", label="U")
    A2 = Lattice("A2(-1)", label="A2")
...
def test_positive_definite_enumeration_suite_matches_sage_and_is_axiom_gated() -> None:
    ...
    def squared_norm(coordinates: Sequence[int], center: Sequence[int] = (0, 0, 0)) -> lexicon.Rational:
        delta = vector(QQ, [QQ(coordinates[i]) - center[i] for i in range(3)])
        return delta * gram * delta

    def brute_force(predicate: Callable[[tuple[int, ...]], bool], lo: int, hi: int) -> set[tuple[int, ...]]:
...
def test_negative_definite_enumeration_transports_through_the_sign_twist() -> None:
...
def test_nikulin_overlattice_and_metabolizer_identities_hold() -> None:
    ...
    def as_finite_quadratic_form(form: lexicon.DiscriminantForm) -> lexicon.DiscriminantForm:
...
def test_orthogonal_group_is_lazily_computed_for_definite_and_explicit_for_indefinite() -> None:
...
def test_g1_indefinite_is_isometric_decides_via_sage_spinor_stack() -> None:
...
def test_reflection_is_an_order_two_isometry_defined_only_for_anisotropic_vectors() -> None:
...
def test_two_signal_placement_gates_dual_and_discriminant_vocabulary_by_stratum() -> None:
...
def test_placement_matrix_both_directions_over_the_spec_fixture_set() -> None:
    ...
    def vocabulary(domain_class: type[da.Lattice]) -> tuple[str, ...]:
        return tuple(
...
def test_named_constructors_route_through_the_category_entry_with_provenance() -> None:
...
def test_rational_dual_is_the_canonical_self_identification() -> None:
...
def test_isometry_group_object_algebra_and_negative_definite_delegation() -> None:
...
def test_endomorphisms_form_a_monoid_with_ring_like_predicates() -> None:
...
def test_bilinear_forms_carry_the_induced_diagonal_quadratic_form() -> None:
...
def test_rational_isometry_decides_by_square_class_via_the_oracle() -> None:
...
def test_cross_ring_isometry_homset_is_an_incoherent_construction() -> None:
