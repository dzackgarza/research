...
def test_named_root_lattices_are_negative_definite_by_default() -> None:
...
def test_minus_one_spelling_is_the_positive_twist_with_provenance() -> None:
...
def test_there_is_no_boolean_sign_flag() -> None:
    r"""The sign is carried by the twist, never a boolean mode flag (policy)."""
    with pytest.raises(TypeError):
        lc.Lattice("E8", negative=True)


def test_root_lattice_is_a_sublattice_of_a_unimodular_lattice_via_embedding() -> None:
...
def test_e8_bourbaki_embedding_realizes_the_half_integral_roots() -> None:
...
def test_is_root_is_reflection_integrality_not_norm_pm2() -> None:
...
def test_is_root_rejects_isotropic_and_non_reflective_vectors() -> None:
...
def test_simple_roots_are_roots() -> None:
    A2 = lc.Lattice("A2")
