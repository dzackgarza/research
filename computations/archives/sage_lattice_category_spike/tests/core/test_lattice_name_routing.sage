...
def test_named_construction_yields_root_generated_membership() -> None:
...
def test_named_construction_carries_the_cartan_datum() -> None:
    assert Lattice("E8").cartan_type() == ("E", 8)
    assert Lattice(("A", 2)).cartan_type() == ("A", 2)


def test_minus_one_spelling_routes_through_lattice_itself_with_provenance() -> None:
...
def test_raw_gram_matrix_never_acquires_the_certificate() -> None:
...
def test_hyperbolic_plane_by_name_routes_to_hyperbolic() -> None:
    L = Lattice("U")
    assert L in Lattices(ZZ).Indefinite().Hyperbolic()
    assert L not in Lattices(ZZ).Even().RootGenerated()


def test_named_construction_defaults_label_to_the_name() -> None:
    assert "A2" in repr(Lattice("A2"))
    assert "E8(-1)" in repr(Lattice("E8(-1)"))


def test_names_compose_with_name_routing() -> None:
    L.<e, f> = Lattice("U")
...
def test_there_is_no_public_root_lattice_constructor() -> None:
...
def test_there_is_no_public_u_constructor() -> None:
...
def test_scaled_hyperbolic_plane_is_composed_not_dispatched() -> None:
