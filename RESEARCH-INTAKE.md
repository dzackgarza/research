...
| Extensions as twisted products — groups and fibrations | User note 2026-09-15 | For `1→A→B→C→1` compute `B` as twisted product `A ⋊_{cocycle} C` from `Ext^1(C,A)` class / factor set; for `F→E→B` compute `E ≃ F×_τ B` from `k`-invariant / twisting cocycle in `H^{n+1}(B;π_n(F))` or `Cotor`, via bar/cobar and `Ext` | `categories/groups/extensions.py` + `categories/algebras/extensions.py` + `categories/topology/fibrations.py` + `categories/homotopy/twisted_products.py` + `categories/derived/ext_tor.py` | Proposed — see note below |
...
## Desired capability: extensions as twisted product / fibrations as twisted product — note 2026-09-15
...
* For groups `1→A→B→C→1` short exact (e.g. `A,C : Groups` with `A ⊲ B` normal, `B/A ≅ C`; `A` abelian or general, `C` acting on `A` via `C → Out(A)`), `B` is not a bare `Group` with a name `B`. It is the twisted product (extension) `B = A ⋊_{α} C` where `α ∈ H^2(C, A)` / `Ext^1(C,A)` / `H^2(C, Z(A))` class determines the multiplication: choose section `s: C→B` (`π∘s=id_C`, `s(1)=1` after normalization), then 2-cocycle `c: C×C→A` with `c(x,y)= s(x)s(y)s(xy)^{-1} ∈ A` (factor set), and `B = {(a,c) | a∈A, c∈C}` with `(a_1,c_1)(a_2,c_2)= (a_1·^{c_1}a_2·c(c_1,c_2), c_1c_2)` where `^{c}a = s(c) a s(c)^{-1}` action `C ↷ A`. Class `[c]∈H^2(C,A)` (or `Ext^1` when `A` abelian `Z[C]`-module, or more generally `H^2(C, Z(A))` when `A` nonabelian with center). `Ext` already intaken via `RHom` on `D(ZZ[C]-Mod)` / `D(ZZ-Mod)` with `Ext^n = R^n Hom`; now algorithmically realize extension: from `[c]` compute `B_c = A ×_c C` as `Groups` object with `1→A→B_c→C→1` and `[B_c]=[c]`. Similarly split case `c=0 ⇒ B = A⋊C` semidirect, central extensions `A ≤ Z(B)` when action trivial. Interface `Extension(A,C,[c]) → B : Groups` with `cocycle: C×C→A` as `Map` satisfying `dc=0` (2-cocycle `c(xy,z)·c(x,y)^{z}=c(x,yz)·c(y,z)`) and `coboundary` equivalence `[c]=[c'] ⇔ ∃b:C→A c'=c·db`, and `B.extension_class() → [c] ∈ H^2(C,A)`.

* Algorithmically computing `B` from `[c]`/`c` requires: group cohomology `H^n(G,M)=Ext_{ZZ[G]}^n(ZZ,M)` already intaken via `BarRes → RHom` with `M : ZZ[C]-Mod` (the `C`-module `A` via `action`), cocycle `c ∈ Z^2(C,A)=ker d^2 ⊂ C^2`, class `[c]∈H^2=Z^2/B^2`, and explicit twisted group law above. For `A` abelian finite with known `Ext^1(C,A)=H^2(C,A)` via Smith over `ZZ` when `C` finite / `C=ZZ^n` etc., produce `B` as `Groups` with underlying set `A×C` and operation `*_c`.

* Similarly for modules/algebras: `0→A→B→C→0` in `R-Mod` (abelian category) is determined by `Ext^1_R(C,A) = H^1(RHom(C,A))` class `ξ: C→A[1]` in `D(R-Mod)`; again `B = A ⊕_{ξ} C` as Baer sum / pushout of extension `ξ`. For abelian `C,A : R-Mod`, `0→A→B→C→0` corresponds to `ξ ∈ Ext^1(C,A)` and `B = (A⊕P)/{(i(p),-p)}` where `P→C` is projective presentation `0→K→P→C→0` and `i: K→A` represents `ξ`. Interface `C.extension_by(A, ξ∈Ext^1(C,A)) → B : R-Mod` with `Ext` already intaken.

* Similarly for fibrations `F→E→B` (Serre fibration `F↪E↠B` with `B` path-connected, `F` fiber over basepoint `b0∈B`, `E = F ×_τ B` twisted cartesian product already intaken via Kenzo `F×_τ B` with twisting operator `τ: B_{∗+1}→G_∗` where `G` acts on `F` via `G=Aut(F)` or `ΩB` loop group. Here `E` as twisted product `F ×_τ B` is determined by `k`-invariant / twisting cocycle `τ ∈ Z^{n+1}(B; π_n(F))` / `H^{n+1}(B;π_n(F))` when `F=K(π_n,n)` iteratively (Postnikov), or more generally by `τ ∈ [B, BAut(F)]` classifying map. Construction `E = F×_τ B` uses same `tEZ` / `BPL` / `Bar/Cobar` as `F×_τ B ⇔ F⊗_t B`; now algorithmically `B` base `sSet` with `EC` effective, `F` fiber with `EC`, and cocycle `τ` as simplicial twisting operator (maps `B_{n+1}→F_n` satisfying Kan condition `d_0 τ(b)= τ(d_0b)·∂b` etc.), then `F×_τ B` sSet with simplices `(f,b)`, `d_0(f,b)=(d_0f·τ(b), d_0b)`. Interface `TwistedProduct(F,B,τ) → E = F×_τ B : sSet` (or `Top`) with `E→B` projection and `F↪E` fiber inclusion, and `E.extension_class() → [τ] ∈ H^{n+1}(B; π_n(F))` / `[B→BAut(F)]`.

* Requires: `H^n(G,M)=Ext_{ZZG}` already intaken (group cohomology), `Ext^1_R(C,A)=R^1Hom` already intaken (derived `RHom`/`D(R-Mod)`), `H^{n}(B;π)` via `sSet` cohomology `H^n(B, π)` already intaken via `EC`/`Ω/B` effective, `Bar`/`Cobar` for `H^2(C,A)` as `Ext`, and `sSet` twisting operator `τ` / `G ⊣ W̄` already intaken. No new cohomology — reuse `Ext`/`Tor`/`D` and `sSet`/`τ`.

Intended owners: `categories/groups/extensions.py` (`GroupExtension 1→A→B→C→1` via `cocycle c∈Z^2(C,A)`, `B=A×_cC` twisted product with `B.extension_class()∈H^2(C,A)`), `categories/algebras/extensions.py` (`ModuleExtension 0→A→B→C→0` via `ξ∈Ext^1_R(C,A)` Baer sum), `categories/topology/fibrations.py` + `categories/homotopy/twisted_products.py` (`TwistedProduct F×_τ B` with `τ: B→G` twisting operator, `E→B` fibration with `τ-cohomology` class `[τ]∈[B,BAut(F)]`/`H^{n+1}`), delegating to `categories/derived/ext_tor.py` (`Ext^1`, `H^2`), `categories/group/cohomology.py` (`H^2(C,A)` via `Bar`), `categories/topology/simplicial_sets.py` (`τ` Kan condition). Not a free `extension(A,C,cocycle)` returning bare set — `Extension(A,C,c)` as `Groups`/`R-Mod`/`sSet` object `B=(A×C, *_c)` / `F×_τ B` with `1→A→B→C→1` / `F↪E↠B` as `D`/`sSet` distinguished triangle/fibration with class in `H^2`/`H^{n+1}`.
