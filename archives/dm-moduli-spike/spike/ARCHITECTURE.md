...
| `atlas()` / `etale_atlas()` | **Deepened, still incomplete** — single owned-presentation registry (`moduli/atlas_ownership.py`: `owned_etale_atlas_presentations()` / `dispatch_etale_atlas` / `etale_atlas_gap_from_registry`). Every `ModuliStack.etale_atlas()` goes through that dispatch: owned row → equation-level domain, else fail-closed `AtlasChart` + structured `etale_atlas_gap()`. Default cardinality **27** (parametric open Knudsen `M_{0,n}` for all `n≥3`; parametric open Legendre/Hesse `M_{1,n}` for all `n≥1`; parametric compact Legendre/Hesse `Mbar_{1,n}` for all `n≥1` via `_legendre_compact_M1n_covering_space` / `_hesse_compact_M1n_covering_space` (`ℙ¹` for `n=1`); plus parametric proper Kapranov `Mbar_{0,n}` for `3≤n≤8` via `kapranov_iterated_blowup_P_{n-3}`, specializing to point / `ℙ¹` / `Bl₄(ℙ²)` / `Bl(ℙ³)` / lazy `Bl(ℙ⁴)` / lazy `Bl(ℙ⁵)`; plus open unmarked Igusa `M_{2,0}` via `igusa_binary_sextic_PGL2` / Rosenhain `Spec(R[λ,μ,ν]_S)` with finite étale `S₆ ≅ Sp₄(𝔽₂)` under `2∈Rˣ`; plus proper unmarked `Mbar_2` via `igusa_mbar06_s6` = Kapranov `Mbar_{0,6}` / `S₆`; plus parametric open marked Rosenhain `M_{2,n}` (`n≥1`) via `_igusa_open_M2n_affine_scheme`; plus parametric compact marked `Mbar_{2,n}` (`n≥1`) via `igusa_compact_rosenhain_*` = Kapranov fiber-product `IgusaCompactMarkedM2nAlgebraicSpace`; plus open unmarked del Pezzo `M_{3,0}` via `del_pezzo_degree2_seven_points_PGL3` / `W(E₇)` under `2∈Rˣ`, covering the non-hyperelliptic dense open; plus locus-only hyperelliptic binary-octic `hyperelliptic_binary_octic_M08_S8` / `S₈`; plus compact del Pezzo `Mbar_3` via `del_pezzo_compact_seven_points_PGL3` = lazy `(ℙ²)³` charts / `W(E₇)` (non-hyperelliptic dense open); plus locus-only hyperelliptic `hyperelliptic_mbar08_s8`; plus parametric marked del Pezzo open/compact `M_{3,n}` (`W(E₇)`); plus locus-only marked hyperelliptic; plus open unmarked canonical `M_{4,0}` via `genus4_canonical_quadric_cubic_P3`; plus locus-only trigonal `genus4_trigonal_cone_cubic_P3`; plus compact Petri `Mbar_4` via `genus4_compact_canonical_petri_P9`; plus parametric marked Petri open/compact `M_{4,n}` / `Mbar_{4,n}` (trigonal locus-only); plus open three-quadrics `M_{5,0}` via `genus5_canonical_three_quadrics_P4` with locus-only trigonal/hyperelliptic). Expand `expand_open_m0n_through=8` + `expand_open_m1n_through=4` + `expand_compact_m1n_through=4` + `expand_proper_m0n_through=8` + `expand_open_m2n_through=4` + `expand_compact_m2n_through=4` + `expand_open_m3n_through=4` + `expand_compact_m3n_through=4` + `expand_open_m4n_through=4` + `expand_compact_m4n_through=4` → **76** concrete rows / **51** type keys. Fail-closed: `(1,*)` when neither `2` nor `3` is a unit (Weierstrass `𝔾_m` evidence for `n≤2`); Igusa / del Pezzo types without `2` unit (`igusa_requires_two_invertible` / `del_pezzo_requires_two_invertible`); `Mbar_{0,n}` for `n>8` with named gap `kapranov_iterated_blowup_P_{n-3}` — **no invented charts**. Remaining pre-#225 atlas debt is **marked `M_{5,n}`, `g≥6`, compact `Mbar_5`, and char-2 Igusa/del Pezzo/hyperelliptic/genus-4/5**. Wave 2 still waits on [#225](https://github.com/dzackgarza/research/pull/225) merge. |
