...
class TorsionModulesWithForm(OwnedCategoryOverBaseRing):
    ...
    class ParentMethods:
        ...
        def gram_matrix(self: "TorsionFormParent") -> GramMatrix:
            ...
            """
            stored: GramMatrix | None = self._subdivided_gram_matrix
            if stored is not None:
                return stored
            form = self.form()
            target = form.codomain()
        ...
        # ---- orbits of a group of automorphisms ----

        def orbit(
            self: "TorsionFormParent",
            element: "Element",
            group: "Parent | None" = None,
        ) -> frozenset:
            r"""Return the orbit $G\cdot x=\{g(x):g\in G\}$ of ``element``.

            $G$ is the full $O(A)$ when no group is supplied, or any
            iterable group of automorphisms of $A$ -- a generated subgroup,
            the image of a discriminant representation.  Finite and
            decidable: $A$ is finite and the acting group is enumerated.
            """
            acting = self.automorphism_group() if group is None else group
            element = self(element)
            return frozenset(action(element) for action in acting)

        def orbits(
            self: "TorsionFormParent",
            group: "Parent | None" = None,
        ) -> tuple:
            r"""Return the orbit partition of $A$ under the acting group.

            The orbits of a group action partition the set acted on; here
            both are finite, so the partition is computed by exhausting the
            elements orbit by orbit.
            """
            acting = self.automorphism_group() if group is None else group
            unseen = set(self)
            partition = []
            while unseen:
                ...
                reached = self.orbit(representative, group=acting)
        ...
        def _subgroups(
            ...
            r"""Iterate over ALL subgroups of $A$ as (elements, generators).

            The unfiltered sibling of :meth:`_isotropic_subgroups`: every
            subgroup is reached from the trivial one by repeatedly adjoining
            an element, so the chain search with no vanishing condition
            exhausts them, each reported once.
            """
            zero = self.zero()
            trivial = frozenset((zero,))
            yield trivial, ()
            nonzero = tuple(
                element for element in self if element != zero
            )
            seen = set([trivial])
            frontier = [(trivial, ())]
            while frontier:
        ...
        def _orbits_on_subgroup_family(
            self: "TorsionFormParent",
            family: "Iterator[tuple[frozenset, tuple]]",
            group: "Parent | None",
        ) -> tuple:
            r"""Return the orbit partition of a family of subgroups of $A$.

            A subgroup is carried as its element set (what identity is
            decided on) together with a generating chain (what the returned
            subobject is built on); an automorphism moves a subgroup to the
            subgroup generated by the images of its generators.
            """
            acting = self.automorphism_group() if group is None else group
            remaining = dict(family)
            partition = []
            while remaining:
                elements, module_generators = next(iter(remaining.items()))
                reached: dict = {}
                for action in acting:
                    image_elements = frozenset(
                        action(member) for member in elements
                    )
                    image_generators = tuple(
                        action(generator) for generator in module_generators
                    )
                    reached.setdefault(image_elements, image_generators)
                partition.append(
                    tuple(
                        self.subobject_generated_by(generators)
                        for generators in reached.values()
                    )
                )
                for key in reached:
        ...
        def orbits_on_subobjects(
            self: "TorsionFormParent",
            group: "Parent | None" = None,
        ) -> tuple:
            r"""Return the orbits of the acting group on ALL subobjects of $A$.

            An automorphism carries a subgroup to a subgroup, so the action
            on elements induces one on the (finite) family of subobjects;
            this is its orbit partition, each orbit a tuple of subobjects.
            """
            return self._orbits_on_subgroup_family(self._subgroups(), group)

        def orbits_on_isotropic_subobjects(
            self: "TorsionFormParent",
            group: "Parent | None" = None,
        ) -> tuple:
            r"""Return the orbits of the acting group on the isotropic subobjects.

            An automorphism preserves the form, so it carries isotropic
            subgroups to isotropic subgroups and the family
            :meth:`_isotropic_subgroups` enumerates is closed under the
            action; the partition is computed on that family directly.
            This is the orbit vocabulary Nikulin's gluing theory consumes
            (Nik80 Prop 1.4.1: even overlattices correspond to isotropic
            subgroups, isometric ones to $O(A)$-orbits).
            """
            return self._orbits_on_subgroup_family(
                self._isotropic_subgroups(), group
            )
        ...
        def subquotient_form(
            ...
            embedding = subobject.structure_morphism()
            larger = over.structure_morphism()
...
def _subgroup_generated(subgroup: frozenset, element: "Element") -> frozenset:
    ...
    return frozenset(
        ...
        for multiple in range(element.additive_order())
    )
...
def relations_among(form: "Morphism", module_generators: "OrderedSet") -> "Matrix":
    ...
    kernel = matrix(SageZZ, lifts).stack(known).left_kernel_matrix()
...
class TorsionFormOrthogonalSubgroups(Category):
    ...
    Held as the engine's subgroup on the crossed generators, not as the
    closure of the generating set under composition.  Both are the same
    group, but the closure is a list of its elements, and the subgroups this
    vocabulary is *for* are too large to list: the stabilizer of the
    degree-two Enriques polarization class has order $94\,755\,225\,600$.
    Order and membership are asked of the engine's group without enumerating it.
    """
    ...
    class ParentMethods:
        def __init__(
            ...
            **rest: "ConstructionData",
            ...
            self._group_generators = finite_ordered_set(group_generators)
            self._domain = supergroup.domain()
            self._engine_group = supergroup._engine_group.subgroup(
                [supergroup._to_engine(generator) for generator in group_generators]
            )

            def defining_predicate(candidate: "Morphism") -> bool:
                return supergroup._to_engine(candidate) in self._engine_group

            super().__init__(
                containing_group=supergroup,
                predicate=defining_predicate,
                description="generated by the specified orthogonal automorphisms",
                cardinality=cardinal(self._engine_group.order()),
                **rest,
            )
        ...
        def intersection(
            ...
            r"""Return $G\cap H$ inside $O(A)$.

            The intersection of two finite subgroups, computed as one by the
            engine's group rather than by comparing element lists; the generators
            it reports cross back as morphisms of $O(A)$.
            """
            from sage.libs.gap.libgap import libgap

            assert (
                other in TorsionFormOrthogonalSubgroups()
                and other.supergroup() is self.supergroup()
            ), "an intersection of subgroups happens inside one O(A)"
            met = libgap.Intersection(
                self._engine_group.gap(), other._engine_group.gap()
            )
            return self.supergroup().subgroup_on(
                tuple(
                    self.supergroup()._from_engine(generator)
                    for generator in self.supergroup()._engine_group._subgroup_constructor(
                        met
                    ).gens()
                )
            )
        ...
        def _repr_(self) -> str:
            return (
                f"Subgroup of the orthogonal group of {self.domain()} "
                f"of order {self.cardinality()}"
            )
