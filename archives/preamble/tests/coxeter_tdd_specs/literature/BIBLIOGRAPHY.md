...
The assignment is recorded here because it is the mathematical decision; the bibliographic entries themselves are **not** written below, because no copy of these works was open while this section was written and a citation key must never be transcribed from memory.
Resolve each against the library and add a full entry above before citing it in a test.
...
Every author-title pair above is transcribed from the corpus's own prompts and test comments, not recalled.
Of the works named, only Humphreys, Bourbaki and Conway–Burgiel have entries above; Kac, Vinberg, Ratcliffe, Ebeling, Serre, Jacobson and Conway–Sloane do not.
