...
| `foundations/v1-consolidated-theory.md` | The v1 archive's own consolidation, and the shortest complete statement of the program: the classification, then a definitions section (Gram matrix, subdiagram, maximal parabolic subdiagram, Coxeter matrix). It carries three statements the other foundations files do not: **eigenvalue monotonicity** (a non-elliptic subdiagram has no elliptic superdiagram, by interlacing), **signature inheritance** for principal submatrices, and **Vinberg's covolume criterion**. |
