<!--
Origin: gitclones/Coxeter-v2/archive/cold_storage_pre_integration/background-research/sage-planning/modules_bak/RMod/BilRMod/BilRMod_category.md
...
```python
# Mathematical assertion: Bilinear forms on canonical root lattices
# sage: R = RootSystem(['A', 3])
# sage: L = R.root_lattice()
# sage: G = L.gram_matrix()
# sage: G.is_symmetric()
# True  # Gram matrices are always symmetric (bilinear form property)

# Mathematical assertion: Negative definiteness for finite types
# sage: R = RootSystem(['E', 8])
# sage: L = R.root_lattice()
# sage: G = L.gram_matrix()
# sage: all(e < 0 for e in G.eigenvalues())
# True  # E8 is finite type, so Gram matrix is negative definite (project convention)

# Mathematical assertion: Bilinear form evaluation preserves structure
# sage: R = RootSystem(['B', 4])
# sage: L = R.root_lattice()
# sage: alpha = L.simple_roots()
# sage: alpha[1].inner_product(alpha[1])
# -2  # Simple roots have self-pairing -2 (negative convention)

# Mathematical assertion: Discriminant invariance under basis change
# sage: R = RootSystem(['D', 5])
# sage: L = R.root_lattice()
# sage: G = L.gram_matrix()
# sage: det1 = G.determinant()
# sage: # Apply a basis change
# sage: P = matrix(ZZ, 5, 5)
# sage: P[0,0] = P[1,1] = P[2,2] = P[3,3] = P[4,4] = 1
# sage: P[0,1] = 1  # Simple basis change
# sage: G_new = P.transpose() * G * P
# sage: det2 = G_new.determinant()
# sage: det2 / det1 == P.determinant()^2
# True  # Discriminant changes by square of transformation determinant

# Mathematical assertion: Radical detection for degenerate forms
# sage: # Create a degenerate bilinear form (affine type has zero eigenvalue)
# sage: R = RootSystem(['A', 2, 1])  # Affine A2
# sage: L = R.root_lattice()
# sage: G = L.gram_matrix()
# sage: 0 in G.eigenvalues()
# True  # Affine types have exactly one zero eigenvalue (degenerate form)

# Mathematical assertion: Direct sum preserves bilinear structure
# sage: R1 = RootSystem(['A', 1])
# sage: L1 = R1.root_lattice()
# sage: G1 = L1.gram_matrix()
# sage: R2 = RootSystem(['A', 2])
# sage: L2 = R2.root_lattice()
# sage: G2 = L2.gram_matrix()
# sage: # Direct sum has block diagonal Gram matrix
# sage: G_sum = block_matrix([[G1, 0], [0, G2]])
# sage: G_sum.dimensions()
# (3, 3)  # Direct sum of rank 1 and rank 2 lattices

# Mathematical assertion: Orthogonality in root systems
# sage: R = RootSystem(['F', 4])
# sage: L = R.root_lattice()
# sage: alpha = L.simple_roots()
# sage: # Non-adjacent simple roots are orthogonal
# sage: alpha[1].inner_product(alpha[3])
# 0  # Simple roots α₁ and α₃ are not connected in F4 Dynkin diagram

# Mathematical assertion: Tensor product of bilinear forms
# sage: from sage.quadratic_forms.quadratic_form import QuadraticForm
# sage: # For crystallographic types, tensor products preserve definiteness
# sage: R = RootSystem(['G', 2])
# sage: L = R.root_lattice()
# sage: G = L.gram_matrix()
# sage: # G2 has mixed root lengths, non-symmetric Cartan matrix
# sage: C = R.cartan_matrix()
# sage: C.is_symmetric()
# False  # G2 has non-symmetric Cartan matrix due to different root lengths
```
...
## Additional Mathematical Test Assertions

```python
# Mathematical assertion: Symmetric monoidal category structure
# sage: # Verify that bilinear modules form a symmetric monoidal category
# sage: # Pentagon axiom: associator coherence
# sage: R1 = RootSystem(['A', 1])
# sage: R2 = RootSystem(['A', 1]) 
# sage: R3 = RootSystem(['A', 1])
# sage: R4 = RootSystem(['A', 1])
# sage: L1 = R1.root_lattice()
# sage: L2 = R2.root_lattice()
# sage: L3 = R3.root_lattice()
# sage: L4 = R4.root_lattice()
# sage: # The pentagon axiom ensures associativity is coherent
# sage: # ((L1⊗L2)⊗L3)⊗L4 can reassociate to L1⊗(L2⊗(L3⊗L4)) in two ways
# sage: # and both paths must commute

# Mathematical assertion: Witt decomposition for indefinite forms
# sage: # Hyperbolic plane has signature (1,1,0)
# sage: from sage.quadratic_forms.quadratic_form import QuadraticForm
# sage: # Standard hyperbolic plane: [[0,1],[1,0]]
# sage: # Has one positive and one negative eigenvalue
# sage: # Witt index = 1 (maximal isotropic subspace dimension)

# Mathematical assertion: Orthogonal groups preserve bilinear forms
...
# sage: M = w.matrix()
# sage: M.transpose() * G * M == G
# True  # Weyl group elements preserve the bilinear form

# Mathematical assertion: Submodule restriction preserves form properties
# sage: R = RootSystem(['E', 6])
# sage: L = R.root_lattice()
# sage: # Take subroot system of type D5
# sage: # The restricted Gram matrix is a principal submatrix
# sage: # and inherits negative definiteness from E6

# Mathematical assertion: Quotient by isotropic submodule is well-defined
# sage: # For affine root systems, the null space is 1-dimensional
# sage: R = RootSystem(['B', 3, 1])  # Affine B3
# sage: L = R.root_lattice()
# sage: G = L.gram_matrix()
# sage: # The null vector spans an isotropic submodule
# sage: # Quotient by this submodule gives a non-degenerate form

# Mathematical assertion: Alternating forms in even characteristic
# sage: # Over GF(2), skew-symmetric and alternating differ
# sage: # A form can be skew-symmetric but not alternating
# sage: # This happens when diagonal entries are non-zero in char 2

# Mathematical assertion: Anisotropic forms have no isotropic vectors
# sage: R = RootSystem(['A', 2])
# sage: L = R.root_lattice()
# sage: G = L.gram_matrix()
# sage: # For negative definite G, no non-zero v satisfies v^T G v = 0
# sage: # This means the form is anisotropic (Witt index 0)

# Mathematical assertion: Field extensions for non-crystallographic types
# sage: # H3 requires the golden ratio field
# sage: # phi = (1 + sqrt(5))/2 satisfies phi^2 - phi - 1 = 0
# sage: # The Gram matrix entries involve cos(pi/5) = phi/2
# sage: # All computations must be exact in Q(phi)
```