<!--
Origin: gitclones/Coxeter-v2/archive/cold_storage_pre_integration/background-research/sage-planning/modules_bak/RMod/BilRMod/CoxeterLattice/coxeter_lattices.md
...
The following assertions verify key mathematical properties of Coxeter lattices using SageMath's canonical constructions. These are ungameable property-based tests that validate mathematical relationships rather than specific values.

### Root System and Lattice Structure Tests

```python
# Mathematical assertion: Root system cardinality follows standard formulas
# Reference: Humphreys "Reflection Groups and Coxeter Groups", Table A.1
# sage: R_A3 = RootSystem(['A', 3])
# sage: len(R_A3.root_lattice().roots())
# 12  # A_n has 2n(n+1) roots = 2·3·4/2 = 12
# sage: R_E8 = RootSystem(['E', 8])
# sage: len(R_E8.root_lattice().roots())
# 240  # E8 has exactly 240 roots (Conway & Sloane)
```

```python
# Mathematical assertion: Weyl group orders follow known formulas
# Reference: Humphreys "Reflection Groups and Coxeter Groups", Ch 3
# sage: A3 = RootSystem(['A', 3])
# sage: A3.weyl_group().order()
# 24  # |W(A_n)| = (n+1)! = 4! = 24
# sage: B3 = RootSystem(['B', 3])
# sage: B3.weyl_group().order()
# 48  # |W(B_n)| = 2^n · n! = 8 · 6 = 48
# sage: G2 = RootSystem(['G', 2])
# sage: G2.weyl_group().order()
# 12  # |W(G_2)| = 12 (dihedral group)
```

### Embedding Index Properties Tests

```python
# Mathematical assertion: Root vs weight lattice embedding indices
# Reference: Bourbaki Lie Groups Ch 6, Proposition 1.6
# sage: A2_root = RootSystem(['A', 2]).root_lattice()
# sage: A2_weight = RootSystem(['A', 2]).weight_lattice()
# sage: A2_weight.index_in(A2_root)
# 3  # |P/Q| = 3 for A_2 (index of root in weight lattice)
# sage: E8_root = RootSystem(['E', 8]).root_lattice()
# sage: E8_weight = RootSystem(['E', 8]).weight_lattice()
# sage: E8_weight.index_in(E8_root)
# 1  # E8 is self-dual: weight lattice = root lattice
```

### Lattice Signature and Classification Tests

```python
# Mathematical assertion: Gram matrix signatures follow Coxeter theory
# Reference: Vinberg "Hyperbolic reflection groups", definiteness criteria
# sage: A3 = RootSystem(['A', 3])
# sage: G = A3.root_lattice().gram_matrix()
# sage: G.eigenvalues()  # All negative (finite type)
# # Signature should be (0, 3, 0) - negative definite
# sage: all(ev < 0 for ev in G.eigenvalues())
# True  # Finite Coxeter types have negative definite Gram matrices
```

### Root Length and Simply-Laced Properties Tests

```python
# Mathematical assertion: Simply-laced vs multiple root length classification
# Reference: Humphreys "Reflection Groups and Coxeter Groups", Ch 2.10
# sage: A3 = RootSystem(['A', 3])
# sage: A3.is_simply_laced()
# True  # A_n types have all roots of equal length
# sage: B3 = RootSystem(['B', 3])
# sage: B3.is_simply_laced()
# False  # B_n types have two root lengths
# sage: F4 = RootSystem(['F', 4])
# sage: F4.is_simply_laced()
# False  # F4 has two root lengths
```

### Cartan vs Gram Matrix Relationship Tests

```python
# Mathematical assertion: Cartan-Gram relationship for simply-laced types
# Reference: Bourbaki Lie Groups Ch 6, Section 1.1
# sage: A2 = RootSystem(['A', 2])
# sage: cartan = A2.cartan_matrix()
# sage: gram = A2.root_lattice().gram_matrix()
# sage: cartan == -gram  # For simply-laced: Cartan = -Gram
# True  # Simply-laced types satisfy A = -G
# sage: B2 = RootSystem(['B', 2])
# sage: cartan_B = B2.cartan_matrix()
# sage: gram_B = B2.root_lattice().gram_matrix()
# sage: cartan_B == -gram_B
# False  # Non-simply-laced types: A ≠ -G due to root length differences
```

### Parabolic Subgroup Structure Tests

```python
# Mathematical assertion: Maximal parabolic count equals rank
# Reference: Bourbaki Lie Groups Ch 4, Section 1.3
# sage: A3 = RootSystem(['A', 3])
# sage: W = A3.weyl_group()
# sage: parabolics = [W.parabolic_subgroup(S) for S in combinations(W.simple_reflections(), 2)]
# sage: len(parabolics)
# 3  # A_3 has 3 maximal parabolics (removing one simple root each)
# sage: D4 = RootSystem(['D', 4])
# sage: W_D4 = D4.weyl_group()
# sage: max_parabolics = [W_D4.parabolic_subgroup(S) for S in combinations(W_D4.simple_reflections(), 3)]
# sage: len(max_parabolics)
# 4  # D_4 has 4 maximal parabolics
```

### Exceptional Group Properties Tests

```python
# Mathematical assertion: Exceptional group orders match literature
# Reference: Conway & Sloane "Sphere Packings, Lattices and Groups", Appendix A
# sage: E6 = RootSystem(['E', 6])
# sage: E6.weyl_group().order()
# 51840  # |W(E_6)| = 2^7 · 3^4 · 5 = 51840
# sage: E7 = RootSystem(['E', 7])
# sage: E7.weyl_group().order()
# 2903040  # |W(E_7)| = 2^10 · 3^4 · 5 · 7 = 2903040
# sage: F4 = RootSystem(['F', 4])
# sage: F4.weyl_group().order()
# 1152  # |W(F_4)| = 2^7 · 3^2 = 1152
```

These assertions ensure that any implementation of CoxeterLattice correctly implements the mathematical structure of root systems, lattice embeddings, and Weyl group actions according to the established literature.