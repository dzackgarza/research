<!--
Origin: gitclones/Coxeter-v2/archive/cold_storage_pre_integration/background-research/sage-planning/modules_bak/RMod/BilRMod/SymBilRMod/nondegenerate_lattices/nondegenerate_lattices.md
...
```python
# Mathematical assertion: Non-zero determinant characterizes nondegeneracy
# sage: R = RootSystem(['A', 5])
# sage: C = R.cartan_matrix()
# sage: G = -C  # Gram matrix for simply-laced types
# sage: G.determinant() != 0
# True  # All finite type root lattices are nondegenerate (Bourbaki, Ch. VI)

# Mathematical assertion: Gram matrix invertibility equivalent to nondegeneracy  
# sage: R = RootSystem(['E', 7])
# sage: C = R.cartan_matrix()
# sage: G = -C
# sage: G.is_invertible() == (G.determinant() != 0)
# True  # Invertibility ⟺ trivial kernel ⟺ nondegenerate

# Mathematical assertion: All classical root lattices have full rank Gram matrices
# sage: R = RootSystem(['D', 6])
# sage: C = R.cartan_matrix()
# sage: G = -C
# sage: G.rank() == G.nrows()
# True  # Full rank implies trivial radical (Conway & Sloane, Sphere Packings)

# Mathematical assertion: Definite lattices have no isotropic vectors
# sage: R = RootSystem(['F', 4])  
# sage: C = R.cartan_matrix()
# sage: G = -C  # Note: This is simplified, F4 has multiple root lengths
# sage: all(e != 0 for e in G.eigenvalues())
# True  # No zero eigenvalues means no isotropic vectors

# Mathematical assertion: Hyperbolic planes have signature (1,1,0)
# sage: H = matrix(ZZ, [[0, 1], [1, 0]])  # Standard hyperbolic plane
# sage: eigs = H.eigenvalues()
# sage: pos = len([e for e in eigs if e > 0])
# sage: neg = len([e for e in eigs if e < 0])
# sage: zero = len([e for e in eigs if e == 0])
# sage: (pos, neg, zero) == (1, 1, 0)
# True  # Hyperbolic signature by definition (Vinberg, 1984)

# Mathematical assertion: E8 root lattice is unimodular
# sage: R = RootSystem(['E', 8])
# sage: C = R.cartan_matrix()
# sage: G = -C
# sage: abs(G.determinant()) == 1
# True  # E8 is the unique even unimodular lattice in dimension 8 (Conway & Sloane)

# Mathematical assertion: Discriminant divides index products
# sage: R = RootSystem(['A', 3])
# sage: C = R.cartan_matrix()  
# sage: G = -C
# sage: disc = abs(G.determinant())
# sage: disc == 4  # |W|/|W'| where W = weight lattice, W' = root lattice
# True  # A_n has discriminant n+1 (Humphreys, Reflection Groups)

# Mathematical assertion: Simply-laced Gram matrices are symmetric negative definite
# sage: for typ in [['A', 4], ['D', 5], ['E', 6]]:
# ...     R = RootSystem(typ)
# ...     C = R.cartan_matrix()
# ...     G = -C
# ...     symmetric = (G == G.transpose())
# ...     neg_def = all(e < 0 for e in G.eigenvalues())
# ...     if not (symmetric and neg_def):
# ...         print(f"Failed for {typ}")
# # (No output means all passed - finite types have negative definite Gram matrices)
```
