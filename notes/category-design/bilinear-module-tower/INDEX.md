...
| `bilinear_module_morphisms.md`, `categories/bilinear_forms/bilinear_module_morphisms.md` | The morphism hierarchy $\mathrm{PrimEmb} \subseteq \mathrm{Emb} \subseteq \mathrm{Hom} \subseteq \mathrm{Hom}_R$ as first-class homsets; kernels, cokernels and images with their restricted forms; the adjoint $f^{*}$; the factor by which a morphism scales the discriminant (`discriminant_multiplier`); $O(L)$. |
...
| `.../two_elementary_lattices/*.md` | 2-elementary lattices: the invariant $a$, the mod-2 quotient with its induced form, the spinor norm, the theta characteristic, the correspondence with binary codes; element order in the discriminant group, mod-2 class, the source's `companion_vector` (a $v'$ with $\langle v, v'\rangle$ odd — no such vector exists in general; see §3), mod-2 orthogonality. |
...
| Document | Mathematical content |
|---|---|
| `RMod/RMod_category.md` | $R$-modules as a category with an abelian supercategory, operator sugar for the Grothendieck ring, the homset tensor, and an optional *parameterised* symmetric-monoidal axiom that takes the product and unit as data. |
...
| `BilRMod/BilRMod_homs.md` | A form-preserving homset with checked construction; orthogonal, symplectic and isomorphism classes of transformation; kernel, image and cokernel with their forms. |
...
| `RMod/RBilForms/RBilForms_homs.md` | The homset hierarchy $\mathrm{PrimEmb} \le \mathrm{Emb} \le \mathrm{Hom} \le \mathrm{Hom}_R$ as first-class parents; the adjoint morphism; the coimage and canonical factorization; the factor by which a morphism scales the discriminant. |
...
`coxeter_diagrams.sage:194` defines `FiniteCoxeterDiagram`, which holds an
...
(radical rank, radical complement, whether the radical splits off as an
orthogonal direct summand, stabiliser of the radical; and for the parabolic case the null root, the level, the height
