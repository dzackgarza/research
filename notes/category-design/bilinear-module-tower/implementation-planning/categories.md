<!--
Origin: gitclones/Coxeter/implementation/planning/categories.md
...
These assertions verify the mathematical properties of each category level using SageMath's existing functionality:

```python
# Mathematical assertion 1: Elliptic types have all negative eigenvalues
# Using our negative convention where elliptic = negative definite
# sage: R = RootSystem(['E', 6])
# sage: L = R.root_lattice()
# sage: G = L.gram_matrix()
# sage: eigs = G.eigenvalues()
# sage: all(e < 0 for e in eigs)
# True

# Mathematical assertion 2: Parabolic types have exactly one zero eigenvalue
# These are the affine/extended Dynkin types
# sage: R = RootSystem(['A', 2, 1])  # Affine A2
# sage: L = R.root_lattice()
# sage: G = L.gram_matrix()
# sage: eigs = G.eigenvalues()
# sage: eigs.count(0) == 1 and all(e <= 0 for e in eigs)
# True

# Mathematical assertion 3: Hyperbolic types have exactly one positive eigenvalue
# Example using a simple hyperbolic construction
# sage: H = matrix(ZZ, [[2, -1, 0], [-1, 2, -1], [0, -1, 1]])  # Hyperbolic example
# sage: eigs = H.eigenvalues()
# sage: pos_eigs = [e for e in eigs if e > 0]
# sage: len(pos_eigs) == 1 and len([e for e in eigs if e < 0]) == 2
# True

# Mathematical assertion 4: Coxeter lattices have symmetric Gram matrices
...
# Mathematical assertion 5: Two-elementary lattices have discriminant groups (Z/2Z)^a
# Example: The even unimodular lattice E8 has trivial discriminant
# sage: R = RootSystem(['E', 8])
# sage: L = R.root_lattice() 
# sage: det(L.gram_matrix())
# 1
# sage: # Discriminant 1 means discriminant group is trivial = (Z/2Z)^0

# Mathematical assertion 6: Category containment for definite lattices
# Negative definite implies definite (no mixed signs)
# sage: R = RootSystem(['D', 4])
# sage: L = R.root_lattice()
# sage: G = L.gram_matrix()
# sage: eigs = G.eigenvalues()
# sage: all(e < 0 for e in eigs)  # negative definite
# True
# sage: # Therefore it's definite (all same sign)
```
