<!--
Origin: gitclones/Coxeter/research/explorations/implementation-notes/misc/ALGORITHMS.md
...
```python
# Mathematical assertion from Conway & Sloane:
# sage: R = RootSystem(['E', 8])
# sage: L = R.root_lattice()
# sage: simple_roots = L.simple_roots()
# sage: len([v for v in L.roots() if v.norm_squared() == 2])
# 240  # E8 has exactly 240 roots of norm 2
# sage: L.gram_matrix().eigenvalues()
# [2, 2, 2, 2, 2, 2, 2, 0]  # E8 lattice is positive semidefinite
```
...
```python
# Mathematical assertion for A3 and D4:
# sage: R_A3 = RootSystem(['A', 3])
# sage: L_A3 = R_A3.root_lattice()
# sage: len(L_A3.roots())
# 12  # A3 has 2 * 3 * 2 = 12 roots (Bourbaki)
# sage: R_D4 = RootSystem(['D', 4])
# sage: L_D4 = R_D4.root_lattice()
# sage: len(L_D4.roots())
# 24  # D4 has 24 roots = 2^3 * 3
```
...
```python
# Mathematical assertion for automorphism groups:
# sage: R = RootSystem(['A', 2])
# sage: L = R.root_lattice()
# sage: W = R.weyl_group()
# sage: W.order()
# 6  # A2 Weyl group is S3 with order 6 (Humphreys)
# sage: R_D4 = RootSystem(['D', 4])
# sage: W_D4 = R_D4.weyl_group()
# sage: W_D4.order()
# 192  # D4 Weyl group order = 2^3 * 4! (triality)
```
...
```python
# Mathematical assertion for theta series convergence:
# sage: R = RootSystem(['A', 2])
# sage: L = R.root_lattice()
# sage: gram = L.gram_matrix()
# sage: gram.is_negative_definite()
# True  # A2 is negative definite in our convention
# sage: # Theta series converges for definite lattices
# sage: # θ(q) = ∑_{v∈L} q^{v·v} converges when |q| < 1
```
...
```python
# Mathematical assertion for root system classification:
# sage: cm = CartanMatrix([[2, -1, -1], [-1, 2, -1], [-1, -1, 2]])
# sage: R = RootSystem(cm)
# sage: R.cartan_type()
# ['A', 3]  # Correctly identifies A3 type
# sage: R_E8 = RootSystem(['E', 8])
# sage: R_E8.rank()
# 8  # E8 has rank 8
# sage: R_E8.root_system().is_simply_laced()
# True  # E8 is simply laced (all roots same length)
```
...
```python
# Mathematical assertion for Weyl group orders:
# sage: W_A3 = WeylGroup(['A', 3])
# sage: W_A3.order()
# 24  # A3 Weyl group is S4 with order 4! = 24
# sage: W_E8 = WeylGroup(['E', 8])
# sage: W_E8.order()
# 696729600  # E8 Weyl group order from Conway & Sloane
# sage: W_E8.order() == 2^14 * 3^5 * 5^2 * 7
# True  # E8 order factorization
```
