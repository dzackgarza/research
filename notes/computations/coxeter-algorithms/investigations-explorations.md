<!--
Origin: gitclones/Coxeter/research/explorations/implementation-notes/misc/INVESTIGATIONS.md
...
### Investigation 1: Hyperbolic Plane U(1,1) Bilinear Form
```python
# Investigation: Canonical hyperbolic plane properties
# sage: U = IntegralLattice([[0, 1], [1, 0]])  # Hyperbolic plane
# sage: U.signature_pair()
# (1, 1)  # One positive, one negative eigenvalue
# sage: U.gram_matrix().eigenvalues()
# [-1, 1]  # Characteristic of indefinite form
```

### Investigation 2: Coxeter Element Spectral Properties (Type A)
```python
# Investigation: Coxeter elements have order equal to Coxeter number
# sage: W = WeylGroup(['A', 3])  # Type A_3 Weyl group
# sage: c = W.coxeter_element()
# sage: c.order() == W.coxeter_number()
# True  # Order equals h = n+1 for type A_n
```

### Investigation 3: Indefinite Lattice Vector Enumeration
```python
# Investigation: QuadraticForm methods for indefinite lattices
# sage: Q = QuadraticForm(ZZ, 2, [0, 1, 0])  # x*y form
# sage: len(Q.find_reps(1))  # Vectors with norm 1
# 4  # {(1,1), (-1,-1), (1,-1), (-1,1)}
```

### Investigation 4: Root System Height Ordering
```python
# Investigation: Poset structure on root lattices
# sage: R = RootSystem(['B', 2]).root_lattice()
# sage: alpha = R.simple_roots()
# sage: beta = alpha[1] + alpha[2]  # Sum of simple roots
# sage: R.root_poset().covers(alpha[1], beta)
# True  # beta covers alpha[1] in height ordering
```