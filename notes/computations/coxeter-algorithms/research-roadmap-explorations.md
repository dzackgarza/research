<!--
Origin: gitclones/Coxeter/research/explorations/implementation-notes/misc/TODO.md
...
```python
# Test: Verify lattice over number field construction works
# sage: K.<phi> = NumberField(x^2 - x - 1)  # Golden ratio field for H3
# sage: L = IntegralLattice(matrix(K, [[2, -phi], [-phi, 2]]))
# sage: L.base_ring()
# Maximal Order in Number Field in phi with defining polynomial x^2 - x - 1
```
...
```python
# Test: Verify theta series computation for positive definite lattice
# sage: L = IntegralLattice(matrix([[2, -1], [-1, 2]]))  # A2 lattice
# sage: L.theta_series(10)  # First 10 coefficients
# 1 + 6*q^2 + 6*q^4 + 6*q^6 + 12*q^8 + O(q^10)
```
...
```python
# Test: Verify coercion between root/weight lattices works
# sage: R = RootSystem(['C', 3])
# sage: RL = R.root_lattice()
# sage: WL = R.weight_lattice()
# sage: alpha = RL.simple_root(1)
# sage: WL(alpha)  # Coercion should work
# Lambda[1] - Lambda[2]
```
...
```python
# Test: Verify non-crystallographic Coxeter matrix construction
# sage: phi = (1 + sqrt(5))/2  # Golden ratio
# sage: M_H3 = matrix([[1, 3, 2], [3, 1, 5], [2, 5, 1]])  # H3 Coxeter matrix
# sage: G_H3 = matrix([[-2, phi, 0], [phi, -2, 1], [0, 1, -2]])  # H3 Gram matrix
# sage: G_H3.base_ring()
