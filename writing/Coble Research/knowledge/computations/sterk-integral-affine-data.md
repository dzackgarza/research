...
**Provenance.** `/home/dzack/gitclones/diss/100-corpus/100-data/dzg-research/computational-research/canonical/notebooks/Sterk IAS Plotting.ipynb`. This is the only place in the corpus with concrete integral-affine output. Absent from the dissertation, which treats integral-affine structures theoretically without tabulating edge lengths.
...
Four consecutive "project-up" cells fail as saved, on `assert all_As.rank() == 10` while the printed rank is 11.
This is an unresolved discrepancy, not a transient error, and it blocks the Sterk 4 and Sterk 5 plots.
