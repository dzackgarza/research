---
title: A_n root lattice
tags:
  - subject/algebraic-geometry
  - subject/lattice-theory
  - type/example
aliases:
  - A_n
created: 2026-05-08
---

> [!example] $A_n$ root lattice
> Let $\mathbf{Z}^{n+1}$ denote the standard diagonal lattice $\langle 1,1,\dots, 1 \rangle$ with generators $\{e_1,\dots, e_{n+1}\}$. 
> 
> The [[Root lattice|root lattice]] $A_n$ is isometric to a root lattice in $v_{n+1}^{\perp \mathbf{Z}^{n+1}}$ where $v_{n+1} \mathrel{\mathop:}= \sum_{i=1}^{n+1} e_i$, and thus concretely identified as a primitively embedded sublattice
> $$ A_n \hookrightarrow \{x\in \mathbf{Z}^{n+1} \mathrel{\Big|} \sum_{i=1}^{n+1} x_i = 0 \} $$


## Properties
- The roots $R(A_n)$ consist of $r_{i,j} \mathrel{\mathop:}= e_i - e_j$ for $1\leq i \neq j \leq n+1$.
- There are $n(n+1)$ roots in total.
- The $n$ simple roots $\Phi(A_n)$ can be chosen as $\alpha_i \mathrel{\mathop:}= e_i - e_{i+1}$ for $1\leq i \leq n$.
