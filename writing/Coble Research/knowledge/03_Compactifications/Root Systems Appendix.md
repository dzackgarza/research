---
title: Root Systems Appendix
tags:
  - subject/algebraic-geometry
  - subject/representation-theory
  - type/appendix
aliases:
  - Root Systems
  - Coxeter Diagrams
created: 2026-05-08
---

> [!definition] Root Systems and Conventions Bourbaki conventions for simply-laced (A, D, E) and non-simply-laced (B, C, F, G) root systems.
> The Coxeter diagrams and Gram matrices are derived from these conventions.
> - $A_n(-1)$
> - $B_n(-2)$
> - $C_n(-1)$
> - $D_n(-1)$
> - $E_n(-1)$ for $n \in \{6, 7, 8\}$
> - $F_4(-2)$
> - $G_2(-1)$
> 
> The tables summarize ranks, root counts, Weyl groups, discriminant groups, and explicit basis constructions for Euclidean embeddings.
