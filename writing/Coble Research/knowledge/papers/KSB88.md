...
Note the relationship $\bar{K}_Z^2 = K_Z^2 + K_{\mathrm{rel},Z}^2$.

(iv) If $X_y, y \in Y$, is a family of surfaces we shall write $K_y^2$ instead of $K_{\bar{X}_y}^2$ (similarly $\bar{K}_y^2, K_{\mathrm{rel},y}^2$) if no confusion is likely.
...
Let $V \subset \mathbb{P}^5$ be the Veronese surface and $C_V \subset \mathbb{P}_6$ the cone over it.
Let $F \subset \mathbb{P}^5$ be the scroll $\operatorname{Proj}_{\mathbb{P}^1}(\mathcal{O}(2) + \mathcal{O}(2))$ embedded with the relative $\mathcal{O}(1)$ , and $C_F \subset \mathbb{P}^6$ be the cone over it.
...
Since $E$ is effective and exceptional with respect to $\bar{X}_0 \to X_0$, and $n^* K_{\bar{X}}$ is relatively nef the last two terms are non-negative.
...
This in turn gives that $K_{\bar{X}_0} = h^{*}K_{\bar{X}_0}$; thus $\bar{X}_0$ has DuVal singularities only.
...
This latter is a polynomial in $\bar{K}_y^2$, $\bar{K}_y \cdot r^* H = K_y \cdot H$ and $\chi(\mathcal{O}_{\bar{X}_y})$.
...
For a generic point $t$ we have $\mathcal{O}(mK_{X / D})\otimes \mathcal{O}_{X_t}\cong \mathcal{O}(mK_{X_t})$, and for 0 we still have an injection $\mathcal{O}(mK_{X / D})\otimes \mathcal{O}_{X_0}\to \mathcal{O}(mK_{X_0})$ which is an isomorphism outside the non-Gorenstein locus.
Therefore,
...
Then the pairs $(E_i, \bar{V}_i)$ are also homeomorphic.
In particular $K_{\mathrm{rel}, V_i}^2$ is independent of $i$.
...
By 2.3 (iii) $\bar{K}_0^2 = K_0^2 + K_{\mathrm{rel},0}^2$ and $\bar{K}_t^2 = K_t^2 + K_{\mathrm{rel},t}^2$.
...
We proceed as in 2.17. For each $y \in Y$ let $r_y \colon \bar{X}_y \to X_y$ be the DuVal resolution of the singularity $p(y)$.
If $E_{i,y}$ are the exceptional curves on $\bar{X}_y$, then for some $a_i \in \mathbb{Q}$ we have $E_{j,y} \cdot \sum a_i E_{i,y} = E_{j,y} \cdot \bar{K}_y$ for every $j$.
...
(iii) If $(z,Z)$ is an isolated surface singularity, $\bar{Z}$ the normalization, and $r\colon Z^{\prime}\to Z$ a resolution, then both $R^{1}r_{*}\mathcal{O}_{Z}$ and $\mathcal{O}_{\bar{Z}} / \mathcal{O}_Z$ are of finite length.
Let $h_z$ denote the difference of these lengths.
One can prove that in a flat family $K_{\mathrm{rel},z}^{2}$ constant implies that $h_z$ is constant.
...
**Proof.** For any $n$ there is a natural map $\omega_{X / S}^{[m]} \otimes_{\mathcal{E}_S} k \to \omega_{X_0}^{[n]}$, where $k$ is the residue field of $S$, which is an isomorphism in codimension one.
...
By what we have just shown, we have $\omega_{X_0^n}^{\otimes r} / S_n \cong \mathcal{O}_{X_0^n}$ for every $n$; it follows that $\omega_{X^0 / S}^{\otimes n} \cong \mathcal{O}_{X^0}$, and the Lemma is proved.
...
**Lemma 3.18.** In these circumstances, there is a natural morphism $\operatorname{Fix} G \to (\operatorname{Def}^r X_0)_{\mathrm{red}}$ that is $1 - 1$.
In particular, $(\operatorname{Def}^r X_0)_{\mathrm{red}}$ is irreducible and has smooth normalization.
...
One of the reasons is that if $f \colon Y \to X$ is a desingularization of a normal surface, then $f_{*} \mathcal{C}_{Y} = \mathcal{C}_{X}$.
...
It is called a minimal good semi-resolution if for every $-1$ curve $E_i$, contracted by $f$, we have $\bar{E}_i \cdot \sum_{j \neq i} \bar{E}_j + \bar{E}_i \cdot \bar{D}_Y &gt; 2$.
...
(i) There is a unique cycle $Z = -\sum a_i E_i$ such that $\bar{Z} \cdot \bar{E}_i = -\deg_{E_i} \omega_Y$.
...
Proof.
The intersection matrix $\bar{E}_i \cdot \bar{E}_j$ is negative definite; this shows the existence of $Z$.
If $f$ is minimal, then $\deg_{E_i} \omega_Y \geq 0$, and (ii) is a formal consequence of this as in [Z, 7.1]

Assume that $a_i = 0$ for some $i$.
Then $\bar{Z} \cdot \bar{E}_i \geq 0$, with equality only if $a_j = 0$ for every $E_j$ such that $\bar{E}_j \cdot \bar{E}_i &gt; 0$.
If $f$ is minimal, then $-\deg_{E_i} \omega_Y \leq 0$; hence we have $\deg_{E_i} \omega_Y = 0$.
...
(ii) The action of $\mathbb{Z}_r$ on $\omega_X / m_x\omega_X$ is faithful ($m_{x} =$ the ideal of $x\in X$)
...
Therefore we may assume for the rest of the proof that the $\mathbb{Z}_2$-action on any component of $E$ is faithful.
Therefore, we obtain that $K_{BY} = h^{*}K_{Y} + F$.
