...
The group $W(\Gamma_\eta) \leq \Orth(\bdlattice{T}{\eta})$ generated by reflections in roots stabilized by $\Gamma_\eta$ defines the stable reflection group, and the associated Coxeter chamber $\thecone{C}(\Gamma_\eta) \subset \bdlattice{T}{\eta, \RR}$ is determined by the set of simple roots for $W(\Gamma_\eta)$ (see \cite{AEGS25}).
...
The full polygon $B(\lambda)$ is then obtained by gluing together two copies of this polygon along the blue equator, where a representation of the doubled polygon is depicted in \cite[Fig.~13]{AEGS25}. Taking the star at each vertex yields a semitoric surface.
:::
