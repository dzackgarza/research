...
This mandates working with "half-divisor" models (\Cref{half-divisor-model}) and searching for $\IAS^2$s invariant under a group generated by two commuting involutions, one corresponding to $\idp$, corresponding to K3 surfaces $X$ in $\fttz$, and another fixed-point-free involution corresponding to $\ien$, coming from the embedded locus $\fent$, those $X$ arising as K3 covers $X\to Z$ of Enriques surfaces.
