...
Let $T$ be an even nondegenerate lattice of signature $(2,n)$ and $\Gamma \leq \OStab(T)$ a finite index arithmetic subgroup. As we have seen and will explore in more detail in \Cref{chapter-4}, the quotient $\FG \da \dmodgamma{ \halfpd{T} }{\Gamma}$ admits a Baily–Borel compactification $\bbcpt{\FG}$.
...
We note that a sufficient condition for integrality is that $2(v, L) \subseteq k \ZZ$.
...
The **Weyl chambers** are the connected components of $\thecone{C}_L^{\circ} \da  \thecone{C}_L \setminus \Delta(L)$ where $\thecone{C}_L$ is the cone of positive-norm vectors in $L$. A **simple system** is a set of roots $v$ such that the mirrors $H_v$ form the bounding hyperplanes for a fundamental domain for the action of $W(L)$ on $L$.
:::
...
Let $v \in L$ be a simple root, the the reflection $s_v$ is in $\Orth(L)$ precisely when $2(v, L)/v^2 \subseteq \ZZ$.
...
$$
\thecone{C}_L^\circ \da  \thecone{C}_L^+ \setminus \bigcup_{v \in \Phi(L)} H_v,
$$
...
- a multiple edge if $3 \leq m_{i, j} < \infty$, so $H_i \cap H_j$ is in the interior of $\HH^n$ and $g_{i,j} < 1$,
- a thick/bold edge if $H_i \cap H_j \in \bd \HH^n$ and $g_{i,j} = 1$,
...
We refer to \cite{Vin72, Vin75} for thorough treatments.
...
Toward carrying out the program proposed above, let $\Gamma \subset \Orth(L)$ be an arithmetic reflection subgroup generated by reflections in simple roots. The following algorithm due to \cite{Vin75} constructs a fundamental chamber $P \subset \HH^n$:
