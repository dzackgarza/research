...
With a transcendental lattice identified, because these lattices have very particular signatures, we are placed in a setting where two combinatorial compactifications are accessible: the Baily Borel compactification of \cite{BB66}, and the semitoroidal compactifications of \cite{Loo85}, which simultaneously generalize both the Baily Borel and toroidal compactifications.
...
To finally attain a comparison with KSBA compactifiations, we then leverage the main result of \cite{AE23}, that the (normalization) of the KSBA compactification of stable K3 pairs $(X, \eps R)$ for a **recognizable** divisor $R$ (see \Cref{chapter-4} and \Cref{chapter-5}) is isomorphic to a semitoroidal compactification.
Then \Cref{main-theorem} follows from \cite[Thm. 3.26]{AEH24}, which shows that certain ramification divisors are recognizable.
...
The approach we take in \cite{AEGS25} largely bypasses many of these computational difficulties, in favor of more easily constructible divisorial log terminal (dlt) models which can be described in the finitary data of an integral affine 2-sphere, which we refer to as an $\IAS^2$ throughout this work.

These folding and involution-based methods recover the work of \cite{Ste91}, while the more general theory established in
\cite{AET23, AEH24, AE22, AET23, AE23, ABE22} has been shown to recover results from e.g. \cite{Sca87} and others.
