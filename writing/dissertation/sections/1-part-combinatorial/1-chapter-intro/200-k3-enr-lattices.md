...
,$$
where $U$ is the hyperbolic plane (\cref{def:hyperbolic-plane}) and $E_8$ is the negative-definite $E_8$ root lattice (see \cref{ex:root-lattices}).
...
By the classification theory for Enriques surfaces (see \cite[Cor. 1.2.3]{CD89}), $\omega_Z$ is a generator of the 2-torsion in $\Pic(Z)$.
...
,$$
the unique even unimodular lattice up to isometry (see \Cref{unimodular-lattices}), where $U$ and $E_8$ are as above.
...
The computation of $\ten$ can be found, for example, in \cite{Ste91}, although it was known earlier. We note that when $X$ is a K3 surface that admits an Enriques involution $\ien$, letting $\Ien$ denote the induced isometry on $H^2(X; \ZZ)$, we obtain
$$
\sen \oplus \ten \iso \lkt^{\Ien = 1} \oplus \lkt^{\Ien=-1} \injects \lkt
,$$
realizing these as the invariant and coinvariant lattices respectively.\footnote{We note that there are mixed conventions in the literature regarding whether $S$ or $T$ should be the invariant sublattice -- we choose here the convention used in \cite{AEGS25}.}
...
where the triples $(r, a, \delta)_{n_+}$ are the invariants shown by \cite{Nik80} to classify 2-elementary lattices $T$ which admit a primitive embedding $T\injects \lkt$.
