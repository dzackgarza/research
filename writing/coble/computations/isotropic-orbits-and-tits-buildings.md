...
The enumeration of $\Orth(T)$- and $\Gamma$-orbits of primitive isotropic vectors in $T_\Co$ is left open in [[open-problems]], where it blocks the uniqueness of the Baily--Borel $0$-cusp.
...
$$
\Orth(L)\longrightarrow\Orth(q_L)
$$
is surjective.
This is the same surjectivity that Nikulin's criteria supply for a $2$-elementary lattice with $r > a$ [@Nik80], and it is the hypothesis under which [[open-problems]] reduces the $T_\Co$ orbit problem to $A_{T_\Co}$.
...
The paper's conclusion is $v_1\sim v_2$ under the group it denotes $\widehat{\Orth}^+(L)$.

For Example 2.6, $v_1 = (1,-1,0,0,0)$ and $v_2 = (1,0,1,0,0)$, and both have $v^2 = -2$.
The paper's conclusion is $v_1\sim v_2$ under the group it denotes $\widehat{S\Orth}^+(L)$, and the in-tree computation reproduces it.
:::
...
1. representatives for the intersection $G_1\cap G_2$ of two subgroups of $\GL_n(\ZZ)$;
...
### The lattice of that computation is $T_{\dP}$
...
This is the lattice $L_{20,2,0} = T_{\dP}$ of two-elementary type $(20,2,0)$ in which the root configuration $\Phi_{(18,2,0)}$ and the folded Sterk diagrams are written ([[root-vectors-and-folded-sterk-diagrams]]), and in which the embedding chain $T_\Co\injects T_\En\injects T_{\dP}$ terminates ([[coble-lattice-isotropic-candidates]]).
...
Step 6 of the recipe in [[computational-toolchain-and-recipe]] invokes `INDEF_FORM_GetOrbitRepresentative` from the C++ library `polyhedral_common` of the same authors, through GAP.
...
The candidate isotropic vectors of $T_\Co$ and the three-way transport of each along $T_\Co\injects T_\En\injects T_{\dP}$ are recorded in [[coble-lattice-isotropic-candidates]]; running the orbit solver on them is the step that would replace the vector-by-vector table with a count.
:::

Related: [[scattone-cusp-method]], [[coble-lattice-isotropic-candidates]], [[computational-toolchain-and-recipe]], [[cusp-correspondence-morphism-chain]], [[open-problems]].
