...
For $d = 1$ the classification uses primitive embeddings into the $24$ Niemeier lattices to distinguish the four types of rank-$17$ boundary lattices; see \longref{rmk:niemeier-enumeration} and \longref{ex:scattone-f2-diagram}.
:::
...
::: {.Theorem #thm:sterk-orbit}
### Orbit classification (Sterk 1991)
...
::: {.Example #ex:scattone-f2-diagram}
### Cusp diagram of $F_2$ (Scattone)
...
\longref{ex:scattone-f2-diagram} is the cusp diagram of the *degree-two K3* moduli space $F_2$, and is distinct from the Coble and Enriques cusp diagrams computed in the Cusp Correspondence section: there the transcendental lattice $T_\Co = (11,11,1)_1$ produces a single $0$-cusp incident to a single $1$-cusp via the mirror-move algorithm, with divisibility data $(2,2)$.
