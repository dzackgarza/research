...
::: {.Lemma #lem:bilinear-quadratic-correspondence}
...
::: {.Definition #def:discriminant-forms}
...
One has $\mathfrak{n}(L)\subseteq\mathfrak{s}(L)$, and $L$ is integral exactly
when $\mathfrak{s}(L)\subseteq\ZZ$.
Under the twist of the Lattice Theory section,
$\mathfrak{s}(L(m)) = m\,\mathfrak{s}(L)$,
$\mathfrak{n}(L(m)) = m\,\mathfrak{n}(L)$ and
$\mathfrak{v}(L(m)) = m^{r}\,\mathfrak{v}(L)$ for $r = \operatorname{rank}(L)$.
:::
...
Then $L_{\ZZ_p} = L\otimes_\ZZ\ZZ_p$ admits an orthogonal decomposition
$$
...
$p^{s_i}$ and the ranks $\operatorname{rank}(L_i)$ are invariants of $L$ at $p$.
:::

::: {.Remark}

The Jordan invariants at every prime are exactly the data compared in
\longref{def:genus}: two lattices lie in the same genus precisely when they have
...
$2$-adic lattices $V_k$ and $U_k$ of \longref{def:nikulin-Vk-Uk} together with
rank-one summands.
:::
...
$h = \abs{\operatorname{cl}(L)}$ in the notation of \longref{def:genus}.
...
\longref{thm:jordan-decomposition}; the explicit unimodular cases are tabulated in
[@CS10 Ch. 16].
Since each summand of \longref{def:mass} is positive, the formula gives a criterion
...
$$
\Orth(H)\longrightarrow\Orth(A_H, q_H)
$$
...
of \longref{def:genus}; it is an invariant of the discriminant form alone.
...
Under the hypothesis $U^{\oplus 2}\subseteq L$ the Eichler criterion
(\longref{thm:eichler-criterion}) makes it effective, bounding the number of
