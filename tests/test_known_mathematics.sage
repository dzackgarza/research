...
# summed against the framing.  ``zipsum`` is the one place that happens.
...
# ---------------------------------------------------------------------------
# The Bogachev--Kolpakov ternary Lorentzian lattices (arXiv:2112.14642v4,
# sec. 6.1 "Non-reflective lattice with roots" and sec. 6.2 "A lattice without
# roots"; the second example was communicated to the authors by Gael Collinet).
# In-tree extract, with the source text and the derived remarks:
# tests/coxeter_tdd_specs/literature/papers/bogachev_kolpakov_thin_hyperbolic_2024.md
#
# The paper prints the QUADRATIC FORM f(x) = (x,x) and works in signature
# (2,1).  Its Gram matrix halves the printed cross coefficients, and this
# repository's negative-definite convention negates the whole form, so the
# catalogue specimen is the twist by -1 and has signature (1,2).  Both steps
# are asserted below rather than assumed, because both are places where a
...
# Note that x_0 occurs only in 98x_0x_2, so this is NOT the index reversal of
# the section 6.1 form -- the determinants (-7^4 against -7^6) separate them.
_BK_WITHOUT_ROOTS_GRAM = matrix(ZZ, [[0, 0, 49], [0, 49, 7], [49, 7, 3]])

# The first sixteen roots Vinberg's algorithm produces for the sec. 6.1
# lattice, verbatim as printed.
_BK_VINBERG_ROOTS = (
    (0, 7, -1),
    (-7, -11, 2),
    (0, 0, 1),
...
def _bk_root(lattice: Parent, coordinates: tuple[int, ...]) -> Element:
    r"""Return the element of ``lattice`` on these framing coordinates."""
    return zipsum(coordinates, lattice.module_generators(), lattice.zero())


def test_the_bogachev_kolpakov_gram_matrices_reproduce_the_printed_forms() -> None:
    r"""arXiv:2112.14642v4 sec. 6.1 and 6.2: the printed $f(x)=(x,x)$
    determines $G$ by $q(x)=xGx^{\mathsf T}$, so every cross coefficient is
    halved -- $14\mapsto 7$ and $98\mapsto 49$, both exact, which is why the
    Gram matrices are integral and the lattices are integral lattices.

    Asserted on the framing: $q(e_i)$ is the printed square coefficient and
    $2b(e_i,e_j)$ is the printed cross coefficient.  The determinants $-7^4$
    and $-7^6$ then separate the two lattices, and both forms have signature
    $(2,1)$ as printed.
    """
    first = IntegralLattice(_BK_NON_REFLECTIVE_GRAM)
    e0, e1, e2 = first.module_generators()
    assert (e0.q(), e1.q(), e2.q()) == (3, 0, 49), "square coefficients 3, 0, 49"
    assert 2 * e0.b(e1) == 14, "the printed 14 x_0 x_1"
    assert 2 * e0.b(e2) == 98, "the printed 98 x_0 x_2"
    assert 2 * e1.b(e2) == 0, "no x_1 x_2 term is printed"
    assert _BK_NON_REFLECTIVE_GRAM.det() == -7**4
    assert first.signature_pair() == (2, 1)

    second = IntegralLattice(_BK_WITHOUT_ROOTS_GRAM)
    f0, f1, f2 = second.module_generators()
    assert (f0.q(), f1.q(), f2.q()) == (0, 49, 3), "square coefficients 0, 49, 3"
    assert 2 * f0.b(f1) == 0, "no x_0 x_1 term is printed"
    assert 2 * f0.b(f2) == 98, "the printed 98 x_0 x_2"
    assert 2 * f1.b(f2) == 14, "the printed 14 x_1 x_2"
    assert _BK_WITHOUT_ROOTS_GRAM.det() == -7**6
    assert second.signature_pair() == (2, 1)


def test_the_bogachev_kolpakov_specimens_are_the_source_lattices_negated() -> None:
    r"""The catalogue holds these two lattices in this repository's
    negative-definite convention, where a rank-three hyperbolic lattice has
    signature $(1,2)$.  The relation to the paper's $(2,1)$ data is the twist
    by $-1$: $L(-1)$ has Gram $-G$, and in odd rank that flips the sign of the
    determinant while leaving the invariant factors, and hence the
    discriminant group, alone.
    """
    for specimen, source in (
        (Lattices.BogachevKolpakovNonReflective, _BK_NON_REFLECTIVE_GRAM),
        (Lattices.BogachevKolpakovWithoutRoots, _BK_WITHOUT_ROOTS_GRAM),
    ):
        assert matrix(ZZ, specimen.gram_matrix()) == -source
        assert matrix(ZZ, specimen.twist(-1).gram_matrix()) == source
        assert specimen.signature_pair() == (1, 2)
        assert specimen.twist(-1).signature_pair() == (2, 1)
        assert (
            matrix(ZZ, specimen.gram_matrix()).elementary_divisors()
            == source.elementary_divisors()
        ), "twisting by -1 does not move the invariant factors"


def test_vinbergs_criterion_bounds_the_root_lengths_of_both_specimens() -> None:
    r"""arXiv:2112.14642v4 sec. 6.2, citing Vinberg: for a root $r$, $f(r)$
    divides twice the last invariant factor of the Gram matrix.

    For the sec. 6.1 lattice the invariant factors are $1,49,49$, so every
    root length divides $98$; for the sec. 6.2 lattice they are $1,49,2401$,
    so every root length divides $4802=2\cdot 7^4$ -- the bound the paper
    names, and the reason "$N$ has no roots" is a finite check rather than an
    unbounded search.  The four candidate lengths the paper is left with,
    $49$, $98$, $2401$ and $4802$, are among the divisors.
    """
    non_reflective = Lattices.BogachevKolpakovNonReflective
    assert matrix(ZZ, non_reflective.gram_matrix()).elementary_divisors() == [1, 49, 49]
    assert tuple(non_reflective.possible_root_lengths()) == (1, 2, 7, 14, 49, 98)

    without_roots = Lattices.BogachevKolpakovWithoutRoots
    assert matrix(ZZ, without_roots.gram_matrix()).elementary_divisors() == [1, 49, 2401]
    lengths = without_roots.possible_root_lengths()
    assert 2 * 2401 == 4802, "twice the last invariant factor"
    assert tuple(lengths) == tuple(ZZ(4802).divisors())
    for candidate in (49, 98, 2401, 4802):
        assert candidate in lengths, "a candidate length the paper still had to exclude"


def test_the_sixteen_printed_vinberg_roots_are_roots_of_the_lattice() -> None:
    r"""arXiv:2112.14642v4 sec. 6.1: the sixteen listed vectors are roots.

    Being a root is the crystallographic condition $2b(e_i,r)/q(r)\in\mathbb Z$
    for every framing generator -- exactly what ``reflection`` asserts before
    it builds $s_r$ -- so constructing the reflection *is* the check, and the
    reflection is an involution of $O(L)$.  Each length divides twice the last
    invariant factor, as Vinberg's criterion requires.

    Two lengths occur: $q(r)=49$ for fourteen of the roots and $q(r)=98$ for
    $v_{10}$ and $v_{16}$ (signs negated here, since the specimen is the
    negative-definite twist).  Derived here, not stated in the paper, and worth
    recording: the correspondence sec. 6.1 calls "the involution
    $v_5\to v_{10}$, $v_{14}\to v_{16}$" sends the two length-$49$ roots
    $v_5,v_{14}$ to the two length-$98$ roots, so it is not induced by an
    isometry of $L$, which preserves lengths.  What "symmetry of the reflective
    part" means there needs the paper's fuller context and is not asserted.
    """
    lattice = Lattices.BogachevKolpakovNonReflective
    lengths = lattice.possible_root_lengths()
    observed = []
    for coordinates in _BK_VINBERG_ROOTS:
        root = _bk_root(lattice, coordinates)
        assert abs(root.q()) in lengths, f"Vinberg's criterion bounds q({coordinates})"
        reflection = lattice.reflection(root)
        assert reflection.then(reflection).is_identity(), (
            f"s_r is an involution, r={coordinates}"
        )
        observed.append(root.q())
    assert set(observed) == {-49, -98}, "the printed roots have two lengths"
    assert observed.count(-98) == 2, "v_10 and v_16 are the long ones"
    assert (observed[9], observed[15]) == (-98, -98)


def test_the_bogachev_kolpakov_lattice_with_roots_is_not_reflective() -> None:
    r"""arXiv:2112.14642v4 sec. 6.1, verbatim: "Then $L$ is non-reflective."

    That is, the subgroup of $O(L)$ generated by the reflections in roots has
    infinite index, so the fundamental Coxeter polyhedron of that subgroup has
    infinite volume.  ``is_reflective`` is the owned name for the question and
    is currently a stated contract with no implementation, so this row is the
    gap map for that surface: the mathematics is settled in the literature and
    the preamble does not yet answer it.
    """
    assert Lattices.BogachevKolpakovNonReflective.is_reflective() is False


def test_the_root_length_49_case_is_excluded_by_a_local_obstruction() -> None:
    r"""arXiv:2112.14642v4 sec. 6.2: for $f(r)=49$ the crystallographic
    conditions force $k_1=m_1$, $k_2=7m_2-3m_3$, $k_3=7m_3$, giving

    .. MATH::

        q(r) = f(r)/49 = 49m_2^2 + 14m_1m_3 - 28m_2m_3 + 6m_3^2,

    and the paper concludes "However, $q$ does not integrally represent $1$."

    The certificate is derived here, not printed in the paper, so that the
    claim is checked rather than quoted: modulo $7$ the first three
    coefficients vanish and $q\equiv 6m_3^2$.  The squares modulo $7$ are
    $\{0,1,2,4\}$, so $q$ takes only the values $\{0,3,5,6\}$ and never $1$ --
    $q$ fails to represent $1$ already over $\mathbb Z_7$, hence over
    $\mathbb Z$.  The remaining candidate lengths $98$, $2401$ and $4802$ are
    excluded by analogous arguments in the paper and are not reproduced here.
    """
    residues = {
    ...
    assert residues == {0, 3, 5, 6}, "q is 6 m_3^2 modulo 7"
    assert 1 not in residues, "q does not represent 1 over ZZ_7, so not over ZZ"


def test_vinberg_enumeration_separates_the_two_bogachev_kolpakov_lattices() -> None:
    r"""arXiv:2112.14642v4 sec. 6.1 against sec. 6.2, on the owned root
    enumeration: the same bounded search finds roots in $L$ and none in $N$.

    The bound is what makes the pair meaningful.  An empty return under a bound
    does not by itself prove $N$ has no roots -- that proof is the divisibility
    argument asserted above -- but the sec. 6.1 lattice is the positive
    control: a search that returned nothing for both would be answering the
    wrong question, and here it does not.

    Incompleteness under the bound is expected and is what the enumeration
    warns about, so the warning is caught rather than left to fail the row.
    """
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", RuntimeWarning)
        with_roots = Lattices.BogachevKolpakovNonReflective.vinberg_algorithm(
            max_decompositions=200
        )
        without_roots = Lattices.BogachevKolpakovWithoutRoots.vinberg_algorithm(
            max_decompositions=200
        )
    assert with_roots, "sec. 6.1: Vinberg's algorithm does produce roots"
    assert not without_roots, "sec. 6.2: N has no roots at all"
