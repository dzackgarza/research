...
def test_cardinal_arithmetic_and_order_do_not_assume_continuum_hypothesis() -> None:
    ...
    assert cardinals.hom(aleph(1), continuum).cardinality() == 1
    assert cardinals.hom(continuum, aleph(1)).cardinality() == 0
    assert cardinals.compare(aleph(2), continuum) is CardinalComparison.INCOMPARABLE
    assert cardinals.hom(aleph(2), continuum).cardinality() == 0
    assert cardinals.hom(continuum, aleph(2)).cardinality() == 0
    assert cardinals.hom(aleph0, continuum).unique_morphism().domain() == aleph0
