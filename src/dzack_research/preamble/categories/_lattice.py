...
def _gram_is_nondegenerate(gram, ring):
    r"""Whether the correlation \(M\to M^\vee\) of the presented form is injective.

    At finite rank it is decided by \(\det G\neq 0\); at infinite rank the
    pairing rule decides it or answers ``Unknown``.
    """
    match _gram_rank(gram).is_finite():
        case True:
            return bool(_gram_determinant(gram, ring) != ring.zero())
        case False:
...
class _PairingGram(ModuleElement, Tensor):
    ...
    def signature_pair(self):
        ...
        assert _gram_rank(self).is_finite(), (
            "this pairing rule states no signature at infinite rank"
        )
    ...
    def dual_gram_on(self, dual_module):
        r"""The rule of the metric dual form on ``dual_module``, for the rules that state one."""
        assert False, (
            f"the metric dual of the infinite-rank pairing rule {self._pairing_name()} "
            "is materialized for diagonal rules with unit entries and their scalings"
        )
...
class _ScaledGram(_PairingGram):
    ...
    def is_nondegenerate(self):
        r"""\(sb\) is nondegenerate when \(s\neq 0\) and \(b\) is nondegenerate."""
        return self._scalar != 0 and self._gram.is_nondegenerate()
...
class _DiagonalGram(_PairingGram):
    ...
    def is_nondegenerate(self):
        r"""A diagonal form is nondegenerate when every diagonal value is nonzero."""
        zero = self.base_ring().zero()
        return self._default != zero and all(value != zero for value in self._exceptions.values())

    def is_unimodular(self):
        r"""A diagonal form is unimodular when every diagonal value is a unit."""
        ring = self.base_ring()
        return ring(self._default).is_unit() and all(ring(value).is_unit() for value in self._exceptions.values())
    ...
    def dual_gram_on(self, dual_module):
        ...
        assert self.is_unimodular(), (
            "the metric dual of a diagonal form is materialized for unit diagonal values"
        )
        return _DiagonalGram(
            dual_module,
...
class _BiproductGram(_PairingGram):
    ...
    def is_nondegenerate(self):
        ...
        return all(block.is_nondegenerate() for block in self._blocks)

    def is_unimodular(self):
        r"""An orthogonal sum is unimodular when every summand is."""
        return all(block.is_unimodular() for block in self._blocks)

    def is_even_in(self, twice):
        r"""An orthogonal sum is even when every summand is."""
        return all(block.is_even() for block in self._blocks)
...
class _ColimitGram(_PairingGram):
    ...
    def pairings_against(self, vector):
        ...
        stage_ranking = stage.module_generating_set().ranking_map()
        return {
    ...
    def signature_pair(self):
        ...
        if large_negative == small_negative and large_positive > small_positive:
            return signature_pair(cardinal(self._module.module_rank()), large_negative)
        if large_positive == small_positive and large_negative > small_negative:
            return signature_pair(large_positive, cardinal(self._module.module_rank()))
        if large_positive > small_positive and large_negative > small_negative:
            rank = cardinal(self._module.module_rank())
            return signature_pair(rank, rank)
        return large
...
def _colimit_lattice(stage, *, category):
    ...
    return _lattice_object(category, module, _ColimitGram(module, stage))
...
def _owned_free_module(data, ring, module_generators=None, names=None):
    ...
    match module_generators is None and not positional:
        case True:
