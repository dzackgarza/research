...
def _cone_from_engine_polyhedron(lattice, polyhedron):
...
class RationalPolyhedralCones(OwnedCategory):
    ...
    def from_rays(self, lattice, rays):
        r"""Return the cone in ``lattice`` generated by the rays of the stated vectors.

        The rays are lattice vectors or their coordinates in the framing of
        ``lattice``; the half-spaces cutting the cone out are computed by the
        exact polyhedral engine.
        """
        rows = tuple(tuple(SageQQ(entry) for entry in ray) for ray in rays)
        assert rows, "a cone generated by rays needs at least one ray"
        return _cone_from_engine_polyhedron(
            lattice, Polyhedron(rays=rows, base_ring=SageQQ)
        )
