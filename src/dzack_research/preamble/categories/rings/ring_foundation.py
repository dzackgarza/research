...
def _ring_morphisms_equal(left, right) -> bool:
    r"""Decide equality of owned ring maps from exact determining data."""
    if left is right:
        return True
    ...
    if domain in FramedAlgebras(base):
        ...
        if not labels.cardinality().is_finite():
            raise NotImplementedError(
                "ring-morphism equality from a framed algebra currently requires a finite determining family"
            )
        if any(
        ...
        if structure_map.domain() is domain:
            raise NotImplementedError(
                "ring-morphism equality needs a smaller scalar source when a framed algebra is based on itself"
            )
    ...
    if left._engine_morphism is not None and right._engine_morphism is not None:
        try:
            ...
            pass
    raise NotImplementedError(
        f"ring-morphism equality from {domain} has no represented determining family"
    )
