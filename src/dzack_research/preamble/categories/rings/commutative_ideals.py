...
def _localized_commutative_ideal(source_ideal, localization_ring):
    ...
    if len(source_regular_labels) != 1 or len(target_regular_labels) != 1:
        raise ArithmeticError(
            f"cannot localize the ideal {source_ideal} along {localization_map}: the regular modules of "
            f"{source_ideal.ring()} and {localization_ring} must each be generated by one element, but they have "
            f"{len(source_regular_labels)} and {len(target_regular_labels)} module generators"
        )
    ...
    class ParentMethods:
        ...
        def ideal_generators(self):
            ...
            assert generators is not None, (
                f"the ideal {self} of {self.ring()} has lost the ideal generators it was constructed from"
            )
            return generators
        ...
        def extension_to_localization(self, localization_ring):
            ...
            if localization_ring not in LocalizationRings():
                raise TypeError(
                    f"cannot extend the ideal {self} to {localization_ring}: that ring is not a localization S^-1 R"
                )
            if localization_ring.localization_source() is not self.ring():
                raise ValueError(
                    f"cannot extend the ideal {self} of {self.ring()} to {localization_ring}: that ring is a "
                    f"localization of {localization_ring.localization_source()}, not of {self.ring()}"
                )
        ...
        def contraction_from_localization(self):
            ...
            assert source_ideal is not None, (
                f"cannot contract the ideal {self}: it was not constructed as the extension S^-1 I of an ideal I "
                "of R"
            )
        ...
        def power(self, exponent):
            ...
            if exponent < 0:
                raise ValueError(
                    f"the ideal power I^n needs n >= 0, but n = {exponent} for I = {self}"
                )
        ...
        def image_under_fraction_field_automorphism(self, morphism):
            ...
            if morphism.domain() is not field or morphism.codomain() is not field:
                raise ValueError(
                    f"cannot apply {morphism} to the ideal {self}: it must be an automorphism of the fraction "
                    f"field {field}, but it is a map {morphism.domain()} -> {morphism.codomain()}"
                )
            images = tuple(
...
def _require_same_ring(left, right):
    if left.ring() is not right.ring():
        raise ValueError(
            f"ideal arithmetic needs both ideals in one ring, but {left} is an ideal of {left.ring()} "
            f"and {right} is an ideal of {right.ring()}"
        )
...
def _cover_lifted_ideal(ideal):
    ...
    assert _realized_as_quotient(ring), (
        f"cannot compute this operation on the ideal {ideal} of {ring}: no algorithm applies to "
        f"{ring} directly, and {ring} is not a quotient P/K of a polynomial ring P"
    )
...
def _flat_extension_commutative_ideal(source_ideal, morphism):
    ...
    if morphism.domain() is not source:
        raise ValueError(
            f"cannot extend the ideal {source_ideal} along {morphism}: the map must start at "
            f"{source}, but it starts at {morphism.domain()}"
        )
    ...
    if len(labels) != len(target_generators):
        raise ArithmeticError(
            f"extending the ideal {source_ideal} along the flat map {morphism} changed the number of "
            f"generators from {len(target_generators)} to {len(labels)}"
        )
