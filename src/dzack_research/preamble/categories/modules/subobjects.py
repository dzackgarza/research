...
class ModuleSubobjects(OwnedCategoryOverBaseRing):
    ...
    class ParentMethods:
        ...
        def contains_subobject(self, other) -> bool:
            r"""Return whether ``other`` is contained in this subobject.

            A subobject of $B$ is a chosen monomorphism into $B$, and one is
            contained in another exactly when its inclusion factors through
            the other's.  Since both are generated by their module
            generators, that holds exactly when every embedded generator of
            ``other`` lies in the image of this inclusion.
            """
            if self.inclusion().codomain() is not other.inclusion().codomain():
                raise ValueError("comparing subobjects requires one common codomain")
            inclusion = self.inclusion()
            return all(
                inclusion.is_in_image(generator)
                for generator in other.embedded_module_generators()
            )

        def is_equal_subobject(self, other) -> bool:
            r"""Return whether ``other`` represents the same subobject.

            Two monomorphisms into $B$ represent one subobject exactly when
            each factors through the other.
            """
            return self.contains_subobject(other) and other.contains_subobject(self)
