...
class _SelectedFinitePresentationModules(OwnedCategoryOverBaseRing):
    ...
    class ParentMethods:
        ...
        def is_projective(self) -> bool:
            r"""Decide projectivity of the selected presentation from its Fitting ideals.

            A finitely presented module is projective exactly when every
            Fitting ideal is generated by an idempotent.  Over a ring whose
            only idempotents are zero and one, an integral domain among them,
            that says the module has one rank ``r`` everywhere, with
            ``Fitt_{r-1}(M) = 0`` and ``Fitt_r(M) = R``.  So the decision is
            read off the presentation, and the module need not be placed as
            projective in advance for the question to be asked.

            The Fitting ideals commute with base change, which is why this
            single condition is equivalent to freeness of every localization.
            """
