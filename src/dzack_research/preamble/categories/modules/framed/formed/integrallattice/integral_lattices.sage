...
class IntegralLattices(CategoryWithAxiom_over_base_ring):
    ...
    class ParentMethods:
        ...
        def is_parabolic(self: "LatticeParent") -> bool:
            r"""Return whether the lattice is parabolic (euclidean type).

            Negative semidefinite **with a radical of rank exactly one**:
            signature $(0, n-1, 1)$.  Semidefiniteness alone is not the
            condition -- every negative definite lattice is negative
            semidefinite, so without the corank clause $E_8$ answers `True`
            and elliptic and parabolic stop being exclusive.  The same
            definition is stated on the Schläfli matrix at
            ``vinberg_invariants.is_parabolic``.

            A parabolic lattice is degenerate, and ``refine_one_lattice``
            keeps a degenerate lattice out of ``IntegralLattices``, so no
            object carrying this method is currently parabolic.  The missing
            piece is the parabolic (and degenerate) category itself, recorded
            in ``notes/category-design/lattice-categories-roster.md``.
            """
            gram = self.gram_matrix()
            return bool(
                (-gram).is_positive_semidefinite()
                and gram.rank() == gram.nrows() - 1
            )
        ...
        def discriminant_representation_is_surjective(self: "LatticeParent") -> bool:
            r"""Return whether $\rho_L: O(L)\to O(A_L)$ is onto.

            :meth:`discriminant_representation` assumes no surjectivity, and
            this is how to ask.  Both sides are answered where they are
            grounded: the image is ``Aut().discriminant_image()``, generated
            by the images of $O(L)$'s generators because a homomorphic image
            is generated by the images of the generators; the codomain is the
            finite $O(A_L)$.  A subgroup of a finite group is the whole group
            exactly when the orders agree, so the comparison is of two
            cardinalities and nothing is enumerated.

            It is a genuine question, not a convention: surjectivity holds
            for an indefinite lattice of large enough rank (Nik80 Thm 1.14.2)
            and fails in general, and it is the standing hypothesis of
            Nikulin's gluing arguments -- including the vector-orbit gluing
            route in :mod:`vector_orbits`, which asserts it by this name.
            """
            image: bool = (
                self.Aut().discriminant_image().cardinality()
                == self.discriminant_group().automorphism_group().cardinality()
            )
            return image
