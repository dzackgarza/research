...
def _ideal_meets_inverted_submonoid(ideal, localization_ring) -> bool:
    r"""Decide whether the ideal ``I <= R`` meets the submonoid ``S`` that ``S^{-1}R`` inverts.

    ``I`` meets ``S`` exactly when ``S^{-1}I`` is the unit ideal of
    ``S^{-1}R``.  The submonoid is read from the localization ring:

    * the complement of a prime ``p`` meets ``I`` exactly when ``I`` is not
      contained in ``p``, that is when a generator of ``I`` lies outside ``p``;
    * the nonzero elements of a domain meet ``I`` exactly when ``I`` is nonzero;
    * a submonoid generated by ``f_1, ..., f_r`` meets ``I`` exactly when a
      power of ``f = f_1 ... f_r`` lies in ``I``, that is when the saturation
      ``I : f^infinity`` is the unit ideal.
    """
    from dzack_research.preamble.categories.rings.commutative_algebra import (
        PrimeLocalizations,
    )

    ring = localization_ring.localization_source()
    match localization_ring:
        case _ if localization_ring in PrimeLocalizations():
        ...
        case _ if localization_ring.is_fraction_field_localization():
            return ideal != ring.ideal(ring.zero())
        case _:
            inverted = prod(localization_ring.inverted_elements(), ring.one())
            return ideal.ideal_saturation(ring.ideal(inverted)).contains_ambient_element(
                ring.one()
            )
    ...
    class ParentMethods:
        ...
        def _fraction_equality_status(self, left, right):
            ...
            match source:
                case _ if source in _SelectedFinitePresentationModules(source_ring):
                    return _ideal_meets_inverted_submonoid(
        ...
        def is_zero(self):
            ...
            match source:
                ...
                case _ if self.is_finite() is True:
                    statuses = tuple(
