...
def _rank_one_coefficient(
    ...
    if not labels.cardinality().is_finite() or int(labels.cardinality()) != 1:
        raise TypeError(
            f"the local trivialization of a cyclic cover needs {module} to be generated by one element, "
    ...
    def __init__(
        ...
        *,
        ...
        match line_bundle in trivialized:
            ...
            case False:
                raise TypeError(
                    f"a cyclic cover needs an invertible sheaf L with a chosen affine trivialization, but "
                    f"{line_bundle} is not one"
                )
        ...
        match line_bundle.cover() in DistinguishedAffineCovers():
            case False:
                ...
                if supplied_parent is not branch_parent:
                    raise ValueError(
                        f"the branch section {branch_section} is not a section of L^{degree} for L = {line_bundle}"
                    )
                if branch_power.gluing_datum() is not line_bundle.gluing_datum():
                    raise ValueError(
                        f"the branch section {branch_section} and L = {line_bundle} must be given on one affine atlas"
                    )
                ...
                for left, right in line_bundle.gluing_datum().transition_index_set():
                    ...
                    if actual != expected:
                        raise ValueError(
                            f"the branch section {branch_section} is not a section of L^{degree} for L = {line_bundle}: "
                            f"its transition on ({left}, {right}) is {actual}, not {expected}"
                        )
            case True:
                ...
                if branch_power.cover() is not line_bundle.cover():
                    raise ValueError(
                        f"the branch section {branch_section} and L = {line_bundle} must be given on one affine cover"
                    )
