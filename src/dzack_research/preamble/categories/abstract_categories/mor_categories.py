...
class CategoricalMor(CategoryPacketMethods, OwnedMor, Category):
    ...
    @cached_method
    def _threaded_element_type(self) -> type:
        r"""Return the arrow implementation generated by the fixed-Mor graph.

        The represented Mor parent has a deliberately isolated Sage
        parent-class spine, because feeding fixed Mor categories into Sage's
        category-class linearizer creates cycles. Arrow implementations are
        different: mathematically Mor_D(A,B) lies below the corresponding
        Mors of every declared supercategory of D, so its arrows inherit
        those arrow types and add only the datum declared by this Mor class.
        Build that one element type directly from the semantic fixed-Mor
        supergraph, leaving the parent/category runtime spine untouched.
        """
        declared = getattr(type(self), "Element", None)
        candidates = ([] if declared is None else [declared])
        candidates.extend(
            supercategory.ElementType
            for supercategory in self.super_categories()
            if hasattr(supercategory, "ElementType")
        )
        bases: list[type] = []
        for candidate in candidates:
            if any(
        ...
        assert bases, f"{self} declares no arrow type and inherits none"
        if len(bases) == 1:
            return bases[0]
        return dynamic_class(
            f"{type(self).__name__}.ElementType",
            tuple(bases),
            None,
            doccls=declared or type(self),
            cache=False,
        )

    @property
    def ElementType(self) -> type:
        r"""The complete arrow type of this represented fixed Mor category."""
        return self._threaded_element_type()

    @property
    def element_class(self) -> type:
        r"""Sage's element-class spelling of ElementType."""
