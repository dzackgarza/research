...
class DiscriminantQuadraticModules(Category):
    ...
    class ParentMethods:
        ...
        def gram_matrix(self: Any) -> Any:
            ...
            raw = TorsionQuadraticModule.gram_matrix_quadratic.f(self)
            cuts = _compute_disc_subdivisions(self, raw)
            if cuts:
                G = raw.parent()(raw)
        ...
        def associated_bilinear_form(self: Any) -> Any:
            ...
            if cuts:
                ...
                G = raw
            return DiscriminantBilinearForm(self.invariants(), self.source_lattice(), G)

        def normal_form(self: Any, *args: Any, **kwargs: Any) -> Any:
            r"""Return the normal form, marked as such."""
            from sage.modules.torsion_quadratic_module import TorsionQuadraticModule

            norm = TorsionQuadraticModule.normal_form(self, *args, **kwargs)
...
def _compute_disc_subdivisions(A_disc: Any, raw: Any) -> list[int]:
...
def _primary_gram_matrix_latex(A_disc: Any) -> str:
    ...
    if not invs:
        return "()"
    gram_str = str(_latex_fn(A_disc.gram_matrix()))
    if _zero_dots():
