...
class IntegralLattices(Category):
    ...
    class ParentMethods:
        ...
        def glue(self: Any, *vectors: Any) -> Any:
            r"""Return the even overlattice generated by rational glue vectors.

            The input vectors are given in the current basis of ``self`` and
            represent lifts of elements of the discriminant module $A_L$.  This
            method constructs the row lattice generated by the original basis
            and the listed rational rows, then induces the bilinear form on
            that lattice.
            """
            rank = self.rank()
            rational_rows = [
            ...
            rational_rows.extend(
                [QQ(coordinate) for coordinate in row] for row in vectors
            )
