...
class ComplexManifolds(OwnedCategory):
    ...
    class ParentMethods:
        ...
        def is_open_submanifold(self) -> bool:
            return self in ComplexOpenSubmanifolds()


class ComplexOpenSubmanifolds(OwnedCategory):
    r"""Open subsets of complex manifolds, each with its inclusion.

    An object is the open subset ``U = {x : restriction(x)}`` of a complex
    manifold ``M``, cut out in the one chart of ``M``.  It is a complex
    manifold with that chart restricted to ``U``, and
    :meth:`ParentMethods.open_inclusion` is the holomorphic inclusion
    ``U -> M``.
    """

    def super_categories(self):
        return [ComplexManifolds()]

    @classmethod
    def _repr_object_names(cls):
        return "open complex submanifolds"
...
class ComplexDiscs(OwnedCategory):
    r"""Discs ``{|z| < r}`` and punctured discs ``{0 < |z| < r}`` in a one-dimensional complex chart.

    The radius ``r`` and whether the centre is removed are the construction
    data.
    """

    def super_categories(self):
        return [ComplexOpenSubmanifolds()]

    @classmethod
    def _repr_object_names(cls):
        return "complex discs"
