...
def _finite_set_quotient_by_generated_relation(target, relation_pairs):
    r"""Return a finite set quotient by the equivalence relation generated by pairs.

    ``SageDisjointSet`` privately computes the transitive-symmetric closure on
    ranking positions.  The returned quotient and projection are owned sets and
    maps; ``representative_point`` is used only to define the universal factor.
    """
    ...
    for left, right in relation_pairs:
        classes.union(position(left), position(right))
    ...
    def representative_point(label):
        return target_points[representatives[int(label)]]

    return quotient, projection, representative_point
...
class Sets(OwnedCategory):
    ...
    class SubcategoryMethods:
        ...
        def _finite_parallel_morphism_family(self, morphisms):
            r"""Return the data of a finite enumerable family of parallel set maps."""
            size = morphisms.cardinality()
            assert size.is_finite(), (
                f"the wide (co)equalizer of {morphisms} is computed here only for a finite family of maps, "
                f"but the family has cardinality {size}"
            )
            if size == cardinal(0):
                raise ValueError(
                    f"the wide (co)equalizer in {self} needs a nonempty family of maps to retain its common "
                    f"domain and codomain, but got {morphisms}"
                )
            index_set = morphisms.index_set()
            assert index_set in EnumeratedSets(), (
                f"the finite family {morphisms} is computed here only when its index set {index_set} carries "
                "an enumeration"
            )
            labels = tuple(index_set)
            reference = morphisms.value(labels[0])
            match reference:
                case Morphism():
                    pass
                case _:
                    raise TypeError(
                        f"a wide (co)equalizer in {self} needs morphisms of sets, but the family contains {reference}"
                    )
            source = reference.domain()
            target = reference.codomain()
            if source not in self or target not in self:
                raise TypeError(
                    f"a wide (co)equalizer in the category of sets needs maps between sets, but {source} or "
                    f"{target} is not a set"
                )
            for label in labels:
                arrow = morphisms.value(label)
                if not (
                    isinstance(arrow, Morphism)
                    and arrow.domain() is source
                    and arrow.codomain() is target
                    and _category_accepts_morphism(self, source, target, arrow)
                ):
                    raise ValueError(
                        f"a wide (co)equalizer needs a parallel family of set maps {source} -> {target}, but "
                        f"the arrow at {label!r} is {arrow}"
                    )
            return labels, reference, source, target

        def _categorical_equalizer_family(self, morphisms):
            return self._categorical_equalizer_family_construction(morphisms).object()

        def _categorical_equalizer_family_construction(self, morphisms):
            r"""Return the selected wide equalizer of a finite represented family of set maps.

            The family is retained as the arrow family of the walking diagram;
            finiteness is only the effective realization boundary.  The cone
            therefore keeps the universal inclusion rather than returning only
            its apex.

            Unverified specimen: residue mod three and zero have wide equalizer
            ``{0, 3}``, and its factorization recovers a compatible source leg::

                sage: from dzack_research.preamble.categories.sets.indexed_families import indexed_family
                sage: source, target = Sets.Δ[5], Sets.Δ[2]
                sage: residue = Sets().Mor(source, target)(lambda n: target(n % 3))
                sage: zero = Sets().Mor(source, target)(lambda _n: target(0))
                sage: family = indexed_family(Sets.Δ[1], lambda i: (residue, zero)[int(i)])
                sage: construction = Sets().equalizer_of_family_construction(family)
                sage: shape = construction.diagram().domain()
                sage: inclusion = construction.structure_morphism(shape.source())
                sage: construction.diagram().morphisms() is family
                True
                sage: {inclusion(x) for x in construction.object()} == {source(0), source(3)}
                True
                sage: apex = Sets.Δ[0]
                sage: source_leg = Sets().Mor(apex, source)(lambda _point: source(0))
                sage: target_leg = residue * source_leg
                sage: cone = construction.diagram().Cones().cone(apex, lambda j: source_leg if j is shape.source() else target_leg)
                sage: factor = construction.factor(cone).apex_map()
                sage: (inclusion * factor)(apex(0)) == source_leg(apex(0))
                True

            A genuine three-map family is not reduced to its first two arrows::

                sage: source, target = Sets.Δ[3], Sets.Δ[1]
                sage: zero = Sets().Mor(source, target)(lambda _n: target(0))
                sage: parity = Sets().Mor(source, target)(lambda n: target(n % 2))
                sage: second_half = Sets().Mor(source, target)(lambda n: target(0 if n < 2 else 1))
                sage: maps = (zero, parity, second_half)
                sage: family = indexed_family(Sets.Δ[2], lambda i: maps[int(i)])
                sage: wide = Sets().equalizer_of_family_construction(family)
                sage: Sets().equalizer(zero, parity).cardinality(), wide.object().cardinality()
                (2, 1)
            """
            from dzack_research.preamble.categories.abstract_categories.products import (
                SelectedLimitConstruction,
                _parallel_family_diagram,
            )

            labels, reference, source, _target = self._finite_parallel_morphism_family(
                morphisms
            )
            equalizer = self.condition_set(
                source,
                lambda element: all(
                    morphisms.value(label)(element) == reference(element)
                    for label in labels
                ),
            )
            inclusion = equalizer.inclusion()
            diagram = _parallel_family_diagram(morphisms, self)
            shape = diagram.domain()
            universal_cone = diagram.Cones().cone(
                equalizer,
                lambda index: (
        ...
        def _categorical_coequalizer_construction(
            ...
            quotient, projection, representative_point = (
                _finite_set_quotient_by_generated_relation(
                    target,
                    (
                        (left_morphism(point), right_morphism(point))
                        for point in source
                    ),
                )
            )
            ...
            def factorizer(cocone):
                ...
                return Sets().Mor(quotient, cocone.apex())(
                    lambda label: target_map(representative_point(label))
                )
        ...
        def _categorical_coequalizer_family(self, morphisms):
            return self._categorical_coequalizer_family_construction(morphisms).object()

        def _categorical_coequalizer_family_construction(self, morphisms):
            r"""Return the selected wide coequalizer of a finite represented family of set maps.

            Relations against one reference arrow generate the same equivalence
            relation as all pairwise relations ``f_i(x) ~ f_j(x)``.  The selected
            construction retains the original indexed diagram, its quotient
            projection and the universal factor.

            Unverified specimen: adjacent pairs identify ``0,1,2`` while leaving
            ``3`` separate, and a compatible map out of the target factors through
            that two-point quotient::

                sage: from dzack_research.preamble.categories.sets.indexed_families import indexed_family
                sage: source, target = Sets.Δ[1], Sets.Δ[3]
                sage: initial = Sets().Mor(source, target)(lambda n: target(n))
                sage: successor = Sets().Mor(source, target)(lambda n: target(n + 1))
                sage: family = indexed_family(Sets.Δ[1], lambda i: (initial, successor)[int(i)])
                sage: construction = Sets().coequalizer_of_family_construction(family)
                sage: shape = construction.diagram().domain()
                sage: projection = construction.costructure_morphism(shape.target())
                sage: construction.diagram().morphisms() is family
                True
                sage: construction.object().cardinality() == cardinal(2)
                True
                sage: projection(target(0)) == projection(target(2)) and projection(target(0)) != projection(target(3))
                True
                sage: apex = Sets.Δ[1]
                sage: target_leg = Sets().Mor(target, apex)(lambda point: apex(0) if point != 3 else apex(1))
                sage: source_leg = target_leg * initial
                sage: cocone = construction.diagram().Cocones().cocone(apex, lambda j: source_leg if j is shape.source() else target_leg)
                sage: factor = construction.factor(cocone).apex_map()
                sage: all((factor * projection)(point) == target_leg(point) for point in target)
                True
            """
            from dzack_research.preamble.categories.abstract_categories.products import (
                SelectedColimitConstruction,
            ...
            labels, reference, source, target = self._finite_parallel_morphism_family(
                morphisms
            )
            assert (
                cardinal(source.cardinality()).is_finite()
                ...
                f"the wide coequalizer of {morphisms} is computed here only between finite sets, but {source} "
                f"or {target} is not known to be finite"
            )
            quotient, projection, representative_point = (
                _finite_set_quotient_by_generated_relation(
                    target,
                    (
                        (reference(point), morphisms.value(label)(point))
                        for point in source
                        for label in labels
                    ),
                )
