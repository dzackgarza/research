...
def _affine_semigroup_algebra(
    ...
    *,
    ...
    r"""Present the affine semigroup algebra on the stated lattice generators.

    The supplied rows are lattice generators ``a_i``.  Sage's ``ToricIdeal``
    computes the kernel of ``k[z_i] -> k[t^M]``, ``z_i |-> t^{a_i}``; the
    resulting owned quotient is therefore the semigroup algebra generated by
    the ``a_i``.  This is shared by affine toric charts and divisor section
    rings so their binomial-relation engine is one construction.
    """
    coordinates = tuple(tuple(int(entry) for entry in row) for row in generator_coordinates)
    if names is None:
        names = _character_names(len(coordinates))
    if len(names) != len(coordinates):
        raise ValueError("an affine-semigroup presentation needs one variable per generator")
    presentation = PolynomialRing(base_ring, tuple(names))
