...
| V | `0e70a242` supplies invertible sheaves on G distinguished affine covers, with transition units, local trivializations, sections, tensor powers and duals. V1 now has `50b89584`, which attaches distinct toric Weil/Cartier and Class/Picard role objects and the explicit Cartier-to-Weil / Picard-to-Class comparison square; `278eb0f1` exposes toric height-one valuations and principal divisors; `06530859` promotes divisor-polytope characters to actual global-section vector spaces; `83468971` computes ample Cartier intersections on smooth complete toric surfaces by normalized divisor-polytope volume and polarization. Assertions in this continuation remain unverified under terminal-T policy. | Continue V1 with the supported Picard/intersection/linear-system cases. Associating these toric Cartier divisors to `InvertibleSheaf` is blocked on a common G extension: `_FiniteSchemeGluingDatum` does not yet expose the distinguished-cover module-descent protocol required by `ModuleGluingDatum`; do not duplicate that descent machinery in V. | V1 checkpoint: `83468971`; claim active |
