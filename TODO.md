...
- [ ] **`suite-within-time-gates`**. **Needs:** `mor-construction-computes-no-presentation`, `small-specimen-tests`.
  ...
  **Observed (2026-09-26):** the smallest `ADELogPairs` witness, type `A_1`, costs 0.7 s per base ring and `P^2` from its fan 0.9 s. Nearly all of it is building the fan's chart changes, and the integer arithmetic in them is negligible. Two wastes remain. (1) Every coefficient readout (`framing_coefficients`) constructs a fresh owned finite ordered set, so one pairing `<m, n>` of rank-2 lattice vectors costs about 1 ms. (2) Each cone re-derives the same pairings with its rays (`pair_with`, `_face_supporting_generator_positions`) on every call instead of once per cone.
