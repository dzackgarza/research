...
- [ ] **`framing-contract-unification`**. **Needs:** none.
  **Owner and delta:** the shared chosen-framing/presentation structure used by groups, modules and algebras. Current policy requires one highest-available `Framed` data contract, but the live tree still has separate module framing plus `GroupsWithChosenFinitePresentation`, `ModulesWithChosenFinitePresentation` and `AlgebrasWithChosenFinitePresentation` surfaces. Reconcile them without erasing their genuinely different relation semantics.
  **Mathematical datum:** a 1-framing is a chosen generating epimorphism from a free object; a chosen presentation extends it by a chosen free relation morphism, and higher stages are chosen syzygy data. Finite generation/presentation remain property axioms asserting existence, not a request to manufacture a witness. A property-to-data crossing is one explicit construction that fixes the chosen datum once (`CAT-02`, `CAT-17`, AGENTS.md *Property subcategories vs data subcategories*).
  **Deliver:** register and own the common framing contract once at the highest category that can state it; make group/module/algebra specializations inherit that datum and add only their structure-specific relation laws or realizations. Preserve the already-delivered selected module epimorphism/presentation objects and free-functor routes. Remove parallel declarations or derived methods only after all consumers use the common owner; do not replace mathematical presentation data by backend classes, generator lists or a compatibility facade.
  **Closure specimens:** a group known finitely presented only as a property has no selected `presenting_free_group`; a group with a chosen presentation exposes its actual free source and relators. A presented module retains its selected `F_1 -> F_0 -> M` maps and an algebra with a chosen presentation retains its generating module, relations and multiplication through the same framing spine. Intake of an object that already carries presentation data reads that datum rather than searching for another one. Equality of maps out of a finitely framed source uses the selected generators when they decide extensional equality; outside a represented exact-decision regime it reaches the repository's declared computational frontier rather than guessing a Boolean (`DEV-52`). Where the additive framing tower is packaged as a resolution, its differentials and augmentation recover the selected maps rather than a new coordinate surrogate. Inspect the complete framing/presentation owner and consumer family across groups, modules and algebras; one migrated spelling does not close the node.

## Diagrams, rings and geometric consumers

- [ ] **`objects-through-categories-algebras`**. **Needs:** `framing-contract-unification`.
...
- [ ] **`owned-provenance-data`**. **Needs:** `objects-through-categories-algebras`, `objects-through-categories-lattice-files`, `sheaf-descent-threading`.
...
- [ ] **`membership-by-placement`**. **Needs:** `objects-through-categories-algebras`, `objects-through-categories-lattice-files`, `group-objects-construction`, `sheaf-descent-threading`.
...
- [ ] **`inherited-operations-not-restated`**. **Needs:** `objects-through-categories-algebras`, `objects-through-categories-lattice-files`.
...
- [ ] **`coordinate-firewall`**. **Needs:** `framing-contract-unification`.
