...
- [ ] **`engine-algebras-module-construction`**. **Needs:** none.
  **Goal:** Every engine-backed algebra is a module constructed through `Modules(R)`, so that the forgetful functor \(\mathbf{Alg}_R\to\mathbf{Mod}_R\), which is the identity on objects, lands on an object that answers its module operations by inheritance for the whole category and not only for algebras built by `Algebras(R)(M, m)`.
  **Observed gap:** `Algebras(R)` declares `Modules(R)`, so `_OwnedAlgebraParent` (`algebras/algebras.py`) -- polynomial rings, \(T_R(M)\), \(\operatorname{Sym}_R(M)\), chosen-presentation quotients -- and `SparseFreeAlgebra` (`algebras/sparse_free_algebras.py`, `Parent.__init__` with hand-written `module_generating_set`, `module_generator`, `linear_combination`, `zero` and `scalar_multiple`) are placed in `Modules(R)` by declaration while none of them establishes its underlying module through the module chain.  `FramedAlgebras.ParentMethods.cardinality` restates a set-level operation on the algebra for the same reason.  Until commit `83c80ce4` the forgetful functor answered a free algebra with a separate graded direct sum of its homogeneous pieces, indexed by words or monomials; that was a wrapper around the algebra rather than the algebra, and the history of `functors/algebra_modules.py` before that commit records how the words and monomials index the pieces.
  **Owners:** `_OwnedAlgebraParent`; `SparseFreeAlgebra`; the graded direct sum module (`modules/graded_direct_sums.py`) as the module a free algebra is constructed on; `FramedAlgebras`.
  **Deliver:** free algebras constructed on the graded direct sum of their homogeneous pieces, through `Modules(R)`, with the hand-written module operations and the algebra-level `cardinality` deleted; a chosen-presentation quotient constructed on its presented module.
  **Acceptance:** `Modules(R).symmetric_algebra()(M)` answers `module_generating_set`, `module_generator` and `scalar_multiple` by inheritance from the module it is constructed on; no operation `Modules(R)` provides is defined in the algebra subtree.
...
- [ ] **`architecture-remediation`**. **Needs:** `mor-spelling-convergence`, `group-objects-construction`, `cat-valued-placement`, `membership-by-placement`, `sheaf-descent-subcategory`, `sheaf-object-placement`, `algebras-are-modules`, `engine-algebras-module-construction`, `arrow-category-placement`, `geometric-space-placement`, `combinatorial-object-placement`, `owner-api-convergence`, `framing-primary-epi`, `framing-specialization-convergence`, `generator-lexicon`, `ambiguous-generator-names`, `owned-provenance-data`, `refinement-convergence`, `assertion-frontiers`, `placeholder-stubs`, `categorical-representation-convergence`, `group-module-scalar-change-convergence`, `memoization-convergence`, `singular-kernel-delegation`, `torsion-action-delegation`, `imperative-algorithm-cleanup`, `owned-product-codomains`, `mathematical-return-types`, `coordinate-firewall`, `canonical-notebook-contract`, `ownership-test-contract`.
