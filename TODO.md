...
- [ ] **`placement-audit`**. **Needs:** none.
  **Owner and delta:** every public object and element operation introduced on each owned category, judged by the `CAT-05` placement test: an operation introduced on `C` is misplaced when it is well-defined on a supercategory of `C`, and moves to `max W_f` over the up-set `U_C = {D > C}`.
  ...
  Loop body: one category `C`. For each introduced object and element operation `f`, state the mathematical definition of `f` with the data and hypotheses it uses, read from a source, not from its method body. Then walk `U_C` upward and decide, for each `D`, from the definition of `D`, whether every object of `D` supplies that data and satisfies those hypotheses. The members that do form `W_f`, and `max W_f` is the home. Then move `f` and route its computation.
