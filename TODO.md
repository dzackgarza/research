...
| M | `e472b85a` establishes direct-action modules and the module-owned action morphism. `ec7c2d00` exposes selected presentation diagrams; `eb27357a` exposes selected framing diagrams; `6fadf470` lifts module maps to commuting squares of selected presentations. `501fb734` enriches noncommutative module Homs over `Z(R)`; `a329daf7` makes restriction of scalars expose `rho_M compose f`; `287d2137` retains source/degree data for tensor, symmetric, exterior, and divided powers; `5b94a550` defines represented commutative linear duals; `f836960e` records localization as scalar extension of the selected presentation/action contract. The direct/presentation `ZZ/6`, free-plus-torsion, infinite-framing, left-regular `M_2(QQ)`, derived-constructor, duality, and localization specimens are committed unverified under terminal-T policy. | M0 is complete. Broader M work resumes at M1 for exact local-module algorithms and at F/U for the generic framework transfer of the framing structure. | Released: `f836960e`; M0 complete, M1 remains open |
