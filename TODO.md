...
| --- | --- | --- | --- | --- | --- |
| `C-parameter-contract-20260907-1630` | C / shared mathematical parameter-domain contract | Chat continuation 2026-09-07 Asia/Taipei; `/home/dzack/research` | `src/dzack_research/preamble/categories/abstract_categories/objects.py` (write) | `3610c3c9c6180be7dabba6475c9c850fe5b51389` | 2026-09-07T07:05:29Z |
...
| C | `e472b85a` unifies ordinary-parent and fixed-Hom construction with nested refinement hooks. `78494432` preserves the selected scalar ring of endomorphism algebras. `a3017a8f` gives exact-carrier `EndofunctorAlgebras(T)=Inserter(T,Id)`. `3e3e443d` removes inherited Hom endpoint reprobes. `3e9f7f3e`/`0a4de522` publish abstract obligations. `d88800f4` types the generic categorical constructors and morphism carriers; `3610c3c9` types Hom packets, fixed Hom objects, restricted families, and isomorphism witnesses. | Make the shared `OwnedParameterizedCategory` interface itself state that `parameter_category()` returns a category and that its parameter is an object of that declared domain; preserve the per-subclass domain obligation instead of replacing it by a catch-all type. | Claimed: `C-parameter-contract-20260907-1630`; broader C open |
