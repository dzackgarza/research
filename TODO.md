...
| V | `0e70a242` supplies invertible sheaves on G distinguished affine covers, with transition units, local trivializations, sections, tensor powers and duals. V1 has `50b89584`, which attaches distinct toric Weil/Cartier and Class/Picard role objects and the explicit comparison square; `278eb0f1` exposes toric height-one valuations and principal divisors; `06530859` promotes divisor-polytope characters to actual global-section vector spaces; `83468971` computes ample Cartier intersections on smooth complete toric surfaces by normalized divisor-polytope volume and polarization; `38ae0f36` records the same quotient construction on `A^2`, `P^2`, and `P^1 x P^1`; `547f9a18` introduces complete linear systems as projective spaces retaining their scheme, divisor, and exact section space. Assertions in this continuation remain unverified under terminal-T policy. | Continue V1 with section/Cox-ring and cycle infrastructure. Cartier-divisor-to-`InvertibleSheaf`, including distinguished `O(1)`, is blocked on a common G extension: neither `_FiniteSchemeGluingDatum` nor the standard projective atlas exposes the distinguished-cover module-descent protocol required by `ModuleGluingDatum`; do not duplicate that descent machinery in V. | V1 checkpoint: `547f9a18`; claim active |
