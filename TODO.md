...
- [ ] **`algebra-structure-morphism-constructor`**. **Needs:** none.
  ...
  **Deliver:** the root entry `Algebras(R)(A, rho)` storing \(\rho\) and nothing else; `algebra_structure_morphism()` the root method returning it; the \((M, m)\) route building \(A\) on \(M\) with multiplication \(m\) and calling the entry with the scalar action; \(R\) placed over itself with \(\mathrm{id}_R\) and every ring over \(\mathbb{Z}\) with its unique morphism; `OwnedAlgebras`, `AlgebraStructureConstruction`, the thirteen leaf restatements, the copy-and-identify machinery and its names deleted; the Hom admitting a morphism \(f\) of the underlying rings with \(f\circ\rho_A=\rho_B\) (for \(A\) not unital associative, a multiplicative additive map with \(f(\rho_A(r)a)=\rho_B(r)f(a)\)), deciding on finitary data and recording `Unknown` as the hypothesis otherwise.
  **Acceptance:** `Algebras(R)(A, rho)` is \(A\) placed in `Algebras(R)`, not a copy, and its `algebra_structure_morphism()` is \(\rho\); \(R\) as an algebra over itself answers \(\mathrm{id}_R\); the algebra level stores no multiplication or unit, so an engine ring's `one()` and product are its own; `algebra_structure_morphism` is defined once, at the root; `R[G]`, centres, quotients, the commutator Lie algebra, every unital associative algebra, and `Lie` and nonunital algebras reach `Algebras(R)` through the one entry; no category in the tree is named for carrying the root's datum.
