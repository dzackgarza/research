...
Read it before changing the preamble, together with the generated `writing/category-theory/preamble-megadoc.md`.
...
The next shared C/P construction must represent distinct algebra structures that forget to the same exact supplied module.
A0 next constructs actual functors `BG -> C`, natural transformations, and genuine group-algebra module parents.
The handoff gives the candidate edit boundaries, mathematical consumers, model assignments, upstream dependencies, and concurrent-work state.
This queue retains all remaining requirements and their existing dependency order.
...
Before editing `src/dzack_research/preamble/**`, follow `AGENTS.md`: read this TODO and the generated `writing/category-theory/preamble-megadoc.md`; regenerate the megadoc first when it is stale relative to the live tree.
...
| Imports and generated documentation | Exact `__init__.py`/session entrypoints; `writing/category-theory/preamble-megadoc.md` and `writing/category-theory/preamble-graph.{json,dot,html}` as one generated set | All streams. One worker integrates exports and regenerates a coherent source snapshot. |
...
| `C-homset-resolution-20260907-1740` | C / inherited Hom construction and endpoint-owned construction packets | Chat continuation 2026-09-07; `/home/dzack/research`; resumes recorded C edits through predecessor session `2026-09-07-9e8d73df` | `src/dzack_research/preamble/categories/abstract_categories/hom_categories.py; src/dzack_research/preamble/categories/sets/set_categories.py; src/dzack_research/preamble/categories/sets/finite_ordered_sets.py; src/dzack_research/preamble/categories/abstract_categories/cat.py; src/dzack_research/preamble/categories/abstract_categories/arrow_categories.py; src/dzack_research/preamble/categories/abstract_categories/category_constructions.py; src/dzack_research/preamble/categories/abstract_categories/functors.py; src/dzack_research/preamble/categories/abstract_categories/products.py; src/dzack_research/preamble/categories/functors/core.py; src/dzack_research/preamble/categories/abstract_categories/hom_foundation.py; src/dzack_research/preamble/categories/sets/indexed_families.py` (write) | `6be6c4151d887fff3eae5292ecb80d31b75d3797` | 2026-09-07T13:02:28Z |
...
| --- | --- | --- | --- |
| E | Research `70f17402` and `e2294760`, with bridge `6625927`, supply persistent OSCAR calls, exact structured matrices, owned dependencies, and faithful initialization errors. `785749ee` uses Singular `modulo` for polynomial-presentation kernel lifts and relations, with native `lift` for the owned inclusion. | Remaining engine operations and conversion registrations follow the selected mathematical consumers. Startup and mathematical execution remain with T. | OSCAR and native quotient-kernel source releases complete; broader E remains open |
| C | `e472b85a` unifies construction hooks; `3e3e443d` repairs Hom inheritance; `3e9f7f3e`/`0a4de522` publish abstract obligations. `d88800f4`, `3610c3c9`, `fcddb1eb`, `92efc99e`, `7310bad8`, `abe0fd22`, `de7a85ed`, and `76993d8e` type the generic category/Hom/set construction owners. `5ba5eb6c` types coordinate-family, finite-family and enumerated-function helper surfaces. `152301b8` and `f629c859` type membership, inclusion, underlying-set Hom, refinement/realization, and the common functor/adjunction dispatch. `4a817e6f` completes the shared abstract-category, Hom-packet, arrow, functor-category, and diagram interfaces. `e22fc7c4` and `0291cbbb` type the generic set/cardinal/indexed-family constructors and their public value interfaces without catch-all annotations. `580c3502` gives functor provenance exact object/morphism records and types generated axiom-base accessors. `e70e1030` declares all categorical construction hooks at `Cat`, dispatches through inherited category methods rather than dynamic capability search, and normalizes wide arrow-family ingress. `598a5d66` removes binary-only direct-sum verification and hidden-field reads; the public accessor now trusts `DirectSumObjects` constructor-owned structure. `4880b067` removes matrix duck-typing from finite coordinate ingress because Sage matrices already iterate by rows. `6be6c415` removes dynamic Hom-carrier probes. | Complete inherited Hom and restricted-Hom resolution against the declared packet, including constructor recursion and endpoint identity. The `object_of(Category, **data)` `Any` boundary remains a cross-stream helper redesign/post-transfer boundary. Stable category ordering is still externally blocked by bare `RootLattices`, `RingedSpaces`, `LocallyRingedSpaces`, and the session realization loop. | Claimed: `C-homset-resolution-20260907-1740`; broader C open |
...
| M | `e472b85a` constructs `Modules(R)(rho)` from its exact action into additive endomorphisms and defines the forgetful functor on objects and maps; `78494432` preserves the scalar base of endomorphism algebras. `41d239c0` through `b67b35e3` define localized fraction actions additively and transport framing data through cooperative construction; assertions are unverified. Existing releases: `55c16f43` gives presented localization equality/vanishing, localized subobjects, fibers, and transported kernels; `65fe44e6` localizes Fitting ideals; `da64cc2e` supplies field/PID flatness criteria; `b9a9770a` supplies invariant-factor free-module isomorphisms and transported local trivializations. | Thread the remaining constructors, selected presentation chain maps, free-plus-torsion and infinite-index data through the defining action. Complete centre enrichment, scalar-change/localization reconciliation, and the remaining exact local algorithms. | Localized action source released: `b67b35e3`; broader M remains open |
| P | `66466ec7`: relative `xy=t`, special fiber, and scalar-preserving successive algebra quotients. `86ed6a99`: free/presented pushout spans. `289d4756`: number-field algebras retain their field engine and chosen primitive presentation. `379ce99d` converts `Algebras(R)` to the arbitrary-multiplication equipped-object node on the exact supplied module, with a genuine algebra-to-module forgetful functor, the common multiplication-preserving Hom, independent associative/unital/commutative refinements, and the ordinary associative-unital ring intersection. Existing ring-backed families and scalar-change/scheme consumers now state the stronger placement explicitly; `R[G]` uses the same exact constructor and retained unit map. The node/axiom assertions are committed and unverified under the preamble execution deferral. | Convert the Lie refinement and commutator construction onto the general node: represent the bracket from multiplication, keep Lie identities independent of associativity/unitality, and route commutator maps through the common algebra Hom. Then promote the associative centre from the general submodule construction to its stronger algebra object where required. | General algebra node released: `379ce99d`; Lie/commutator conversion unclaimed |
...
| A | `d7aef49c` supplies `G.classifying_category()` and `ClassifyingFunctor(phi)` with committed, unverified assertions. Earlier releases: `0e25eedb`: the owned group algebra `R[G]` with group inclusion, augmentation, centre and Maschke; `Groups().group_algebra(R)` on subgroup inclusions. `2ddbfcb4`: group modules are `Modules(R[G])`, refining `GObjects(G, Modules(R))`; `GroupModules` retired. `d73b5709`: G-set vocabulary on `FiniteSets()`/`FiniteGSets(G)`; lattices with an action are `Lattices(ZZ[G])`. `5ad9314c`: `Modules(S)(M, rho)` from a ring morphism `S -> End(M)`; `Hom_R(S, -)` as `Modules(R).coextension_of_scalars(f)` for `S` finitely framed over `R`, with `Res_f -\| Hom_R(S, -)`; the scalar-change triple spelled on `Modules(R)`. `bd7a44e8`: induction/restriction/coinduction are scalar extension/restriction/coextension along `R[H] -> R[G]`, recognized on the map and realized on a transversal; `Modules(R[G])(M, rho)` restricts a ring morphism along `G -> R[G]`. `fd12b070`: the trivial action, invariants and coinvariants are restriction, coextension and scalar extension along the augmentation `R[G] -> R`, taken as a ring morphism; the standalone adjunction functions left the session. `233963af`: restriction along `R -> R[G]` forgets the action, so `Res -\| Hom_R(R[G], -)` along the structure map has its unit and counit; group modules multiply by group-algebra scalars. `4150e30b`: every commutative owned ring is a commutative algebra over itself while retaining its proper scalar-restriction towers; self-algebra `.Mor` keeps ring-morphism dispatch, base scalars embed canonically in endomorphism rings, regular `R[G]` modules/actions are preserved, and `R[G].augmentation()` is an `R`-algebra morphism. Predicate subrings and regular-module localization remain coherent under the new placement. `a8224db4` completes A0: `GObjects(G,C)(F)` takes an actual `BG -> C` functor; generic equivariant arrows are the existing natural transformations; represented G-sets and `R[G]`-modules expose the same `action_functor()`; the forgetful functor is evaluation at the unique object; restriction is precomposition by `ClassifyingFunctor(phi)`; and transport is postcomposition by `C -> D` on objects and nonidentity arrows. The focused assertions are committed unverified under terminal-T deferral. `8caaf5ab` makes the finite torsor refinement executable: free/transitive membership, chosen-point enumeration/cardinality, and unique transporters. `027c097f` puts finite point stabilizers and orbit-indexed stabilizers at the G-set owner and distinguishes a generic transporter witness from the unique torsor transporter. `b1ff9abe` makes the represented underlying finite-set functor recover a concrete G-set from its literal same-parent image, supplying the chosen-preimage path used by the free/underlying/cofree adjunction transposes. These assertions remain unverified under terminal-T deferral. `9e9fca6a` makes default `End()` and `Aut()` on an `R[G]`-module equivariant, retains explicit underlying `Modules(R).End/Aut`, and restricts equivariant endomorphisms/automorphisms through the same acted subobject to actual `End_{R[G]}`/`Aut_{R[G]}` objects. The S3 permutation-module reflection `1-2P_triv` is committed as an unverified specimen restricting to `+1` on the standard isotypic piece and `-1` on invariants. | A0 is complete and the general A1 representation-functor plus restricted-automorphism contracts are released. Remaining A-owned requirements must be selected from the live detailed queue; the `(4,4)` section-space decomposition waits on its geometry, while scheme-action reconciliation remains with Q. | A1 restricted automorphisms released: `9e9fca6a`; audit remaining A queue |
| Q | `28b23405` constructs affine scheme actions and fixed subschemes. `dfdb460d` constructs invariant algebras for supported finite linear polynomial actions, their affine quotient maps, and invariant-map factorizations. `1847d9c1` assembles finite glued quotients from indexed affine G-charts and supplied descended overlap isomorphisms, retaining source equivariance, descent squares, the global action, quotient map, and affine-target factorization. The integrated source and its assertion file are committed; the recorded handoff has no pending source edit. | Reconcile these constructors with the released A0/G defining-data contracts before further quotient constructions. T owns current mathematical execution. | Earlier finite glued-quotient source released; broader Q requirements remain open |
