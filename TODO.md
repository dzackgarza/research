...
- [ ] **`algebra-structure-morphism-constructor`**. **Needs:** `remote-remediation-branches`.
  ...
  **Remaining:** the root now owns `(M,m)`, its unit refinement, the structure morphism and the multiplication-preserving Hom equation; the duplicate multiplication categories, source-copy identification maps and leaf structure-map definitions have been removed. The native self-referential ring/algebra constructors now initialize that root datum before returning, and the self-based identity structure retains the actual ring. Complete the construction contract for every engine route, notably predicate subrings and non-self-based rings given a second scalar structure, rather than treating native bootstrap initialization as a replacement for the already-required module construction. Complete the general unframed tensor/classifying-map route so multiplication-preserving Homs are represented outside a chosen module framing. The remaining specialized Lebesgue constructor still allocates its own module facade; its exact root data are retained, but it must be constructed through the function/module owner, not accepted merely because the old fake category names disappeared.
