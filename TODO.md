...
- [ ] **`finite-affine-atlases`**. **Needs:** none.
  ...
  **Observed gap:** `CoveringFamilies(C)` now constructs each family on its finite presentation diagram and places that diagram in the existing slice `Cat/C`; distinguished affine covers thread their defining elements through that entry, deriving charts and indices from the root family. `DescentDataOnCover` deliberately has only `Objects()` as its same-object parent: descent is relative to a fibre theory, and forgetting to the local family or Čech diagram changes the object and is a functor (`CAT-16`), not a supercategory edge. Remaining are `RingedCoveringFamily`, `RingedCoveringOverlap` and `AffineOverlapRefinement` in `schemes/covering_families.py`, the finite-atlas/refinement/descent-data classes in `schemes/gluing.py`, and the geometric `CoverRefinement` record in `schemes/ringed_spaces.py`. Their current implementation-class/getattr routing is part of the missing concrete atlas/refinement/descent owners, not a local probe-removal exercise; they must construct through those owners rather than allocating host records.
  **Owners:** `CoveringFamilies`, `DescentDataOnCover` (`presheaves.py`); the Zariski coverage on `Sch_R/X`.
  **Deliver:** the atlas, refinement and finite-atlas descent-data categories declared into the covering/descent owners; an actual represented finite-atlas site/descent object for finite-atlas sheaves; the gluing classes built through them.
...
- [ ] **`owned-provenance-data`**. **Needs:** `objects-through-categories-scheme-gluing`, `objects-through-categories-divisors`, `objects-through-categories-rings`, `objects-through-categories-modules`, `objects-through-categories-algebras`, `objects-through-categories-lattice-files`, `objects-through-categories-functions-sets-tensors`.
