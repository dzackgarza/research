...
- [ ] **`closure-verification-2026-09-16`**. **Needs:** none.
  ...
  **Observed gap:** eight nodes were checked that day by commits with empty bodies (`e0f3aaf8`, `93483130`, `04e2e735`, `2ba4cf2d`, `1a3a5ba1`, `8d19fc96`, `d8128029`, `0d458d7e`).  Two acceptances are falsified by count at `HEAD`: `owned-provenance-data` (side-attribute assignments `x._preamble_* = ` numbered 327 before and after its closing commit, 326 now, in 62 files) and `membership-by-placement` (29 `__contains__` bodies still decide by `isinstance`).  The other six are unverified, not known to hold.
...
- [ ] **`runtime-root-convergence`**. **Needs:** none.
  **Goal:** Only the owned root realizes `Parent`/`Element` and performs non-cooperative initialization; every other object is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory.
  **Observed gap:** 198 classes in 66 files subclass `Parent`, `SageObject` or `_OwnedRingParent` directly, and 64 sites in 35 files call `Parent.__init__` from a non-root level.  By subtree: schemes 67 classes and 16 calls (`schemes/gluing.py` 24 classes), group 17 and 8, divisors 14, abstract categories 13, rings 11 and 6, modules 7 and 4, algebras 4 and 8; the lattice files under `categories/` hold 40 more.  These objects are placed by a `category=` argument, not built through a construction chain, which is the same misstep as the algebra copy at the level of the runtime root.  Related tells in the same files: 869 `isinstance` sites (120 files), 317 `getattr` (91 files), 500 `try:` blocks (111 files), 47 sites routing on membership in a `WithChosen` category (24 files).
  **Owners:** `OwnedParent` and the owned root runtime (`owned_category.py`); each category whose objects are today hand-assembled.
  **Deliver:** per subtree, the hand-assembled parents replaced by objects constructed through their category's entry; the counts above become zero as each subtree lands.
  **Acceptance:** no class outside the owned root subclasses `Parent` or `SageObject`; no `Parent.__init__` call outside the root; `just category-graph` and the obligations sweep construct every object through a category entry.
...
- [ ] **`architecture-remediation`**. **Needs:** `mor-spelling-convergence`, `group-objects-construction`, `cat-valued-placement`, `membership-by-placement`, `sheaf-descent-subcategory`, `sheaf-object-placement`, `algebras-are-modules`, `algebra-structure-morphism-constructor`, `engine-algebras-through-the-structure-constructor`, `structured-module-constructors-route-through-the-datum`, `closure-verification-2026-09-16`, `runtime-root-convergence`, `arrow-category-placement`, `geometric-space-placement`, `combinatorial-object-placement`, `owner-api-convergence`, `framing-primary-epi`, `framing-specialization-convergence`, `generator-lexicon`, `ambiguous-generator-names`, `owned-provenance-data`, `refinement-convergence`, `assertion-frontiers`, `placeholder-stubs`, `categorical-representation-convergence`, `group-module-scalar-change-convergence`, `memoization-convergence`, `singular-kernel-delegation`, `torsion-action-delegation`, `imperative-algorithm-cleanup`, `owned-product-codomains`, `mathematical-return-types`, `coordinate-firewall`, `canonical-notebook-contract`, `ownership-test-contract`.
