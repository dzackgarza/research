...
- [ ] **`triage-wrong-answers`**. **Needs:** none.
  **Owner and delta:** each operation below returns a false value instead of the mathematics. A wrong answer outranks every failure that raises. Each item closes with its test green.
  ...
  - A set literal lowers to an owned finite *ordered* set, and `FiniteOrderedSets.__eq__` compares order, so `{x, -x} == {-x, x}` is False for points with no order (orderable points are sorted, so `{1, 2} == {2, 1}` holds).  An ordered set is a pair `(X, R)` with `R` a subobject of `X x X`, so its equality is equality of both, which in general cannot be decided extensionally and follows how the engines decide equality of sets (identity of unique constructions, comparison of defining predicates, exhaustive comparison only for small finite data) (owner ruling 2026-09-25).  A set literal denotes a set, with no `R`, and is compared as one. `tests/rings/test_number_field_arithmetic_archive.sage::test_gaussian_primitive_element_has_two_owned_embedding_images`.
...
- [ ] **`rings-are-framed-algebras-over-themselves`**. **Needs:** none.
  **Owner and delta:** `Algebras(R).Associative().Unital().Framed()` in `categories/algebras/algebras.py`. A commutative ring `R` is an `R`-algebra framed by the empty generating set: the unit map from the free algebra on no generators, `R -> R`, is surjective. The ring constructors (`_own_ring` and the order and field routes that build `ZZ`, `QQ`, `RR`) establish that framing, so `R` is placed in that category with the framing datum it carries.
  **Observed:** `ZZ` is not in `Algebras(ZZ).Associative().Unital().Framed()` on `main` or on rack's head (`72650e04f`). Sage's `_meet_` of `ZZ.category()` and `RR.category()` still produces a join containing that category, so `RR.coerce_map_from(ZZ)` raises `ValueError: Integer Ring is not in Join of ... associative unital framed algebras ...` inside `DefaultConvertMap_unique`. Each of `4 / pi**2`, collection of `tests/functions/test_young_convolution.sage`, and `tests/lattices/test_d4_metric_invariants.sage::test_d4_successive_minima_hermite_invariant_and_gaussian_heuristic` hits it. It came from rack's separation of algebra framing from module framing (`09215fd7e`), and it was not present before that separation reached `main`.
  **Observed when placing the framing (2026-09-25):** fixing the empty framing in `_algebra_from_native_ring` for a ring over itself places `ZZ` and `QQ` correctly, and `QQ.coerce_map_from(ZZ)` then fails: Sage's meet builds a join in which `Modules(ZZ)` carries `Framed`, and `QQ` is refused. The join states a theorem: every `R`-module `M` is framed by the counit `Free_R(U(M)) -> M` of the free/forgetful adjunction, which is surjective, so `QQ` is a framed `ZZ`-module (not a finitely framed one). The defect is that membership in `C.Framed()` is decided by a stored datum, so an object with no explicitly chosen framing is refused although its canonical counit framing exists. Frameable and framed are different notions. The framed objects of `C` are pairs `(X, Fr^•(X))`; frameability is a property that depends on which framings count. The notion must recover the framed manifold (nLab *framed manifold*, *G-structure*): a framing of `M` is a trivialization of `TM`, a section of the frame bundle, the `G = {e}` case of a G-structure, i.e. a lift of the classifying map through `* -> BGL(n)`. Through Serre-Swan that is a basis of the module of vector fields `Der(C^∞(M))`: a framing that is an isomorphism from a free module; a stable framing is an isomorphism `Der(C^∞(M)) (+) R^k = R^(n+k)`. An epimorphism `Free(S) ->> X` is the degree-0 stage of a free resolution, and a resolution (cofibrant replacement) is an equivalence from a degreewise free object, the homotopical form of a trivialization. A basis is the resolution concentrated in degree 0; the bar resolution of the free/forgetful comonad always exists. So `Framed` is not an axiom: frameability conditions (free, stably free, finite through degree n) are relative properties, and the framed objects are the category of pairs. A meet by intersection of declared supercategories was also tried; its Hom is the owned ring Mor, whose `homset_category()` is Sage's `Sets()`, which owned objects are not in (owned `Sets` declares `Objects()` only, contrary to the `owned_category.py` module docstring).
  **Closure:** `ZZ in Algebras(ZZ).Associative().Unital().Framed()` with an empty `algebra_generating_set()`, and `4 / pi**2` evaluates in a session.
