...
- [ ] **`triage-mor-sets-and-groups-are-sets`**. **Needs:** none.
  **Site:** `categories/sets/set_categories.py`, `SetMorCategory`: `Sets().Mor(X, Y)` is placed in Sage's "endsets of sets" category, not in the owned `Sets()`, so it has no cardinality and is refused wherever a set is required ("a map of sets needs a set as domain"; `group/g_objects.py`, `action_functor`).  ARC-07 makes `Mor(X, Y)` a category and a set the discrete one, so `Mor_Set(X, Y)` is the set `Y^X`; the tree also owns `Y^X` as `FunctionSets`, whose `mor()` returns this homset.  Which of the two carries the set structure is an owner decision before implementation.  Blocks every `FiniteGSets(G)(points, action)`, every `AffineGSchemes` action, descent equalizers and naturality squares.
  **Example:** `tests/schemes/test_glued_invariant_quotients_of_glued_lines.sage::test_negation_is_an_involution_of_the_line`.

- [ ] **`endomorphism-algebra-construction-cascade`**. **Needs:** none.
  **Site:** `categories/modules/module_morphisms/module_morphisms.py`, `_initialize_module_mor_parent`: a Mor parent between presented modules computes its presented internal-Hom model when it is constructed, and an End parent also builds its algebra structure then (`_algebra_from_native_ring`), whose multiplication `End ⊗ End -> End` is constructed through another presented Mor parent, and so on.  Measured 2026-09-25 on `C = coker(ZZ -> ZZ^n, 1 |-> 2e_0) = Z/2 + Z^(n-1)`: building `C.Mor(C)` takes 0.42 s at n = 1, 2.76 s at n = 2, 329 s at n = 3.  Every composition of maps of presented modules builds such a parent, so a saturation on the rank-4 lattice `U + U` does not finish.  Deciding when the model and the algebra structure are computed is an owner decision (AGENTS.md: a cache or laziness on a hot path is a design change to discuss).
  **Example:** `tests/lattices/test_lattice_semantic_api.sage::test_the_isotropic_reduction_of_two_U_along_a_primitive_isotropic_line_is_U`.

- [ ] **`undeclared-functors-are-not-unfaithful`**. **Needs:** none.
  **Site:** `categories/functors/core.py`, `Functor._faithful = False`, read by `is_faithful()`.  Every functor that does not declare faithfulness answers False, a claim of non-faithfulness nobody made.  An undeclared functor answers that faithfulness is not decided, not False; each functor that is faithful declares it with its proof, as the power-set functors do.
...
- [ ] **`triage-wrong-answers`**. **Needs:** none.
  **Owner and delta:** each operation below returns a false value instead of the mathematics. A wrong answer outranks every failure that raises. Each item closes with its test green.
  ...
  - A set literal lowers to an owned finite *ordered* set, and `FiniteOrderedSets.__eq__` compares order, so `{x, -x} == {-x, x}` is False for points with no order (orderable points are sorted, so `{1, 2} == {2, 1}` holds).  Whether `==` on ordered sets means equality of the sets or of the ordered sets is an owner decision; a set literal denotes a set either way. `tests/rings/test_number_field_arithmetic_archive.sage::test_gaussian_primitive_element_has_two_owned_embedding_images`.
  - `Lp(1)(sin(x))`, `Lp(2)(exp(x))` and `ell(1)(n)` are accepted as elements. `tests/functions/test_lebesgue_spaces_on_the_line.sage`.
  - `diagonal_gram({0: -1})` gave `e_0^2 = 1` while integer keys hashed apart from their values; recheck after f0033cb5b. `tests/lattices/test_lattice_construction_routes.sage::test_a_diagonal_gram_changes_only_its_stated_entry`.
