...
- [ ] **`engine-algebras-through-the-structure-constructor`**. **Needs:** `algebra-structure-morphism-constructor`.
  ...
  **Deliver:** each engine-backed algebra route computing \((A, \rho)\) and feeding the constructor; the hand-written module operations and the algebra-level `cardinality` deleted.
  **Acceptance:** `Modules(R).symmetric_algebra()(M)` and `R.polynomial_ring(names)` answer `module_generating_set`, `module_generator` and `scalar_multiple` by inheritance from the module the constructor built; no operation `Modules(R)` provides is defined in the algebra subtree.

- [ ] **`structured-module-constructors-route-through-the-datum`**. **Needs:** none.
  **Goal:** `Modules(R[G])(M, rho)`, `FormModules(R)(M, b)` and the lattice constructors are the one entry of their categories (`CON-16`): the object is built on the data of `M`, `M` is returned by `unformed_module()` as the specification writes it, elements pass between them by coercion, and no identification map back to `M` is public.
  **Observed gap:** the same shape as the algebra copy at three more sites.  Group modules expose `forget_action_morphism`, `equip_action_morphism` and `unacted_module` (`modules/group_modules/group_modules.py:530-560`); formed modules expose `forget_form_morphism` and `equip_form_morphism` built from `_form_module_construction` (`modules/framed/formed/form_modules.py:1000-1030`); lattices store `_module` and expose `forget_form_morphism` (`lattices.py:1105-1130`), against the always-on rule that no `forget_*` method exists at any level.  Six `*_source_module` accessors (completion, localization, restricted scalars, free algebras) name a functor's preimage by an implementation word where `CON-05` asks for the functor-image object.  None of these arrived with a recorded reason: `equip_action_morphism` and `equip_form_morphism` first appear in the megadoc-generator commit of 2026-09-01, `_form_module_construction` in `351f8c38` with an empty body.
  **Owners:** `Modules(R[G])`, `FormModules(R)`, `Lattices(R)`; the completion, localization and restriction functors for their provenance.
  **Deliver:** the identification maps and the stored lower-level modules deleted; every consumer that transported data along them states its data on the structured object; the `*_source_module` accessors replaced by the functor-image provenance `CON-05` names.
  **Acceptance:** no `equip_*`/`forget_*` morphism and no `*_source_module` accessor in the tree; `form.unformed_module() is module` and `Modules(R[G])(M, rho)` answer as the specification writes them; one constructor entry per category with every convenience routing into it.
...
- [ ] **`glued-sheaves-through-the-sheaf-entry`**. **Needs:** none.
  ...
  **Observed gap:** the 2026-09-16 closure of `sheaf-descent-subcategory` (`2ba4cf2d`, empty body) was re-verified on 2026-09-17 and does not hold.  The definition exists (`Sheaves` declares `[presheaf_category()]`, `DescentEqualizer`, `DescentData`, and the entry `Sheaves.object`, `abstract_categories/presheaves.py:624-1062`), but `Sheaves.object` has no caller.  `glue_modules` returns `ModuleGluingDatum` (`schemes/ringed_spaces.py:676-681`), placed under `DescentDataOnCover` which declares `[Objects()]` (`presheaves.py:558-559`).  `GluedModuleSheaf`, `GluedAlgebraSheaf` and `StructureSheaf` place themselves by `Parent.__init__(category=Cat().meet(...))` (`gluing.py:4397-4416, 4860`; `ringed_spaces.py:192`), which `Sheaves.__contains__` accepts by `is_subcategory`; the asserted value category is `Sets()` for a sheaf of modules.  `SheafObjects(X)`, `SheafedSpaces`, `DescentDataOnCover` and `CoveringFamilies` declare `[Objects()]`.
  **Owners:** `Sheaves`, `DescentDataOnCover`, `CoveringFamilies` (`presheaves.py`); `SheafObjects`, `ModuleSheaves`, `DistinguishedAffineCover` (`ringed_spaces.py`); the glued sheaf classes in `gluing.py`.
  **Deliver:** `SheafObjects(X)` declared into `Sheaves` for the Zariski coverage on `X` with the module category as value category; the three self-placed sheaf classes built through `Sheaves.object` with their `DescentData`; `glue_modules` returning the sheaf; `DescentDataOnCover` and `CoveringFamilies` declared into the categories their definitions name.
...
- [ ] **`objects-through-categories-scheme-gluing`**. **Needs:** none.
  ...
  **Observed gap:** `schemes/gluing.py` alone: 24 classes on `Parent`/`SageObject`, 9 non-root `Parent.__init__`, 20 side attributes, 66 `isinstance`, and 2 `__contains__` deciding by `isinstance`; the gluing data, transition functions and glued sheaves are assembled by hand and placed by a `category=` argument.  The descent categories delivered under `sheaf-descent-subcategory` place them; the objects themselves are still not constructed through those categories.
  **Owners:** the descent-data and sheaf categories from `sheaf-descent-subcategory`; `Modules(R)` for the local modules; the owned root runtime (`owned_category.py`).
  ...
  **Acceptance:** the counts in the gap statement are zero for `schemes/gluing.py`; every object there is constructed by its category's entry; `just category-graph audit` reports no hand-assembled parent in it.
...
- [ ] **`objects-through-categories-divisors`**. **Needs:** none.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
  **Observed gap:** 14 classes on `Parent`/`SageObject` (`divisors/invertible_sheaves.py` 5, `linearizations.py` 4, `general_divisors.py` 3, `chow_groups.py` 2), 2 non-root `Parent.__init__`, 37 `isinstance`, 18 `getattr`, 6 `try:`; fourteen private `_*Construction` argument packages in this subtree alone.
  **Owners:** the divisor, Picard, class-group, section-ring and linear-system categories; the owned root runtime.
  **Deliver:** Per file, each class on `Parent`/`SageObject`/`_OwnedRingParent` becomes an object of its category built by that category's entry; each non-root `Parent.__init__` becomes cooperative `super().__init__`; each `x._preamble_* = ` assignment becomes a constructor datum or is deleted as derivable; each `try:` becomes a `case`/`match` on categorical containment or an asserted frontier; each `isinstance`/`getattr`/`hasattr`/`__dict__` probe outside `_element_constructor_`, `__contains__`, `__eq__` and a declared engine adapter (`OWN-06`) becomes a placement question.
  **Acceptance:** the counts in the gap statement are zero for `divisors/`; every object there is constructed by its category's entry; `just category-graph audit` reports no hand-assembled parent in it.
...
- [ ] **`objects-through-categories-modules`**. **Needs:** `structured-module-constructors-route-through-the-datum`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`objects-through-categories-lattice-files`**. **Needs:** `structured-module-constructors-route-through-the-datum`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`construction-datum-classification`**. **Needs:** none.
  ...
  **Observed gap:** 57 such classes in 35 files (`group/predicate_subgroups.py` 6, `divisors/linear_systems.py` 6, `schemes/geometric_cohomology.py` 5).  First specimens on each side: `_ChosenAlgebraMultiplicationDatum` (a package around `(M, m)`, deleted by `algebra-structure-morphism-constructor`) and `_GroupModuleConstruction`/`_FormModuleConstruction` (retained inputs behind identification maps, handled by `structured-module-constructors-route-through-the-datum`).
...
- [ ] **`banned-language-residue`**. **Needs:** none.
  ...
  **Observed gap:** `carrier` at 30 sites in 8 files (`abstract_categories/arrow_categories.py` 17); 2 public `.Hom(` spellings outside engine adapters; the `*_source_module` accessors are handled by `structured-module-constructors-route-through-the-datum`.
