...
| C | `e472b85a` unifies ordinary-parent and fixed-Hom construction with nested refinement hooks. `78494432` preserves the selected scalar ring of endomorphism algebras. `a3017a8f` gives exact-carrier `EndofunctorAlgebras(T)=Inserter(T,Id)`. `3e3e443d` removes inherited Hom endpoint reprobes and reconciles `SubobjectCategory` with the owned runtime. `3e9f7f3e` publishes generic functor, contravariant/bifunctor, adjunction, and restricted-Hom obligations through Sage abstract-method descriptors. `0a4de522` does the same for enumerated-set ranking. | State mathematical parameter/result types on the surviving generic abstract-category and functor constructors so Arrow/Slice/opposite/product/diagram/discrete/functor constructions no longer erase what their inputs are. Stable category ordering retains its external O/S/session prerequisite. | Claimed: `C-abstract-signatures-20260907-1630`; broader C open |
