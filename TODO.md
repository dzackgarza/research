...
- [ ] **`membership-by-placement`**. **Needs:** none.
  ...
  **Unresolved induced transport:** `functors/mor_packets.py` still tests the raw image arrow as an object of the target fixed Mor in `_InducedMorFunctor.object_image`. Follow the common induced Mor/End/Aut route through arrow admission and the target's object construction; preserve enriched element behavior and identity 2-arrows, and distinguish an unwrapped arrow from its constructed object in an unenriched fixed category (COMPLAINTS 22).
