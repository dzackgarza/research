...
- [ ] **`triage-wrong-answers`**. **Needs:** none.
  **Owner and delta:** each operation below returns a false value instead of the mathematics. A wrong answer outranks every failure that raises. Each item closes with its test green.
  ...
  - The Cartier datum of a line on `P^2` is 0, not `e_1^*`, so a line is reported not ample. `tests/schemes/test_toric_cartier_data_and_linear_systems.sage::test_a_line_on_the_projective_plane_is_basepoint_free_and_ample`. Specimen after the 2026-09-25 rack merge, where the enumeration order of close vectors changed: `tests/lattices/test_close_vectors_archive.sage::test_the_points_of_rank_one_lattice_two_within_norm_one_of_one_half_are_zero_and_e` (`{e_0, 0} == {0, e_0}` is False).
