...
- [ ] **`algebra-structure-morphism-constructor`**. **Needs:** none.
  ...
  **Remaining:** Finish native module-and-product construction in `_OwnedRingParent`, `_OwnedAlgebraParent`, predicate subrings, exact real scalars, localizations and the sparse-free/power/cohomology/restricted-graded algebra engines. `_initialize_engine_algebra` still reaches `_install_multiplication`, which sets `unformed_module` to the object itself and records a native binary rule instead of constructing its module and feeding that module with its tensor classifier to the one entry. The integer/self-scalar bootstrap must retain the exact scalar object and identity morphism without becoming an exemption. Explicit central ring maps and the pointwise Lebesgue algebra now use the module and tensor owners; preserve those routes. The convolution route still allocates `_LebesgueAlgebraFromMultiplication` and `GradedTensorSquare` locally. Its asserted total-algebra domain is false: the constant one lies in its L-infinity summand but its convolution with itself diverges, and `UnitInterval` has only a partial operation s+t-1. Repair the full Young-domain operation and its almost-everywhere semantics at the function/module owner, retaining all admissible exponents and existing convolution computations; neither restricting the goal to L1 nor assigning zero to inadmissible products discharges this obligation. See the algebra complaint for the counterexample and affected paths. The original one-entry acceptance below remains in force.
