...
- [ ] **`module-construction-knot`**. **Needs:** none.
  **Owner and delta:** the common boundary of `modules/pure/modules.py`, `modules/module_morphisms/module_morphisms.py` and framed/internal morphism-space construction: settle admission and the fixed framing datum together before the separate family migrations consume either contract.
  **Invariants:** trace module datum -> free framing source and images -> morphism admission -> enriched Mor construction. Fix endpoints, scalar action, relations and spanning/lift hypotheses before exposure; realizing an arrow cannot demand an infinite tower of its own framed Mor. Its linearity cannot be justified by membership that the same admission is meant to establish. Retain the existing lazy realization where it satisfies this contract (`OWN-22`, `OWN-23`). The output is this functioning construction and its declaration-side authority, not a new datum wrapper or a design report.
  **First consumers and closure:** a product projection and an equalizer inclusion/factorizer use that same owner with their actual construction-derived laws. An arbitrary callback cannot select that derivation by passing a label. A relationful module, an unframed product and a nested Mor space retain the same sources and maps when addition, generators and framing are requested in different orders. Follow the complete constructor calls to the already-fixed data that terminate them; bank order-variation specimens for T. Migrate every caller invalidated by this shared change in this unit; the later admission and framing nodes cover independently remaining routes.
...
- [ ] **`morphism-admission`**. **Needs:** `module-construction-knot`.
...
- [ ] **`framing-presentation-data`**. **Needs:** `module-construction-knot`, `base-ring-bootstrap`.
...
- [ ] **`group-module-data`**. **Needs:** `algebra-defining-data`, `module-scalar-change`, `formed-module-data`.
...
- [ ] **`objects-through-categories-rings`**. **Needs:** `base-ring-bootstrap`, `localization-completion-threading`.
  ...
  **Coverage boundary:** enumerate the classes and construction/raising routes in that source population, subtract only unchanged routes whose earlier delivery proves this obligation, and repair the remaining routes. Constructor defects used by an earlier node already block that node. This family reconciliation does not postpone them or rebuild its delivered rings.
...
- [ ] **`objects-through-categories-algebras`**. **Needs:** `free-algebra-module-factor`.
  ...
  **Coverage boundary:** reconcile every constructor and raising route in that subtree against the original algebra requirement. Earlier algebra repairs include their own private boundaries, placement and consumers; this node owns independently remaining cases, not permission to defer defects in those repairs.
...
- [ ] **`projectivization`**. **Needs:** `sheaf-descent-threading`, `free-algebra-module-factor`.
  ...
  **Closure specimens:** a rank-two bundle on a two-chart scheme with a nonconstant transition recovers the induced projective gluing; the trivial bundle agrees with relative projective space. Over A=k[x], F=A/(x) direct-sum A gives Proj(A[u,v]/(x*u)), with fibre P^1 at x=0 and P^0 away from zero. Retain its relation, universal quotient and both base-change maps; a locally-free-only implementation or a rank-based projective-space factory fails this specimen. An arbitrary map of sheaves need not induce an everywhere-defined map of projectivizations: restrict to the locus where the induced quotient is surjective, retaining that open immersion.
...
- [ ] **`geometric-consumer-integration`**. **Needs:** `projectivization`, `localization-completion-threading`, `graded-complex-connection-data`, `involution-primitive-extension`.
