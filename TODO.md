...
- [ ] **`glued-sheaves-through-the-sheaf-entry`**. **Needs:** none.
  ...
  **Remaining:** Distinguished-cover module and algebra gluing return canonical `Sheaves.object`s with private realizations retaining their chosen descent data. The affine structure sheaf now has its actual module-valued presheaf on the represented affine slice, every represented distinguished cover supplies a cached rank-one module descent datum and canonical Čech sheaf, and `X.structure_sheaf()` is the sole construction surface for a private realization through the owned algebra/quasi-coherent sheaf refinements. What remains is the genuine arbitrary-cover comparison identifying that affine-site presheaf with the distinguished-cover equalizers before the structure sheaf itself can enter `Sheaves.object`; finite-atlas sheaf realizations still need an actual represented site/descent object. `SheafObjects(X)` cannot simply be declared module-valued wholesale: integral local systems and higher direct images also use it with different value theories, so those consumers must be split into their genuine sheaf categories rather than forcing a false graph edge. `CoveringFamilies(C)` is structured finite families of arrows with chosen overlap spans but the tree still has no category of indexed families carrying that structure; `DescentDataOnCover` likewise has only its concrete module/algebra descent subcategories. Resolve those actual object categories rather than replacing `Objects()` by a label.
