...
- [ ] **`suite-within-time-gates`**. **Needs:** `mor-construction-computes-no-presentation`, `small-specimen-tests`.
  ...
  **Observed (2026-09-26):** in `test_categories_inhabited.sage`, six geometric categories account for about 220 of 280 s of test time in the witness check (`test_a_category_over_a_ring_is_inhabited_over_every_ring`, 2,290 runs measured): `ToricFixedPointBlowups` (~7 s per ring, 30 s timeout over `AA`), `ADELogPairs`, `LogPairs`, `ProjectiveCompleteIntersections`, `ToricLogPairs` and `ToricSchemes` (1-2 s per ring). Their `an_object()` builds a full geometric construction for each of 14 base rings, where a witness needs only the smallest object of the category; the other ~300 categories answer in milliseconds.
