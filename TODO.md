...
- [ ] **`dual-lattice-through-the-discriminant-sequence`**. **Needs:** none.
  **Owner and delta:** the dual of a `W`-valued bilinear module `(L, b)` is reached through the exact sequences `0 -> Rad_i(L) -> L -> L^# -> A_L -> 0`, `i = 1, 2` for the left and right radicals (one radical when `b` is symmetric or skew). `L^# := L^{#R}`, where `L^{#S}` for a submodule `S <= W` is the `S`-dual of the `W`-valued module. There is an explicit map `L^# -> L^*`, and the form on `L^*` is the one transported along it; the map `L -> L^*` is the composite `L -> L^# -> L^*`. No direct map `M -> Hom_R(M, R)` is used, since none exists in general. `Lattices.dual_lattice` (`categories/lattices.py`) instead equips the dual module with a `QQ`-valued form and refines it into `FormModules(ZZ).Nondegenerate()`, whose predicate asks for a correlation `M -> Hom_R(M, R)` (`categories/modules/hodge.py`, `_algebraic_correlation_morphism`) defined only for `W = R`.
  **Invariants:** left and right nondegeneracy are `Rad_1(L) = 0` and `Rad_2(L) = 0` as modules (the form on a radical is zero), not injectivity of a map into `Hom_R(L, R)`. `A_L` is the cokernel of `L -> L^#`. The `W = R` case is recovered, not special-cased.
