...
- [ ] **`discrete-category-universal-constructions`**. **Needs:** none.
  **Owner and delta:** `categories/abstract_categories/functors.py`, on `DiscreteCategory`. Realize finite products, coproducts, equalizers, coequalizers and nonempty wide (co)equalizers exactly when the universal cone/cocone exists in the discrete category, with the forced identity structure maps and factorization; report mathematical nonexistence separately from missing computation. In particular the one-object discrete category is the terminal category, so every represented finite diagram into it has the unique object as both limit and colimit. This must feed the existing generic `Limits`/`Colimits` and their functors rather than adding a parallel diagram algorithm.
  **Closure specimens:** in `Disc({*})`, binary products/coproducts, equalizer/coequalizer of the identity, a nonempty identity family, pullback/pushout of the identity, and a constant diagram over a finite shape all select `*` with identity structure maps; in `Disc({p,q})`, the product/coproduct of `p,q` is correctly reported nonexistent rather than as an implementation failure. Execution remains under terminal T.
- [ ] **`discrete-fixed-mor-universal-constructions`**. **Needs:** `discrete-category-universal-constructions`.
  **Owner and delta:** `categories/abstract_categories/mor_categories.py`, on fixed Mor categories whose selected 2-Mor family is `_DiscreteTwoMorCategoryOf`. Reuse the discrete-category universal-construction authority for their represented arrow objects: finite products, coproducts, (wide) equalizers/coequalizers, pullbacks and pushouts exist exactly by the same equality criterion and carry the forced identity 2-morphisms. Do not classify `MorCategories()` itself as discrete: `_FunctorCategory` overrides the 2-Mor with natural transformations and remains a genuine non-discrete fixed Mor category.
  **Closure specimens:** `End_Set(*)` has one represented arrow and every finite universal construction and constant finite diagram selects that arrow with identity 2-morphisms; a fixed Mor with two distinct represented arrows reports the product/coproduct of those distinct objects nonexistent; a functor category continues to use natural transformations rather than this discrete route. Execution remains under terminal T.
- [ ] **`set-wide-equalizers-and-coequalizers`**. **Needs:** none.
  **Owner and delta:** `categories/sets/set_categories.py`, through the common `Cat.ParentMethods.equalizer_of_family` / `coequalizer_of_family` contract. For an indexed family of parallel set maps `f_i : X -> Y`, realize the wide equalizer as the subobject of `X` where all `f_i` agree and the wide coequalizer as the quotient of `Y` by the equivalence relation generated by `f_i(x) ~ f_j(x)`, retaining the universal inclusion/projection and factorization. The public common operations already exist; `Sets` currently supplies only the binary `_categorical_equalizer` / `_categorical_coequalizer`, while source search finds the family realization only for modules. Preserve arbitrary represented families as the mathematical input and state the effective computational frontier rather than changing the object to a finite prefix.
  **Closure specimens:** the family `(n mod 3, 0) : {0,...,5} -> {0,1,2}` has wide equalizer `{0,3}`; the family `(n, n+1) : {0,1} -> {0,1,2,3}` has a two-point wide coequalizer with `0,1,2` identified and `3` separate; a three-map family distinguishes the wide construction from a binary-only alias. Each construction retains and satisfies its universal map. Execution remains under terminal T.
...
- [ ] **`architecture-remediation`**. **Needs:** `engine-wiring-audit`, `dual-lattice-through-the-discriminant-sequence`, `discrete-category-universal-constructions`, `discrete-fixed-mor-universal-constructions`, `set-wide-equalizers-and-coequalizers`.
