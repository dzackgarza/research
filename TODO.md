...
- [ ] **`orthogonal-reduction-sequences`**. **Needs:** none.
  **Owner and delta:** for a nondegenerate lattice `L` over `R` with `K = Frac(R)` and `L_K = L tensor_R K`, the preamble constructs two exact sequences of groups, each an owned object with the kernel inclusion, the named morphism and the cokernel projection. (1) The *discriminant reduction sequence* `1 -> K_L -> O(L) -f-> O(A_L) -> C_L -> 1`, where `f` is the *discriminant reduction morphism* (the functorial map now spelled `discriminant_representation`, `categories/lattices.py`), `K_L = ker f` is the *discriminant reduction kernel*, written `\tilde O(L)` in prose, and `C_L = O(A_L)/f(O(L))` is the *discriminant reduction cokernel*. (2) The *`K`-spinor norm sequence* `1 -> K_{sn_K} -> O(L_K) -sn_K-> K^x/(K^x)^2 -> C_{sn_K} -> 1`, where `sn_K(s_{v_1}...s_{v_m}) = prod -(v_i, v_i)/2` in the square-class group (Kneser, recorded in Gritsenko--Hulek--Sankaran; cite from the source, not from this row). `sn_K` needs an owned square-class group `K^x/(K^x)^2` and the group `O(L_K)` of the base-changed lattice. `K_{sn_K}` is the *`K`-spinor kernel* (rational for `K = QQ`), and `K_{sn_K} cap O(L)` is the *`R`-spinor kernel* (integral). Prose follows the paper: `O_{sn_K}(L) := O(L) cap ker sn_K`, so `O^+(L) = O_{sn_RR}(L)`, and `SO_{sn_QQ}(L) = SO(L) cap ker sn_QQ` is the spinorial kernel `O'(L)`; `\tilde O^+`, `SO^+` and `\tilde SO^+` are the corresponding intersections. The spinor-norm convention (`-(v,v)/2` versus the norm of `-b`) is a stated parameter of the morphism, not a constant hidden in `real_spinor_norm_sign` (`categories/lattice_morphisms.py`). The present `Lattices.O_plus`, which returns `\tilde O(L)`, is replaced by the paper's meaning, and each caller is moved to the name of the group it means. The base-changed quadratic space `L_K` also answers its *Witt index*: the dimension of a maximal totally isotropic subspace of `L_K`, obtained through the Witt decomposition `L_K = H^i + (anisotropic)` and computed privately through OSCAR's `witt_index` for quadratic spaces; `L` answers it through `L_K`. The OSCAR adapter (`categories/lattice_engines.py`) returns the whole square class of the rational spinor norm, not only its sign.
  **Closure specimens:** for `A_1` the discriminant reduction cokernel is trivial and `\tilde O(A_1)` has index 2 in `O(A_1)`. For `U + A_1(-1)` (signature `(2,1)` in the `(2,n)` convention), `sn_QQ` of the reflection in a vector `v` with `(v,v) = -2` is the class of `1`; its `sn_RR` is `+1`, so the reflection lies in `O^+`. `SO_{sn_QQ}(L)` is contained in `SO(L)` for this lattice. `U + U` has Witt index 2 over `QQ`, `U + A_1(-1)` has Witt index 1, and `E_8` has Witt index 0.
