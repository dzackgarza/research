...
- [ ] **`objects-through-categories-scheme-gluing`**. **Needs:** `objects-through-categories-functions-sets-tensors`.
  ...
  **Remaining:** The glued scheme now uses Schemes(R) with a private realization and the shared Hom/structure-map entry, module transition maps are arrows of the varying-ring module category, finite affine atlases/refinements and their module/algebra descent data are built through the covering/descent owners, finite-atlas inverse image and pullback are owned functors, the actual module of compatible sections is the selected `Modules(O(X))` equalizer, and compatible algebra sections are built on that module through its owned tensor square and the `Algebras(R)` entry; the former `GlobalSectionModules` and `GlobalSectionAlgebras` alternate owners are gone. Remaining is `QuasiCoherentSheavesWithChosenDescentDatum` through its genuine sheaf entry and the subtree-wide host/probe cleanup required by the acceptance below. Retain the descent interpretation needed to validate cyclic-cover branch sections, not just component arithmetic. Sheaves must use Sheaves.object, and literal ingress belongs to the indexed-family owner.
  **Owners:** the `Schemes(R)` glued-scheme entry; the `Modules(R)` tensor construction; `QuasiCoherentSheaves(X)`.
  **Deliver:** the glued scheme built by its `Schemes(R)` entry on the gluing datum; the categories `GluedSchemes`, `GlobalSectionModules`, `GlobalSectionAlgebras` and `QuasiCoherentSheavesWithChosenDescentDatum` removed, with their engines kept and their objects placed in `Schemes(R)`, `Modules(O(X))`, the algebra category over \(\mathcal{O}(X)\) and `QuasiCoherentSheaves(X)`.
...
- [ ] **`cartier-divisors-as-sections`**. **Needs:** none.
