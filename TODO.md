...
- [ ] **`test-suite-mathematical-assertions`**. **Needs:** none.
  **Owner and delta:** the tests outside `tests/constructions/`, `tests/user_simulations/` and `tests/conftest.py` (which are protected specifications and are never edited to match an implementation).
  **Invariants:** each test is an independently verifiable mathematical fact: it constructs its objects through their public mathematical entries and asserts what the objects are and compute -- invariants, maps, equalities of mathematical objects, results that a source states. It does not import underscored names, read private attributes, assert through `repr`/`str`, check classes or types, reach engine objects, or compare a `category()` by identity. Follow `test-guidelines`: a non-compliant test is replaced by the mathematical claim it was reaching for, or deleted with the claim named as proved elsewhere or recorded as debt, never patched to pass.
  **Observed:** see [COMPLAINTS.md](COMPLAINTS.md#the-test-suite-collapses-under-refactoring-instead-of-stating-mathematical-facts) for the 2026-09-23 counts (151 of 668 files import private names; 4% of the triage failures are assertion failures).
  **Closure:** a scan of the non-protected tests finds none of the patterns above; an internal refactor that keeps the mathematics (for example, renaming a private helper) leaves them green.
