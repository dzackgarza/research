...
- [ ] **`objects-through-categories-divisors`**. **Needs:** none.
  ...
  **Deliver:** the three linear-system packages dissolved into data threaded by the projective-space entry; `linearizations.py`'s classes built by their categories; `__init__.py` importing its submodules eagerly.
