...
- [ ] **`arrow-category-is-the-functor-category`**. **Needs:** none.
  ...
  **Remaining:** Module, lattice, subgroup and submonoid subobjects now enter their exact fixed-base subobject category by construction; their Homs derive the fixed-edge triangle from the slice owner, and containment is decided before factor construction rather than by caught exceptions. The unresolved representation is the set case: power-set elements are still represented directly by `SetInclusion` morphisms, whereas `SubobjectCategory` is a category of structured parent objects. Reconcile that one object model so a subset `A -> X` enters the fixed slice/mono construction without identifying the bare set `A` with all of its possible ambient inclusions or introducing a second arrow wrapper registry. The walking-arrow identity, natural-transformation squares and slice/coslice fixed-edge Homs remain at the single functor-category owner.
