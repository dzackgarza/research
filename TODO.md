...
- [ ] **`membership-by-placement`**. **Needs:** none.
  ...
  **Unresolved family graph:** reconcile inclusions between fixed Mor categories with membership of those categories in their classifying families. The declared `Auts(C) -> Ends(C)` and `Monos(C) -> Mors(C)` edges currently conflict with their distinct selected fixed objects (COMPLAINTS 21). Every retained family edge must preserve the category object itself; transport to a different fixed category requires the stated functor, not construction during containment.
