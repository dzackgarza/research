...
- [ ] **`suite-within-time-gates`**. **Needs:** `mor-construction-computes-no-presentation`, `small-specimen-tests`.
  ...
  **Observed (2026-09-26):** in `test_categories_inhabited.sage` the witness check (`test_a_category_over_a_ring_is_inhabited_over_every_ring`, 3,772 runs) takes 101 s of test time now that the toric, log-pair, blowup and complete-intersection witnesses are the smallest objects of their categories. `ADELogPairs` is 41 s of it: its smallest witness, type `A_1`, still costs about 1.5 s per base ring.
