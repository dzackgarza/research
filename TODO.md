...
- [ ] **`category-graph-engine`**. **Needs:** none.
  **Goal:** The declared-category-graph tool computes its graph theory with Sage, under Sage's interpreter, as `AGENTS.md` (*Reading the declarations*) requires, and the cost of Sage's routines on this graph is measured and recorded rather than asserted.
  **Observed gap:** HEAD's `category_graph.py` hand-rolls components, depth and shortcut detection and has no `cells` view; its justfile recipe runs plain `python3`.  A Sage-backed `cells` view (`Graph.minimum_cycle_basis`, `SimplicialComplex.homology`) existed uncommitted on 2026-09-16 and was replaced by a `networkx` version on an unmeasured speed claim (`ENG-07`, `DEV-62`); that version sits in the working tree, uncommitted, and is not the design.
  **Owners:** `src/dzack_research/utilities/category_graph.py`, the `category-graph` recipes, `TRAPS.md`.
  **Deliver:** the `shape` and `cells` views on Sage's `Graph`, `DiGraph` and `SimplicialComplex`, importing those modules and not `sage.all` (`TRAPS.md`, interpreter-startup row); and, before any route is chosen, the wall time of `minimum_cycle_basis`, `longest_path` and `homology` on Sage graphs of the declared graph's shape at three sizes spanning its current order, recorded in `TRAPS.md` with the `algorithm=` choices read from source.  This graph is the graph Sage walks to join and linearize the preamble's categories, so the curve is also a cost model for session import and refinement.  The tool reads source with `ast` and touches no preamble object, so the measurement does not conflict with the phase rule on `architecture-remediation`.
  **Acceptance:** `just category-graph cells` runs under the `.envrc` interpreter and prints the minimum cycle basis split into generators killed by one deleted declaration and those owing a 2-cell; the `TRAPS.md` rows exist with command, sizes and version; no graph algorithm is hand-rolled in the module and no second graph library is imported.
...
- [ ] **`presheaf-categories`**. **Needs:** none.
  **Goal:** Own the presheaf construction \(\mathrm{Presh}(C, D) := [C^{\mathrm{op}}, D]\) as a bifunctor \(\mathbf{Cat}^{\mathrm{op}} \times \mathbf{Cat} \to \mathbf{Cat}\), so that every presheaf-shaped object in the tree is an object of a functor category.
  **Observed gap:** no presheaf category exists.  Sheaves, structure sheaves, invertible sheaves and quasi-coherent sheaves are therefore plain `SageObject`s outside the category graph, and the one sheaf category that exists declares `Sets()`.
  **Owners:** the owned functor category and the opposite-category construction, both already present; the bifunctor is stated on `Cat` as a construction of its own, not as a new theory.
  **Deliver:** the construction on objects and on morphisms, with the value category \(D\) a parameter rather than fixed to \(\mathbf{Set}\) --- that parameter is what carries the later passage to sheaves of modules, of algebras, and to stacks.  The Yoneda embedding \(C \to \mathrm{Presh}(C)\) is the first specimen that distinguishes this from a category of maps.
  **Separating cases:** \(D = \mathbf{Set}\) and \(D = \mathbf{Mod}_R\) on the same small \(C\); a \(C\) that is a poset (the opens of a space) and one that is not.
  **Acceptance:** presheaves valued in two different \(D\) over one \(C\) are distinct objects of `Cat`, each with its own Hom; the Yoneda embedding is an actual functor whose morphism action is checked on a non-identity arrow of \(C\).

- [ ] **`sheaf-descent-subcategory`**. **Needs:** `presheaf-categories`.
...
- [ ] **`architecture-remediation`**. **Needs:** `category-graph-engine`, `presheaf-categories`, `sheaf-descent-subcategory`, `sheaf-object-placement`, `algebras-are-modules`, `arrow-category-placement`, `geometric-space-placement`, `combinatorial-object-placement`, `owner-api-convergence`, `framing-primary-epi`, `framing-specialization-convergence`, `generator-lexicon`, `ambiguous-generator-names`, `owned-provenance-data`, `refinement-convergence`, `assertion-frontiers`, `placeholder-stubs`, `categorical-representation-convergence`, `group-module-scalar-change-convergence`, `memoization-convergence`, `singular-kernel-delegation`, `torsion-action-delegation`, `imperative-algorithm-cleanup`, `owned-product-codomains`, `mathematical-return-types`, `coordinate-firewall`, `canonical-notebook-contract`, `ownership-test-contract`.
