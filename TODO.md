...
- [ ] **`objects-through-categories-schemes`**. **Needs:** none.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
  **Remaining:** The affine/projective, closed/open, product/pullback, toric and relative-family schemes now use root construction and fixed-endpoint Hom owners. The affine structure sheaf enters through `Sheaves.object`; affine associated module sheaves realize `QuasiCoherentSheaves(X)` privately; affine group schemes/actions realize their existing categories privately; and the Legendre smooth-stratum local system is literally its functor-category object, with the false `IntegralLocalSystem` and `HigherDirectImageSheaf` parents removed. The direct non-root `Parent.__init__` residue in this subtree is now confined to `gluing.py`, which belongs to the separate blocked gluing/sheaves-of-modules nodes; the surviving `schemes.py` class probes are in equality methods or the declared engine adapter. Remaining here are formal and analytic family records, AT21 engines, non-gluing covering/refinement and comparison records, geometric and log-pair placements, generic non-affine underlying-space placement, and the remaining `SageObject`/probe/exception families. Resolve the schemes-package cold-import cycle through invertible sheaves and K3/cyclic-cover families before replacing its lazy table. Renamed records and categories minted for engine outputs do not discharge these obligations.
...
- [ ] **`objects-through-categories-divisors`**. **Needs:** `objects-through-categories-schemes`, `invertible-sheaf-axiom`, `quasi-coherent-sheaf-morphisms-on-non-affine-schemes`, `sheaves-of-modules-over-a-sheaf-of-rings`, `cartier-divisors-as-sections`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`finite-affine-atlases`**. **Needs:** `objects-through-categories-abstract-categories`.
  ...
  **Observed gap:** `CoveringFamilies` now constructs its objects, and distinguished affine covers thread their defining elements through that entry, deriving charts and indices from the root family. The common `CoveringFamilies(C)` and `DescentDataOnCover` owners still fall back to root `Objects()` because the tree lacks the genuine category of indexed arrow families with overlap spans and the common category of cover-relative descent presentations; those foundations must be defined here rather than relabelled. Remaining are also the finite-atlas, refinement and descent-data classes in `schemes/gluing.py` and the geometric `CoverRefinement` record in `schemes/ringed_spaces.py`; they must construct through the covering/descent owners rather than allocating host records.
