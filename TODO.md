...
`remote-remediation-branches` precedes every other ready node: the in-flight remediation branches are absorbed before any overlapping node is worked from `main`.
...
## Remote remediation branches

- [ ] **`remote-remediation-branches`**. **Needs:** none.
  **Goal:** the complaint remediations already in flight on `origin/remediate/scheme-gluing`, `origin/remediate/schemes`, `origin/remediate/groups`, `origin/remediate/divisors`, `origin/remediate/abstract-categories`, `origin/remediate/modules`, `origin/remediate/algebra-constructor`, `origin/remediate/lattice-files` and `origin/remediate/functions-sets-tensors` are integrated into `main` instead of being redone from `main`.
...
- [ ] **`algebra-structure-morphism-constructor`**. **Needs:** `remote-remediation-branches`.
...
- [ ] **`objects-through-categories-schemes`**. **Needs:** `remote-remediation-branches`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`objects-through-categories-groups`**. **Needs:** `remote-remediation-branches`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`objects-through-categories-divisors`**. **Needs:** `glued-sheaves-through-the-sheaf-entry`, `objects-through-categories-schemes`, `invertible-sheaf-axiom`, `quasi-coherent-sheaf-morphisms-on-non-affine-schemes`, `sheaves-of-modules-over-a-sheaf-of-rings`, `cartier-divisors-as-sections`, `projectivization-functor`, `remote-remediation-branches`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`objects-through-categories-abstract-categories`**. **Needs:** `remote-remediation-branches`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`objects-through-categories-modules`**. **Needs:** `remote-remediation-branches`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`objects-through-categories-lattice-files`**. **Needs:** `remote-remediation-branches`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`objects-through-categories-functions-sets-tensors`**. **Needs:** `remote-remediation-branches`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
