...
| `C-hom-packet-inheritance-20260907-1404` | C / inherited Hom packet without endpoint reprobes | Chat takeover 2026-09-07 14:04 Asia/Taipei; `/home/dzack/research` | `src/dzack_research/preamble/categories/abstract_categories/hom_categories.py; src/dzack_research/preamble/categories/abstract_categories/arrow_categories.py` (write) | `dbaeab573ec720f99d21ebf775c956c58dbdfb3d` | 2026-09-07T06:12:29Z |
...
| C | `e472b85a` unifies ordinary-parent and fixed-Hom construction with nested refinement hooks and establishes the direct-action module consumer. `78494432` keeps endomorphism algebra enrichment over its selected commutative scalar ring. `a3017a8f` represents `EndofunctorAlgebras(T)=Inserter(T,Id)` on the existing exact arrow objects: a structure arrow `T(X) -> X` retains the exact carrier and structure morphism, the carrier functor returns `X` literally, and a structured map is forced to the square `(T(f),f)`. The two-products-on-one-`QQ`-module specimen and its nonidentity product-preserving projection are committed, unverified. | Remove endpoint membership reprobes from inherited Hom-packet traversal: membership in the base category already implies membership in each declared semantic supercategory, while reprobes re-enter selected-inclusion construction for subobjects. Reconcile `SubobjectCategory` with the owned category runtime in the same release. Category ordering, remaining defining-data conversions, discoverable contracts, and import boundaries follow. | Claimed: `C-hom-packet-inheritance-20260907-1404`; exact equipped-object prerequisite remains released at `a3017a8f` |
