...
- [ ] **`rings-are-framed-algebras-over-themselves`**. **Needs:** none.
  ...
  **Observed when placing the framing (2026-09-25):** fixing the empty framing in `_algebra_from_native_ring` for a ring over itself (source `R`, the initial `R`-algebra; identity epimorphism) places `ZZ` and `QQ` correctly, but then `QQ.coerce_map_from(ZZ)` fails: Sage's `Category._meet_` joins meets over supercategories, and the join's axiom closure now adds `Framed` to `Modules(ZZ)`, requiring `QQ` to be a framed `ZZ`-module. `Framed` is one global axiom (`objects.py`), so any join with one framed branch frames every other branch that admits the axiom; rack's `09215fd7e` made framing relative to its category, which contradicts that closure. Replacing the meet by the intersection of declared supercategories gives the right categories, but the Hom they type is the owned ring Mor, whose `homset_category()` is Sage's `Sets()`, and Sage rebuilds a weakened coercion map's parent in `Sets()`, which no owned object is in (owned `Sets` declares `Objects()` only, contrary to the `owned_category.py` module docstring). The node therefore waits on a ruling: whether `Framed` stays one global axiom or becomes relative to the category that owns the free functor.
