...
- [ ] **`objects-through-categories-scheme-gluing`**. **Needs:** `objects-through-categories-functions-sets-tensors`, `finite-affine-atlases`, `tensor-products-of-unframed-modules`.
  ...
  **Remaining:** The glued scheme now uses Schemes(R) with a private realization and the shared Hom/structure-map entry, module transition maps are arrows of the varying-ring module category, finite-atlas inverse image and pullback are owned functors, and the actual module of compatible sections is the selected `Modules(O(X))` equalizer. Remaining are finite atlases/refinements and descent data and their morphisms. `GlobalSectionModules` survives only as legacy superclass scaffolding for `GlobalSectionAlgebras`; the algebra category and `QuasiCoherentSheavesWithChosenDescentDatum` still require their genuine algebra/sheaf entries, after which that legacy module category is removed here. Retain the descent interpretation needed to validate cyclic-cover branch sections, not just component arithmetic. Algebras must use Algebras(R)(M,m), sheaves must use Sheaves.object, and literal ingress belongs to the indexed-family owner.
  **Owners:** the `Schemes(R)` glued-scheme entry; the category of finite affine atlases of `X`; the `Modules(R)` tensor construction.
  **Deliver:** the glued scheme built by its `Schemes(R)` entry on the gluing datum; atlases, finite-atlas descent data (declaring `DescentDataOnCover`) and their morphisms built by their categories; the categories `GluedSchemes`, `GlobalSectionModules`, `GlobalSectionAlgebras` and `QuasiCoherentSheavesWithChosenDescentDatum` removed, with their engines kept and their objects placed in `Schemes(R)`, `Modules(O(X))`, the algebra category over \(\mathcal{O}(X)\) and `QuasiCoherentSheaves(X)`.
...
- [ ] **`objects-through-categories-divisors`**. **Needs:** `invertible-sheaf-axiom`, `quasi-coherent-sheaf-morphisms-on-non-affine-schemes`, `cartier-divisors-as-sections`.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`finite-affine-atlases`**. **Needs:** `objects-through-categories-abstract-categories`.
  ...
  **Observed gap:** `CoveringFamilies` now constructs its objects, and distinguished affine covers thread their defining elements through that entry, deriving charts and indices from the root family. The common `CoveringFamilies(C)` and `DescentDataOnCover` owners still fall back to root `Objects()` because the tree lacks the genuine category of indexed arrow families with overlap spans and the common category of cover-relative descent presentations; those foundations must be defined here rather than relabelled. Remaining are also `RingedCoveringFamily`, `RingedCoveringOverlap` and `AffineOverlapRefinement` in `schemes/covering_families.py`, the finite-atlas/refinement/descent-data classes in `schemes/gluing.py`, and the geometric `CoverRefinement` record in `schemes/ringed_spaces.py`. Their current implementation-class/getattr routing is part of this missing owner, not a local probe-removal exercise; they must construct through the covering/descent/refinement owners rather than allocating host records.
  **Owners:** `CoveringFamilies`, `DescentDataOnCover` (`presheaves.py`); the Zariski coverage on `Sch_R/X`.
  **Deliver:** the genuine common object-category foundations of `CoveringFamilies(C)` and `DescentDataOnCover`; `CoveringFamilies` with its constructor entry; the atlas, refinement and finite-atlas descent-data categories declared into those owners; an actual represented finite-atlas site/descent object for finite-atlas sheaves; the gluing classes built through them.
  **Acceptance:** the listed `gluing.py` classes are objects of those categories built by their entries.

- [ ] **`tensor-products-of-unframed-modules`**. **Needs:** none.
  **Goal:** The tensor product \(M \otimes_R N\) of \(R\)-modules without a chosen framing, with its universal bilinear map, at `Modules(R)`.
  **Observed gap:** The general tensor quotient and binary classifier are represented at `Modules(R)` without a framing. The exact module of compatible sections is now the owner-built `Modules(O(X))` Čech equalizer, but the global-section algebra in `schemes/gluing.py` still bypasses `Algebras(R)(M,m)`; integrate its chartwise product and unit with that classifier. Keep its descent interpretation and cyclic-cover consumers intact, and retain undecided quotient equalities as `Unknown` rather than confusing representatives with quotient classes.
  **Owners:** `Modules(R)`.
...
- [ ] **`invertible-sheaf-axiom`**. **Needs:** none.
...
- [ ] **`quasi-coherent-sheaf-morphisms-on-non-affine-schemes`**. **Needs:** none.
  ...
  **Observed gap:** the owned Hom of quasi-coherent sheaves is affine only; the comparison isomorphisms of \(\mathcal{O}_X(d)\) in `divisors/invertible_sheaves.py` are hand-built.  `AffineQuasiCoherentAdjunction` in `schemes/sheaf_functors.py` is therefore still a record rather than the generic `Adjunction`: its current unit/counit are underlying module arrows, not arrows of `QuasiCoherentSheaves(X)`.  Once this Hom exists, the affine adjunction must enter the shared adjunction owner rather than wrapping it.
...
- [ ] **`cartier-divisors-as-sections`**. **Needs:** `finite-affine-atlases`.
...
- [ ] **`objects-through-categories-abstract-categories`**. **Needs:** none.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
  **Observed gap:** subobject factorization and the generic mono/epi families still use exception-based decisions; biproduct/tensor occurrence admission still probes for structure rather than following construction placement. The fixed Hom/native category-endpoint boundary and the construction of descent comparison data still require owner-level review. `CoveringFamilies(C)` and `DescentDataOnCover` now construct their objects but still declare root `Objects()` because their genuine common structured-family/descent object categories have not been built; that generic foundation is shared with `finite-affine-atlases`, not permission to relabel them. Replacing probes by class patterns, or catching the same owned exceptions in a helper named an engine adapter, is not remediation. Functor, arrow, covering-family and sheaf objects now use the owner entries, and fixed Hom categories reuse the arrow objects. Keep the selected-limit and selected-colimit engine records: they retain already-owned cone objects and their factorization rules, and are not grounds for inventing categories of engine outputs. Inspect the remaining constructor/membership occurrence family, including inherited declarations, before closing the node.
...
- [ ] **`objects-through-categories-algebras`**. **Needs:** none.
  **Goal:** Every object of this subtree is an object of its category, constructed through that category's one entry (`CON-16`) and threaded through its immediate supercategory, so that the runtime root alone realizes `Parent`/`Element`; the side attributes, probes and exception branches those hand-assembled parents needed become construction data, placement questions and `case`/`match` on placement or asserted frontiers (`POLICY.NO_EXCEPTION_CONTROL_FLOW`).
...
- [ ] **`banned-language-residue`**. **Needs:** none.
  ...
  **Observed gap:** `carrier` at 30 sites in 8 files (`abstract_categories/arrow_categories.py` 17); 2 public `.Hom(` spellings outside engine adapters.
...
- [ ] **`geometric-space-placement`**. **Needs:** none.
  ...
  **Observed gap:** the enumerated table in [COMPLAINTS.md](COMPLAINTS.md) lists each with its docstring and the category it should be declared into.  `TopologicalManifolds` needs a category of topological spaces, which is absent.  The generic `SchemeUnderlyingSpace` fallback in `schemes/ringed_spaces.py` has the same missing owner: affine schemes now use their actual prime spectra, while a non-affine ringed/scheme space cannot be honestly placed until topological/geometric spaces and continuous maps exist.  `LogPairs` is a scheme with a divisor.  `HyperbolicSpaces`, `HyperbolicPolyhedra` and `PositiveConeComponents` are subspaces and projectivizations of \(L \otimes \mathbf{R}\).  `ConvexPolytopes` and `RationalPolyhedralCones` are convex bodies and cones in \(L \otimes \mathbf{Q}\).
