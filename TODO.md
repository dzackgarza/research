...
| M | `e472b85a` constructs direct-action modules; `41d239c0` through `b67b35e3` construct localized actions. `ec7c2d00` exposes chosen finite presentations as objects of `Arr(R-Mod)` and records direct-action `ZZ/6`, free-plus-torsion, infinite-framing, and contractible-stabilization specimens. `501fb734` enriches `Hom_R(M,N)` over `Z(R)` for noncommutative `R`, with the left regular `M_2(QQ)` action as specimen. `a329daf7` makes restriction along `f:R->S` retain the exact underlying additive group and expose `rho_M compose f`. Assertions are unverified. Existing M1 releases remain unchanged. | Complete the chosen-framing construction owner and its morphisms, then reconcile the remaining tensor/Hom/biproduct/power constructors with that selected-data contract. M1 retains the remaining exact local algorithms. | M0 checkpoint: `a329daf7`; M0 remains claimed |
...
| `M0-defining-action-presentations-20260907-1538` | M0 / defining action through free and selected-presentation constructors | Chat continuation 2026-09-07; `/home/dzack/research` | `src/dzack_research/preamble/categories/modules/pure/modules.py; src/dzack_research/preamble/categories/modules/framed/framed_free_modules.py; src/dzack_research/preamble/categories/modules/framed/finitely_generated/finitely_presented_modules.py; src/dzack_research/preamble/categories/modules/module_morphisms/module_morphisms.py; src/dzack_research/preamble/categories/modules/powers.py; src/dzack_research/preamble/categories/functors/scalar_change.py; tests/modules/test_module_defining_action_constructors.py` (write) | `8a8f66d5f379725f171f3c1c7f49e0c9251d666e` | 2026-09-07T15:41:14Z |
