...
- [ ] **`objects-through-categories-scheme-gluing`**. **Needs:** `objects-through-categories-functions-sets-tensors`, `finite-affine-atlases`, `limits-of-modules-created-by-underlying-sets`, `tensor-products-of-unframed-modules`.
  ...
  **Remaining:** The glued scheme now uses Schemes(R) with a private realization and the shared Hom/structure-map entry, module transition maps are arrows of the varying-ring module category, and finite-atlas inverse image and pullback are owned functors. Remaining are finite atlases/refinements and descent data and their morphisms. GlobalSectionModules, GlobalSectionAlgebras and QuasiCoherentSheavesWithChosenDescentDatum still require their genuine module/algebra/sheaf entries. Retain the descent interpretation needed to validate cyclic-cover branch sections, not just component arithmetic. Algebras must use Algebras(R)(M,m), sheaves must use Sheaves.object, and literal ingress belongs to the indexed-family owner.
  **Owners:** the `Schemes(R)` glued-scheme entry; the category of finite affine atlases of `X`; the `Modules(R)` limit and tensor constructions.
