...
| **22. Induced Mor transport confuses an image arrow with its constructed categorical object.** | `CAT-23`, `OWN-02`, `OWN-07` | In `functors/mor_packets.py`, `_InducedMorFunctor.object_image` computes a raw image arrow, then tests it with `image in self.codomain()`. An unenriched `FixedMorCategory` now reads object placement, so an admitted arrow still needs that category's `object` construction. The End and Aut transport routes already invoke their target constructor, while the Mor route returns the raw arrow directly. Reconcile all three with their identity 2-arrow action and preserve the independent element behavior of enriched Mor parents. Observed by source review; the repair remains in `membership-by-placement`, with execution deferred to T. |
