...
| **23. Core and wide-subcategory endpoint admission is inconsistent across the construction entry.** | `CAT-23`, `OWN-02`, `OWN-13` | In the current inherited `abstract_categories/arrow_categories.py` changes, `_CoreCategory.Mor` and `_WideSubcategory.Mor` admit endpoints in their base category, while their delegated same-object containment has been removed. Both then call `_MorCategoryOf.Of`, which still requires endpoints in the core/wide category itself. The local base-category check does not establish that construction placement. Reconcile the same-object definition, endpoint construction and fixed-Mor admission together, retaining the selected arrow predicates. The inherited changes remain separate; source observation under `membership-by-placement`, not a runtime result. |
| **24. The induced higher-Mor helper equates admitted 2-arrows with identities.** | `CAT-23`, `OWN-07`, `CAT-21` | `_identity_two_arrow_image` in `functors/mor_packets.py` asserts membership in the source 2-Mor and returns a target identity. This is the discrete-Mor action, but `_FunctorCategory.Mor` in `abstract_categories/cat.py` represents actual natural transformations, not only identities. Membership alone does not supply the identity hypothesis or the needed nonidentity higher action. Trace the represented functor-category regime and its specified action before treating this helper as general Mor transport. The discrete object/identity action has source specimens; the non-discrete route remains in `membership-by-placement`, with runtime work deferred to T. |
