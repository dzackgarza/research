...
| **20. Membership in a Mor family still selects a construction instead of reading its placement.** | `CAT-23`, `OWN-02`, `DEV-68` | In current `abstract_categories/mor_categories.py`, `_MorCategoryOf.__contains__` calls `self.Of(domain, codomain)` after checking the candidate's endpoints. A membership query can therefore select or construct a fixed Mor and populate the family's endpoint cache. The constructor must establish the relevant family placements, including inherited families sharing one Mor, without relying on a later containment query. This remains part of `membership-by-placement` in TODO; no runtime behavior has been claimed from source inspection. |
