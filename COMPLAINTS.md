...
| **9. Visible mathematical placeholder stubs remain.** | `STY-48`, `STY-160` | `tensor.py:312–315` implements `_index_ranks()` as unconditional `assert False`; `tensor.py:343–345` does the same for `tensor_valence()`. The previously recorded profinite abstract-method placeholder is no longer present in current source. |
