...
| **21. The Mor-family graph conflates inclusions of arrows with membership of the fixed categories themselves.** | `CAT-21`, `CAT-23`, `OWN-13` | A family `Mors(C)` is documented to have the selected fixed categories `Mor_C(A,B)` as objects. In `abstract_categories/mor_categories.py`, `_AutCategoryOf.super_categories()` declares `Auts(C) -> Ends(C)`, but their constructors select `Iso_C(A,A)` and `Mor_C(A,A)`, respectively; these are generally distinct categories. `_RestrictedMorCategoryOf.super_categories()` likewise declares the Mono/Epi families below the Mor family, although their selected restricted categories are distinct from the full Mor. The inclusions of their **arrows** belong to the fixed categories' own graph and do not by themselves place those entire categories in one another's classifying families. Reconcile the family declarations, constructor-owned placements and containment, including cross-base family edges; retain genuine transport as explicit functorial structure rather than predicate membership. Source observation only; this is unfinished work in `membership-by-placement`. |
