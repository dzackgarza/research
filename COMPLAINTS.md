...
| --- | --- | --- |
| **1. `preamble.all` is still a global operation catalogue.** | `ARC-12`, `API-07`, `STY-01`–`04`, `OWN-02` | `all.py:30–106` still exports `Biproduct`, `Coproduct`, `Core`, `Product`, `Pushout`, `TensorProduct`, `TensorSquare`, etc.; `all.py:1159–1194` exports `AdicCompletion`, `Localization`, `QuotientRing`, etc. A signature scan finds **261 exported top-level functions across 107 source files** whose arguments include an existing mathematical object and therefore fall directly under `ARC-12`'s owner-in-argument ban. |
...
| **20. The test harness currently hides rather than detects the global-operation violation.** | `DEV-06`, `DEV-37`, `DEV-43`, `STY-118`, `STY-125` | `tests/conftest.py:30–34` states that the session “no longer publishes” owner-in-argument globals. That is false at current HEAD. It then injects replacement lambdas with `module.__dict__.setdefault(name, owned)` at `94–101`; because `preamble.all` already imported many forbidden globals, `setdefault` leaves the forbidden implementation untouched. There are **135 old-global call sites in tests**, and **51 test files import `preamble.all` via `*`**. Consequently passing tests do not establish that the migration happened. |
