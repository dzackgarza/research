...
The glued scheme and its Hom now use `Schemes(R)` through a private realization (`6faa74327`, `755c1250b`). The remaining atlas, refinement and finite-atlas descent records still need their owners. Module-valued \(\Gamma(X,F)\) is now the selected equalizer in `Modules(O(X))`; `GlobalSectionModules(R)` remains only as legacy superclass scaffolding for the still-local `GlobalSectionAlgebras(R)` construction and is removed with that later placement work.
...
Scheduled in [TODO.md](TODO.md) as `quasi-coherent-sheaf-morphisms-on-non-affine-schemes`, `invertible-sheaf-axiom`, `cartier-divisors-as-sections`, `finite-affine-atlases` and `tensor-products-of-unframed-modules`.
